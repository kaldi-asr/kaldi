...
// Options struct for class BackoffLmForSampling.
struct BackoffLmForSamplingOptions {
  ...
  BackoffLmForSamplingOptions(): vocab_size(-1),
                                 ngram_order(3),
  ...
  void Register(OptionsItf *po) {
    ...
    po->Register("discounting-constant", &discounting_constant, "Constant for "
                 "absolute discounting (you likely won't have to change this).");
...
class BackoffLmForSampling {
 public:
  ///  Constructor.  Retains a reference to 'config'.
  BackoffLmForSampling(const BackoffLmForSamplingOptions &config);
  ...
  */
  ...
  // Estimates the language model (internal representation); includes discounting.
  void Estimate();
 ...
 private:
  ...
  struct Count {
    ...
    double count;  // total count for this word.
  ...
  */
  ...
  const BackoffLmForSamplingOptions &config_;
