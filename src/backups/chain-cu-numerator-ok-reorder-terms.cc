...
#endif
  {
    ...
    for (int32 s = 0; s < num_sequences; s++) {
      BaseFloat inv_arbitrary_scale =
          prev_alpha[max_num_hmm_states * num_sequences + s];
      for (int32 h = 0; h < num_graph_.NumStates()[s]; h++) {
        ...
        for (; trans_iter != trans_end; ++trans_iter) {
          ...
          this_tot_alpha += this_prev_alpha / inv_arbitrary_scale * transition_prob * prob;
    ...
    for (int32 s = 0; s < num_sequences; s++) {
      for (int32 h = 0; h < num_graph_.NumStates()[s]; h++) {
        ...
        for (; trans_iter != trans_end; ++trans_iter) {
          ...
          BaseFloat occupation_prob = variable_factor * this_alpha_prob; //occupation_factor;
