...
//          this_tot_alpha += Exp(Log(this_prev_alpha) + Log(transition_prob) + Log(prob) - Log(arbitrary_scale));
...
//  std::cout << "Alphas (Before) for seq 7, t= " << t << ":\n";
...
//  std::cout << std::endl;
  ...
  CuSubVector<BaseFloat> alpha_sum_vec(this_alpha +
                                     num_graph_.MaxNumStates() * num_sequences_,
                                     num_sequences_);
  //for (int h = 0; h < max_num_hmm_states; h++)
  //  std::cout << "alpha for t=" << t << ", h=" << h << " is " << alpha_mat(h, 7) << "\n";
  alpha_sum_vec.AddRowSumMat(1.0, alpha_mat, 0.0);
  alpha_sum_vec.InvertElements();
  KALDI_LOG << "inv-al-sum for seq=7, t=" << t << " is " << alpha_sum_vec(7);

  alpha_mat.MulColsVec(alpha_sum_vec);  

//  std::cout << "Alphas (**After**) for seq 7, t= " << t << ":\n";
//  for (int h = 0; h < num_graph_.NumStates()[7]; h++)
//    std::cout << alpha_mat(h, 7) << " ";
//  std::cout << std::endl;
...
BaseFloat CuNumeratorComputation::ComputeTotLogLike() {
  ...
  CuSubMatrix<BaseFloat> inv_arbitrary_scales(
      alpha_, 0, frames_per_sequence_ + 1,      /////////////////
    ...
    for (int32 s = 0; s < num_sequences; s++) {
      for (int32 h = 0; h < num_graph_.NumStates()[s]; h++) {
        BaseFloat this_alpha_prob = this_alpha[h * num_sequences + s],
                  inv_arbitrary_scale =
                             next_alpha[max_num_hmm_states * num_sequences + s];
        ...
        for (; trans_iter != trans_end; ++trans_iter) {
          ...
          BaseFloat occupation_prob = variable_factor * this_alpha_prob * inv_arbitrary_scale;////////////// occupation_factor;
          BaseFloat old = log_prob_deriv_data[pdf_id * deriv_stride + s];
          log_prob_deriv_data[pdf_id * deriv_stride + s] += occupation_prob;
          if (log_prob_deriv_data[pdf_id * deriv_stride + s] - log_prob_deriv_data[pdf_id * deriv_stride + s] != 0) {
            KALDI_LOG << "t: " << t << ", seq: " << s << ", h: " << h << ", pdf_id: " << pdf_id
            << ", var-factor: " << variable_factor 
            << ", ocup-factor: " << occupation_factor
            << ", old: " << old
            << ", arbitrary_scale: " << inv_arbitrary_scale;
