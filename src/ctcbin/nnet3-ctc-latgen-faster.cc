// nnet3bin/nnet3-ctc-latgen-faster.cc
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    const char *usage =
        "Generate lattices using nnet3+ctc neural net model.\n"
        "Usage: nnet3-ctc-latgen-faster [options] <nnet-in> <fst-in|fsts-rspecifier> <features-rspecifier>"
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) == kNoRspecifier) {
...

      {
        ...
        for (; !feature_reader.Done(); feature_reader.Next()) {
          ...
          double like;
          if (DecodeUtteranceLatticeFasterCctc(
