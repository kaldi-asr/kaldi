// ctcbin/nnet3-ctc-subset-egs.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Creates a random subset of the input nnet3+ctc examples, of a specified size.\n"
        "Uses no more memory than the size of the subset.\n"
        "\n"
        "Usage:  nnet3-ctc-cubset-egs [options] <egs-rspecifier> [<egs-wspecifier2> ...]\n"
        "\n"
        "e.g.\n"
        "nnet3-ctc-subset-egs [args] ark:- | nnet-subset-egs --n=1000 ark:- ark:subset.cegs\n";
    ...
    std::vector<std::pair<std::string, NnetCctcExample> > egs;
    egs.reserve(n);
