...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Copy examples for nnet3+ctc network training, possibly changing the binary mode.\n"
        "Supports multiple wspecifiers, in which case it will write the examples\n"
        "round-robin to the outputs.\n"
        "\n"
        "Usage:  nnet3-ctc-copy-egs [options] <egs-rspecifier> <egs-wspecifier1> [<egs-wspecifier2> ...]\n"
        "\n"
        "e.g.\n"
    ...
    std::vector<NnetCctcExampleWriter*> example_writers(num_outputs);
