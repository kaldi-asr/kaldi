...
/**
  ...
  \f$\lambda\f$ using:
    \f[\lambda = \alpha/(N D) trace(R^T, R)\f]
  for some small \f$\alpha\f$ such as \f$\alpha = 0.1\f$.  However, we leave
  ...
  might want to set \f$\lambda\f$ using "other" data, e.g. a previous
  minibatch.

  The output of this function is a matrix \f$P\f$, each row \f$p_i\f$ of
  which is related to \f$r_i\f$ by:
    \f[p_i = G_i r_i\f]
  Here, \f$p_i\f$ is preconditioned by an estimated Fisher matrix
...
/**
   ...
   using \f$\lambda = \alpha/(N D) trace(R^T, R)\f$, and calls
