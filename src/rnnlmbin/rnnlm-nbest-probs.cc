...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "This program computes the probability per word of the given text data.\n"
        "This program doesn't train, and doesn't write the model;\n"
        "it just prints the probability to the standard output (in addition\n"
        "to printing various diagnostics to the standard error).\n"
        "\n"
        "Usage:\n"
        " rnnlm-nbest-prob [options] <rnnlm> <word-embedding-matrix> <text>\n"
        "e.g.:\n"
        " rnnlm-nbest-prob final.raw 0.word_embedding data/rnnlm/eval2000.txt\n"
