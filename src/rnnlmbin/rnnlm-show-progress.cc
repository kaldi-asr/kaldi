...
//    }
    ...
    if (!examples_rspecifier.empty()) {
      ...
      int32 num_updatable = NumUpdatableComponents(nnet1.Nnet()) + 2;
      ...
      for (int32 s = 0; s < num_segments; s++) {
        ...
        LmNnet interp_nnet(nnet2);
//        ScaleNnet(middle, &interp_nnet);
//        AddNnet(nnet1, 1.0 - middle, &interp_nnet);
        interp_nnet.Scale(middle);
        interp_nnet.Add(nnet1, 1.0 - middle);

        LmNnetComputeProb prob_computer(compute_prob_opts, interp_nnet);
    ...
    { // Get info about magnitude of parameter change.
      LmNnet diff_nnet(nnet1);
//      AddNnet(nnet2, -1.0, &diff_nnet);
      diff_nnet.Add(nnet2, -1.0);

      int32 num_updatable = NumUpdatableComponents(diff_nnet.Nnet()) + 2;
