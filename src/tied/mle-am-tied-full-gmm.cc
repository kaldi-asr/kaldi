...
void MleAmTiedFullGmmUpdate(
  ...
  for (size_t i = 0; i < acc.NumFullAccs(); i++) {
    ...
    KALDI_LOG << "MleFullGmmUpdate pdf(" << i << ") delta-obj=" << (tmp_obj_change/tmp_count) << " count=" << tmp_count;
  ...
  if (config_tied.interpolate()) {
    ...
    if ((flags & kGmmMeans) && config_tied.interpolate_means) {
      KALDI_LOG << "...means";
      for (int32 i = 0; i < model->NumPdfs(); ++i)
        model->GetPdf(i).SmoothWithFullGmm(config_tied.interpolation_weight, &(oldm->GetPdf(i)), kGmmMeans);
    }

    if ((flags & kGmmVariances) && config_tied.interpolate_variances) {
      KALDI_LOG << "...variances";
      for (int32 i = 0; i < model->NumPdfs(); ++i)
        model->GetPdf(i).SmoothWithFullGmm(config_tied.interpolation_weight, &(oldm->GetPdf(i)), kGmmVariances);
    }
