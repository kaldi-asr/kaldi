...
AccumDiagGmm& AccumAmTiedDiagGmm::GetDiagAcc(int32 codebook_index) const {
...
BaseFloat AccumAmTiedDiagGmm::AccumulateForTied(const AmTiedDiagGmm &model,
...
void AccumAmTiedDiagGmm::AccumulateFromPosteriors(
  ...
  KALDI_ASSERT(codebook_index < NumDiagAccs());
...
void AccumAmTiedDiagGmm::Read(std::istream &in_stream, bool binary, bool add) {
  ...
  if (!add || (add && gmm_accumulators_.empty())) {
    ...
    for (vector<AccumDiagGmm*>::iterator it = gmm_accumulators_.begin(),
...
void AccumAmTiedDiagGmm::Write(std::ostream &out_stream, bool binary) const {
  ...
  for (vector<AccumDiagGmm*>::const_iterator it = gmm_accumulators_.begin(),
       end = gmm_accumulators_.end(); it != end; ++it) {
...
void MleAmTiedDiagGmmUpdate(
  ...
  if (flags & kGmmWeights) {
    ...
    for (size_t i = 0; i < acc.NumTiedAccs(); i++) {
      ...
      if (obj_change_out_tied != NULL) *obj_change_out_tied += tmp_obj_change;
