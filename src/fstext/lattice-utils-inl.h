...
void PruneCompactLattice(
    Weight beam,
    MutableFst<ArcTpl<CompactLatticeWeightTpl<Weight, Int> > > *fst) {  
  typedef ArcTpl<CompactLatticeWeightTpl<Weight, Int> > Arc;
  typedef typename Arc::StateId StateId;
  typedef typename Arc::Label Label;
  typedef Fst<Arc> F;
  typedef MutableFst<Arc> MF;
  typedef CompactLatticeWeightTpl<Weight, Int> CWeight;
  
  if (fst->Start() == kNoStateId) {
    KALDI_WARN << "Pruning empty lattice.";
    return;
  }
  StateId num_states = fst->NumStates();

  if (fst->Properties(kCyclic, true) != 0) {
    KALDI_ERR << "Trying to prune compact-lattice that has cycles "
        "(perhaps your decoding graph had cycles or epsilon self-loops?)";
  }
  
  TopSort(fst); // Make it topologically sorted.  This is always possible,
  // as lattices are acyclic.
  
  std::vector<Weight> forward_weight;
  try {
    forward_weight.resize(num_states, Weight::Zero());
  } catch (std::bad_alloc) {
    KALDI_WARN << "Allocation error pruning lattice.";
    throw;
  }
  std::vector<Weight> &backward_weight(forward_weight); // Semantically
  // different but share the same memory.
  Weight total_weight = Weight::Zero();
  forward_weight[fst->Start()] = Weight::One();
  // work out forward weight.
  for (StateId state = 0; state < num_states; state++) {
    for (ArcIterator<F> aiter(*fst, state); !aiter.Done(); aiter.Next()) {
      const Arc &arc = aiter.Value();
      assert(arc.nextstate > state); // This might not work correctly with
      // self-loops, for general semirings.  But for idempotent semirings should
      // be OK, as long as weight is nonnegative.. change code if it's a problem.
      forward_weight[arc.nextstate] = Plus(forward_weight[arc.nextstate],
                                           Times(forward_weight[state],
                                                 arc.weight.Weight()));
    }
    total_weight = Plus(total_weight, Times(forward_weight[state],
                                            fst->Final(state).Weight()));
  }
  Weight cutoff_weight = Times(total_weight, beam);
  // Work out backward weight, and prune away some arcs and states.
  // Arcs we don't want will have their next-state set to kNoStateId
  assert(static_cast<StateId>(-1) < static_cast<StateId>(0)); // Next loop will
  // never terminate if StateId is unsigned-- but it's not, in current code.
  StateId unreachable_state = kNoStateId,
      num_states_pruned = 0,
      num_arcs_pruned = 0;
  for (StateId state = num_states-1; state >= 0; state--) {
    Weight this_forward_weight = forward_weight[state];
    backward_weight[state] = fst->Final(state).Weight();
    bool kept_any_arc = false;
    for (MutableArcIterator<MF> aiter(fst, state); !aiter.Done(); aiter.Next()) {
      Arc arc = aiter.Value();
      Weight this_backward_weight = Times(backward_weight[arc.nextstate],
                                          arc.weight.Weight());
      if (Compare(Times(this_forward_weight,
                        this_backward_weight), cutoff_weight) < 0) {
        // Note: Compare is not a standard OpenFst template, but a kind of Kaldi
        // extension that is always defined for things we template Lattice on.
        arc.weight = CWeight::Zero(); // Caution: in general you're not supposed to
        // set arc's weights to zero.  We're just doing this temporarily while
        // we work out which unreachable state we should make it point to.
        aiter.SetValue(arc);
        num_arcs_pruned++;
      } else {
        kept_any_arc = true;
      }
      backward_weight[state] = Plus(backward_weight[state], this_backward_weight);
    }
    if (!kept_any_arc)
      fst->DeleteArcs(state); // kept no arcs.
    if (Compare(Times(this_forward_weight, fst->Final(state).Weight()),
                cutoff_weight) < 0) {
      // final-prob is pruned.
      fst->SetFinal(state, CWeight::Zero());
    }
    if (!kept_any_arc && fst->Final(state) == CWeight::Zero()) {
      unreachable_state = state;
      num_states_pruned++;
    }
  }
  KALDI_VLOG(2) << "Pruned " << num_states_pruned << " out of "
                << num_states << " states; pruned " << num_arcs_pruned
                << " arcs.";
  if (unreachable_state == kNoStateId)
    unreachable_state = fst->AddState();
  for (StateId state = num_states-1; state >= 0; state--) {
    for (MutableArcIterator<MF> aiter(fst, state); !aiter.Done(); aiter.Next()) {
      Arc arc = aiter.Value();
      if (arc.weight == CWeight::Zero()) { // weight is zero... this means we
        // set it to zero, but note that OpenFst does not treat zero weights as
        // valid.  So set it to one, and make it go to unreachable state.
        // We'll remove it later with Connect().
        arc.weight = CWeight::One();
        arc.nextstate = unreachable_state;
        aiter.SetValue(arc);
      }
    }
  }
  Connect(fst); // Will remove arcs that point to the unreachable state.
  KALDI_VLOG(2) << "Num-states now " << fst->NumStates();
}
