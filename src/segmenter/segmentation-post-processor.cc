...
static inline bool IsMergingLabelsToBeDone(const SegmentationPostProcessingOptions &opts) {
  return (!opts.merge_labels_csl.empty() || opts.merge_dst_label != -1);
}

static inline bool IsPaddingSegmentsToBeDone(const SegmentationPostProcessingOptions &opts) {
  return (opts.pad_label != -1 || opts.pad_length != -1);
}

static inline bool IsShrinkingSegmentsToBeDone(const SegmentationPostProcessingOptions &opts) {
  return (opts.shrink_label != -1 || opts.shrink_length != -1);
}

static inline bool IsBlendingShortSegmentsToBeDone(const SegmentationPostProcessingOptions &opts) {
  return (opts.blend_short_segments_class != -1 || opts.max_blend_length != -1);
}

static inline bool IsRemovingSegmentsToBeDone(const SegmentationPostProcessingOptions &opts) { 
  return (!opts.remove_labels_csl.empty()); 
}

static inline bool IsMergingAdjacentSegmentsToBeDone(const SegmentationPostProcessingOptions &opts) {
  return (opts.merge_adjacent_segments);
}

static inline bool IsSplittingSegmentsToBeDone(const SegmentationPostProcessingOptions &opts) {
...
void SegmentationPostProcessor::DoBlendingShortSegments(Segmentation *seg) const {
...
void SegmentationPostProcessor::DoMergingAdjacentSegments(Segmentation *seg) const {
