...
void SimpleOptions::Register(const std::string &name,
  ...
  option_infos_.push_back(std::make_pair(name, OptionInfo(doc, BOOL, value)));
}  
...
void SimpleOptions::Register(const std::string &name,
                                    ...
                                    const std::string &doc) {
  int_map_[name] = value;
  option_infos_.push_back(std::make_pair(name, OptionInfo(doc, INT32, value)));
}  
...
void SimpleOptions::Register(const std::string &name,
  ...
  option_infos_.push_back(std::make_pair(name, OptionInfo(doc, UINT32, value)));
}  
...
void SimpleOptions::Register(const std::string &name,
  ...
  option_infos_.push_back(std::make_pair(name, OptionInfo(doc, FLOAT, value)));
}  
...
void SimpleOptions::Register(const std::string &name,
  ...
  option_infos_.push_back(std::make_pair(name, OptionInfo(doc, DOUBLE, value)));
}  
...
void SimpleOptions::Register(const std::string &name,
  ...
  option_infos_.push_back(std::make_pair(name, OptionInfo(doc, STRING, value)));
}  
...
bool SimpleOptions::SetOption(const std::string &key, const int32 &value) {
  if (!SetOptionImpl(key, value, int_map_)) {
    if (!SetOptionImpl(key, (uint32)value, uint_map_)) {
      return false;
    }
  }
  return true;
...
bool SimpleOptions::SetOption(const std::string &key, const uint32 &value) {
  if (!SetOptionImpl(key, value, uint_map_)) {
    if (!SetOptionImpl(key, (int32)value, int_map_)) {
      return false;
    }
  }
  return true;
...
bool SimpleOptions::SetOption(const std::string &key, const float &value) {
  if (!SetOptionImpl(key, value, float_map_)) {
    if (!SetOptionImpl(key, (double)value, double_map_)) {
      return false;
    }
  }
  return true;
...
bool SimpleOptions::SetOption(const std::string &key, const double &value) {
  if (!SetOptionImpl(key, value, double_map_)) {
    if (!SetOptionImpl(key, (float)value, float_map_)) {
      return false;
    }
  }
  return true;
...
bool SimpleOptions::SetOption(const std::string &key, const std::string &value) {
...
std::vector<std::pair<std::string, SimpleOptions::OptionInfo> > SimpleOptions::GetOptionInfos() {
  return option_infos_;
}
 
bool SimpleOptions::GetOptionType(const std::string &key, OptionType &type) {
  for( std::vector <std::pair<std::string, OptionInfo> >::iterator dx = option_infos_.begin(); dx != option_infos_.end(); dx++) {
