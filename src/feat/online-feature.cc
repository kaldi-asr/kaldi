...
extern unsigned long long srfft_compute_time;
extern unsigned long long power_spectrum_compute_time;
extern unsigned long long mel_banks_compute_time;
extern unsigned long long mel_engergies_log_time;
extern unsigned long long dct_addmatvev_time;
...
RecyclingVector::RecyclingVector(int items_to_hold) : items_to_hold_(items_to_hold == 0 ? -1 : items_to_hold),
    ...
    BaseFloat sampling_rate, const VectorBase<BaseFloat> &original_waveform)
{
  ...
  ComputeFeatures();
  TEST_TIME(end_time);
  std::cout << "\033[0;35mAcceptWaveform: Compute features time " << end_time - resize_time << " ms. \033[0;39m" << std::endl;
}
...
OnlineCmvnState::OnlineCmvnState(const OnlineCmvnState &other) : speaker_cmvn_stats(other.speaker_cmvn_stats),
...
OnlineCmvn::OnlineCmvn(const OnlineCmvnOptions &opts,
                       ...
                       OnlineFeatureInterface *src) : opts_(opts), temp_stats_(2, src->Dim() + 1),
...
OnlineDeltaFeature::OnlineDeltaFeature(const DeltaFeaturesOptions &opts,
                                       OnlineFeatureInterface *src) : src_(src), opts_(opts), delta_features_(opts) {}
