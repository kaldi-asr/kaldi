...
struct Link {
  ...
  fst::StdArc::Label i_label;
  fst::StdArc::Label o_label;
   ...
   * constructor
   * @param next_tok
   * @param i_label input label in FST arc.
   * @param o_label output label in FST arc.
   * @param graph_cost
   * @param acoustic_cost
   */
  explicit Link(Token *next_tok, fst::StdArc::Label i_label,
                fst::StdArc::Label o_label, BaseFloat graph_cost,
                BaseFloat acoustic_cost)
      : next_tok(next_tok), i_label(i_label), o_label(o_label),
...
struct StdToken {
  ...
  std::forward_list<Link<StdToken>> links;
  ...
  StdToken(BaseFloat tot_cost, BaseFloat extra_cost, Token *backpointer)
      : tot_cost(tot_cost), extra_cost(extra_cost) {}
};

/**
 * BackpointerToken is like Token but also Standard token type for
 * LatticeFasterDecoder. Each active HCLG(decoding-graph) state on each frame
 * has one token.
 */
struct BackpointerToken {
  ...
  std::forward_list<Link<BackpointerToken>> links;
  ...
  BackpointerToken(BaseFloat tot_cost, BaseFloat extra_cost,
                   Token *backpointer) :
      tot_cost(tot_cost), extra_cost(extra_cost), backpointer(backpointer) {}
};
...
class LatticeFasterDecoderTpl {
 public:
  ...
  int32 NumFramesDecoded() const noexcept {
    return static_cast<int32>(active_toks_.size()) - 1;
  }
 ...
 protected:
  ...
  // list of Tokens of per-frame (list is in topological order),
  ...
  // Lists of tokens, indexed by frame (members of TokenList are toks,
  // must_prune_forward_links, must_prune_tokens).
  std::vector<TokenList> active_toks_;

  // This contains, for each frame, an offset that was added to the acoustic
  // log-likelihoods on that frame in order to keep everything in a nice dynamic
  // range i.e.  close to zero, to reduce roundoff errors.
 ...
 private:
  ...
  // less far.
  void PruneActiveTokens(BaseFloat delta) noexcept;

  // Prune away any tokens on this frame that have no forward links.
  // [we don't do this in PruneForwardLinks because it would give us
  // a problem with dangling pointers].
  // It's called by PruneActiveTokens if any forward links have been pruned
  void PruneTokensForFrame(int32 frame_plus_one) noexcept;
  ...
  inline Token *FindOrAddToken(StateId state,
                               decltype(active_toks_.size()) frame_plus_one,
                               BaseFloat tot_cost, Token *backpointer,
                               bool *changed) noexcept;

  void PossiblyResizeHash(size_t num_toks) noexcept;

  // There are various cleanup tasks... the toks_ structure contains singly
  // linked lists of Token pointers, where Elem is the list type. It also
  // indexes them in a hash, indexed by state (this hash is only maintained for
  // the most recent frame).  toks_.Clear()
  // deletes them from the hash and returns the list of Elems.  The function
  // DeleteElems calls toks_.Delete(elem) for each elem in the list, which
  // returns ownership of the Elem to the toks_ structure for reuse, but does
  // not delete the Token pointer.  The Token pointers are reference-counted
  // and are ultimately deleted in PruneTokensForFrame, but are also linked
  // together on each frame by their own linked-list, using the "next" pointer.
  // We delete them manually.
  void DeleteElems(Elem *list) noexcept;

  /**
   * A cleanup routine, at utt end/begin
   */
  void ClearActiveTokens() noexcept;
   ...
   */
  static std::vector<const Token *>
  TopSortTokens(const std::list<Token> &toks) noexcept;

  /**
   * update token set and a token to position map.
   * Only be called by TopSortTokens.
   */
  static void
  UpdateTokSetAndTok2Pos(const std::forward_list<decoder::Link<Token>> &links,
                         int32 pos, unordered_set<const Token *> *process,
                         unordered_map<const Token *, int32> *token2pos,
                         int32 *cur_pos) noexcept;
  ...
  unsigned num_toks_; // current total #toks allocated.
  bool warned_;
  BaseFloat final_relative_cost_;
  const FST *fst_;  // fst_ is a pointer to the FST we are decoding from.
