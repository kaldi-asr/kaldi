...
struct StdToken {
  ...
  // identitfy the token is in current heap or not(-1). Point out the position
  // in current heap so that fix the heap after updating the cost of an existing
  // token is more convience and faster
  size_t position_in_heap;
  ...
  inline StdToken(BaseFloat tot_cost, BaseFloat extra_cost, StateId state_id,
      ...
      links(links), next(next), position_in_heap(-1) { }
};
...
class LatticeFasterDecoderCombineTpl {
 ...
 protected:
  ...
  // It is a minimum heap (A[parent(i)] <= A[i]) since the lower cost the better
  // in decoding.
  // The index of node of the heap is zero-beased.
  // Given the parent is i, the left child is 2*i+1 and the right child is
  // 2*i + 2
  // Given the child is i, the parent is "( (i+1) / 2) - 1"
  struct TokenHeap {
    std::vector<Token*> elements;

    inline void Siftup(size_t child_index, size_t length) {
      while(true) {
        if (child_index == 0) break;  // it doesn't have parent node
        KALDI_ASSERT(child_index < length);
        size_t parent_index = (child_index + 1) / 2 - 1;
        if (elements[child_index]->tot_cost < elements[parent_index]->tot_cost) {
          // Update the index of token
          elements[parent_index]->position_in_heap = child_index;
          elements[child_index]->position_in_heap = parent_index;
          // Swap
          std::swap(elements[parent_index], elements[child_index]);
          // Update child_index for next turn
    ...
    inline void Siftdown(size_t parent_index, size_t length) {
      while(true) {
        if (parent_index >= elements.size() / 2) break; 
        // Prepare indexes
        size_t left_child_index = parent_index * 2 + 1;
        size_t right_child_index = parent_index * 2 + 2;
        size_t largest = parent_index;
        // Get the largest index
        if (left_child_index < length &&
            elements[left_child_index]->tot_cost < elements[largest]->tot_cost) {
          largest = left_child_index;
        }
        if (right_child_index < length &&
            elements[right_child_index]->tot_cost < elements[largest]->tot_cost) {
          largest = right_child_index;
        }
        // Swap
        if (largest != parent_index) {
          // Update the index of token
          elements[largest]->position_in_heap = parent_index;
          elements[parent_index]->position_in_heap = largest;
          // Swap
          std::swap(elements[parent_index], elements[largest]);
          // Update parent_index for next turn
    ...
    inline Token* Top() {
      KALDI_ASSERT(!elements.empty());
      return elements[0];
    }

    inline void Pop() {
      // Set the position
      elements[0]->position_in_heap = -1;

      // Swap with the last element of the heap
      std::swap(elements[0], elements[elements.size() - 1]);
      elements[0]->position_in_heap = 0;

      // Delete it from heap
      elements.erase(elements.end() - 1);

      // Tune the position from top to down
      Siftdown(0, elements.size());
    }

    // Push a new token into the heap
    inline void Push(Token* tok) {
      KALDI_ASSERT(tok->position_in_heap == -1);  // not in heap
      // Push to the end of the heap
      tok->position_in_heap = elements.size();
      elements.push_back(tok);
      // Tune
      Siftup(tok->position_in_heap, elements.size());
    }
    ...
    // Build Heap. The complexity of the function is O(n) rather than O(nlogn),
    // as the series convergence.
    inline Token* BuildTokenHeap(const TokenList &token_list, size_t num) {
      KALDI_ASSERT(elements.empty());
      elements.reserve(num * 1.5);
      // Add elements
      ...
      // Sort with Siftdown
      size_t start = elements.size() / 2 - 1; // start is the index of the last
                                              // parent node
  ...
  // must_prune_tokens).
  TokenHeap cur_heap_;  // temp variable used in ProcessForFrame
