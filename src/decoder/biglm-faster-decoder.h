...
#include "decoder/deterministic-composer.h"
...
class BigLmFasterDecoder {
 public:
  ...
  ~BigLmFasterDecoder() {
    ...
    delete composed_fst_;
  }

  void Decode(DecodableInterface *decodable) {
    // clean up from last time:
    ClearToks(toks_.Clear());
    StateId start_state = composed_fst_->Start();
  ...
  bool GetOutput(bool is_final, fst::MutableFst<fst::StdArc> *fst_out) {
    ...
    if (!is_final) {
      ...
      for (Elem *e = toks_.GetList(); e != NULL; e = e->tail) {
        Weight this_weight = Times(e->val->weight, composed_fst_->Final(e->key));
    ...
    assert(arcs_reverse.back().nextstate == composed_fst_->Start());
    ...
    if (is_final)
      fst_out->SetFinal(cur_state, composed_fst_->Final(best_tok->arc_.nextstate));
    else
 ...
 private:
  ...
  BaseFloat ProcessEmitting(DecodableInterface *decodable, int frame) {
    ...
    if (best_elem) {
      ...
      for (DeterministicComposer::ArcIterator aiter(composed_fst_, state);
          !aiter.Done();
          aiter.Next()) {
        Arc arc = aiter.Value();
  ...
  DeterministicComposer* composed_fst_;
