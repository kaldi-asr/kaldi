...
LatticeFasterDecoderCombineTpl<FST, Token>::LatticeFasterDecoderCombineTpl(
  ...
  config.Check();
  prev_toks_ = new StateIdToTokenMap();
  prev_toks_->SetSize(1000);
  cur_toks_ = new StateIdToTokenMap();
...
template <typename FST, typename Token>
void LatticeFasterDecoderCombineTpl<FST, Token>::DeleteElems(
    Elem *list, HashList<StateId, Token*> *toks) {
...
template <typename FST, typename Token>
LatticeFasterDecoderCombineTpl<FST, Token>::~LatticeFasterDecoderCombineTpl() {
  DeleteElems(cur_toks_->Clear(), cur_toks_);
  ClearActiveTokens();
...
void LatticeFasterDecoderCombineTpl<FST, Token>::InitDecoding() {
  ...
  DeleteElems(prev_toks_->Clear(), prev_toks_);
  DeleteElems(cur_toks_->Clear(), cur_toks_);
  cost_offsets_.clear();
  ClearActiveTokens();
  ...
  cur_toks_->Insert(start_state, start_tok);
  num_toks_++;
}
...
inline Token* LatticeFasterDecoderCombineTpl<FST, Token>::FindOrAddToken(
  ...
  if (e_found == NULL) {  // no such token presently.
    ...
    num_toks_++;
    // insert into the map
    token_map->Insert(state, new_tok);
...
void LatticeFasterDecoderCombineTpl<FST, Token>::ComputeFinalCosts(
  ...
  const Elem *final_toks = cur_toks_->GetList();
...
BaseFloat LatticeFasterDecoderCombineTpl<FST, Token>::GetCutoff(
    const Elem *list_head, size_t *tok_count, BaseFloat *adaptive_beam,
    Elem **best_elem) {
  ...
  if (config_.max_active == std::numeric_limits<int32>::max() &&
    ...
    for (const Elem *e = list_head; e != NULL; e = e->tail, count++) {
      BaseFloat w = static_cast<BaseFloat>(e->val->tot_cost);
      if (w < best_weight) {
        best_weight = w;
        if (best_elem) *best_elem = const_cast<Elem*>(e);
      }
    }
    ...
    KALDI_VLOG(5) << "Number of tokens active on frame " << NumFramesDecoded()
                  << " is " << tmp_array_.size();
...
template <typename FST, typename Token>
void LatticeFasterDecoderCombineTpl<FST, Token>::PossiblyResizeHash(
    size_t num_toks) {
...
void LatticeFasterDecoderCombineTpl<FST, Token>::ProcessForFrame(
  ...
  StateIdToTokenMap *tmp = prev_toks_;
  prev_toks_ = cur_toks_;
  cur_toks_ = tmp;
  DeleteElems(cur_toks_->Clear(), cur_toks_);

  if (prev_toks_->GetList() == NULL) {
    if (!warned_) {
  ...
  BaseFloat cur_cutoff = GetCutoff(prev_toks_->GetList(), &tok_cnt,
  ...
  for (const Elem *e = prev_toks_->GetList(); e != NULL; e = e->tail) {
    cur_queue_.push(e->key);
    e->val->in_current_queue = true;
  }
  ...
  while (!cur_queue_.empty()) {
    ...
    Token *tok = prev_toks_->Find(state)->val;
...
void LatticeFasterDecoderCombineTpl<FST, Token>::ProcessNonemitting(
  ...
  if (token_orig_cost) {  // Build the elements which are used to recover
    for (const Elem *e = cur_toks_->GetList(); e != NULL; e = e->tail) {
      (*token_orig_cost)[e->val] = e->val->tot_cost;
    }
  }
  ...
  for (const Elem *e = tmp_toks->GetList(); e != NULL; e = e->tail) {
    if (fst_->NumInputEpsilons(e->key) != 0) {
      cur_queue_.push(e->key);
      e->val->in_current_queue = true;
    }
  }
  ...
  // It will not be updated.
  BaseFloat adaptive_beam;
  BaseFloat cur_cutoff = GetCutoff(tmp_toks->GetList(), NULL, &adaptive_beam, NULL);

  while (!cur_queue_.empty()) {
    StateId state = cur_queue_.front();
    cur_queue_.pop();

    Token *tok = tmp_toks->Find(state)->val;
    BaseFloat cur_cost = tok->tot_cost;
    if (cur_cost > cur_cutoff)  // Don't bother processing successors.
      continue;
    // If "tok" has any existing forward links, delete them,
