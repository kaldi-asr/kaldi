// nnet3bin/nnet3-compute-from-sparse-input.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    po.Register("apply-exp", &apply_exp, "If true, apply exp function to "
                "output");
    po.Register("sparse-input-dim", &sparse_input_dim, "Sparse dim");
    ...
    KALDI_ASSERT(sparse_input_dim > 0);
    ...
    SequentialPosteriorReader feature_reader(feature_rspecifier);
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      SparseMatrix<BaseFloat> features(sparse_input_dim, feature_reader.Value());
