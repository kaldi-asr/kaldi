...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Propagate the features through neural network model "
        "and write the pseudo log-likelihoods (after dividing by priors).\n"
        "If --apply-exp=true, apply the Exp() function to the output "
        "before writing it out.\n"
        "\n"
        "Usage: nnet3-am-compute [options] <raw-nnet-in> <features-rspecifier> "
        "<log-likes-wspecifier>\n"
    ...
    CachingOptimizingCompiler compiler(am_nnet.GetNnet(), opts.optimize_config);
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      DecodableNnetSimple nnet_computer(
          opts, am_nnet.GetNnet(), am_nnet.Priors(),
          features, &compiler,
