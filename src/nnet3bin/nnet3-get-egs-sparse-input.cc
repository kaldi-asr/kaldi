// nnet3bin/nnet3-get-egs-sparse-input.cc
...
static void ProcessFile(const SparseMatrix<BaseFloat> &feats,
  ...
  for (int32 t = 0; t < feats.NumRows(); t += frames_per_eg) {
    ...
    SparseMatrix<BaseFloat> input_frames(tot_frames, feats.NumCols());
    ...
    for (int32 j = -left_context; j < frames_per_eg + right_context; j++) {
      ...
      input_frames.SetRow(j + left_context, feats.Row(t2));
    }
...
int main(int argc, char *argv[]) {
  try {
    ...
    int32 num_pdfs = -1, left_context = 0, right_context = 0,
        num_frames = 1, length_tolerance = 100, sparse_input_dim = -1;
    ...
    po.Register("num-pdfs", &num_pdfs, "Number of pdfs in the acoustic "
                "model");
    po.Register("sparse-input-dim", &sparse_input_dim, "Sparse input feature dimension");
    ...
    if (num_pdfs <= 0)
      KALDI_ERR << "--num-pdfs option is required.";

    if (sparse_input_dim <= 0) 
      KALDI_ERR << "--sparse-input-dim option is required.";
    ...
    SequentialPosteriorReader feat_reader(feature_rspecifier);
    ...
    for (; !feat_reader.Done(); feat_reader.Next()) {
      ...
      SparseMatrix<BaseFloat> feats(sparse_input_dim, feat_reader.Value());
