...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Generate lattices using GMM-based model, using arc lattices from forward path.\n"
        "Usage: gmm-latgen-tracking [options] model-in (fst-in|fsts-rspecifier) features-rspecifier"
...

      {
        ...
        for (; !feature_reader.Done(); feature_reader.Next()) {
          ...
          if (DecodeUtteranceLatticeTracking(
                  decoder, gmm_decodable, arc_graph, word_syms, utt,
