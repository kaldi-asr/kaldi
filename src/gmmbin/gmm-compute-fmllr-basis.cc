...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Estimate fMLLR basis representation, either per utterance or for the supplied\n"
        "set of speakers (spk2utt option). Reads posterior to accumulate fMLLR stats \n"
        "for each speaker/utterance. Outputs basis matrices.\n"
        "Usage: gmm-fmllr-base-training [options] <model-in> <feature-rspecifier>"
        "<post-rspecifier> <basis-wspecifier>\n";
    ...
    BasisFmllrGlobalParams basis_params(am_gmm.Dim());
    ...
    if (spk2utt_rspecifier != "") {  // per-speaker mode
      ...
      for (; !spk2utt_reader.Done(); spk2utt_reader.Next()) {
        ...
        basis_params.AccuGradientScatter(spk_stats);
      ...
      for (; !feature_reader.Done(); feature_reader.Next()) {
        ...
        basis_params.AccuGradientScatter(utt_stats);
      } // end looping over utterances
    }

    // Estimate the basis matrices
    int32 base_num;
    basis_params.EstimateFmllrBasis(am_gmm, &base_num);
    {
      Output ko(basis_wspecifier, binary_write);
      basis_params.Write(ko.Stream(), binary_write);
    }
    KALDI_LOG << "Done " << num_done << " files, " << num_no_post
              << " with no posts, " << num_other_error << " with other errors.";
    KALDI_LOG << "Generate " << base_num << " bases, written to" << basis_wspecifier;
