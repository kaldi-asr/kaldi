...
int main(int argc, char *argv[]) {
  try {
    ...
    using kaldi::int32;
    /*
      fsttablecomposelog should always give equivalent results to compose,
    ...
    */
    ...
    const char *usage =
        "Composition algorithm [between two FSTs of standard type, in log\n"
    ...
    po.Register("binary", &binary, "Read/write fst in binary or not.");
    ...
    if (!is_table_1 && !is_table_2) { // Only dealing with files...
      ...
      VectorFst<LogArc> *fst1_log = new VectorFst<LogArc>;
      Cast(*fst1, fst1_log); 
      VectorFst<StdArc> *fst2 = ReadFstKaldi(fst2_in_str);
      VectorFst<LogArc> *fst2_log = new VectorFst<LogArc>;
      Cast(*fst2, fst2_log);
      ...
      VectorFst<LogArc> composed_fst_log;

      TableCompose(*fst1_log, *fst2_log, &composed_fst_log, opts);
      VectorFst<StdArc> composed_fst;
      Cast(composed_fst_log, &composed_fst);
      ...
      TableComposeCache<Fst<LogArc> > cache(opts);
      VectorFst<StdArc> *fst1 = ReadFstKaldi(fst1_in_str);
      VectorFst<LogArc> *fst1_log = new VectorFst<LogArc>;
      Cast(*fst1, fst1_log);
      ...
      for (; !fst2_reader.Done(); fst2_reader.Next(), n_done++) {
        ...
        VectorFst<LogArc> fst2_log;
        Cast(fst2, &fst2_log);
        VectorFst<LogArc> fst_out_log;
        TableCompose(*fst1_log, fst2_log, &fst_out_log, &cache);
        VectorFst<StdArc> fst_out;
        Cast(fst_out_log, &fst_out);
      ...
      for (; !fst1_reader.Done(); fst1_reader.Next()) {
        ...
        if (!fst2_reader.HasKey(key)) {
          ...
          VectorFst<LogArc> fst1_log, fst2_log;
          Cast(fst1, &fst1_log);
          Cast(fst2, &fst2_log);
          VectorFst<LogArc> result_log;
          TableCompose(fst1_log, fst2_log, &result_log, opts);
          if (result_log.NumStates() == 0) {
            KALDI_WARN << "Empty output for key " << key;
            n_err++;
          } else {
            VectorFst<StdArc> result;
            Cast(result_log, &result);
