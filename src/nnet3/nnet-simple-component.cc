...
/*
  FULL_BATCH_NORM_MATH

  This comment describes the equations involved in full batch normalization
  (FullBatchNormComponent), and derives the forward and back-propagation.
  Caution: we use the apostrophe, as in x', to indicate the derivative of
  the objective function w.r.t. x.

  This is all dimension-by-dimension, so we just imagine the inputs
  are scalars x(i), for i=0 .. N-1.

  We (conceptually) do the batch normalization in two stages-- first mean, then
  variance-- to keep the math simple.  x(i) gets mean-normalized to y(i),
  then variance-normalized to z(i).

  MEAN NORMALIZATION FORWARD PASS:
                mean = 1/N sum_i x(i)
                y(i) = x(i) - mean

  MEAN NORMALIZATION BACKWARD PASS:
       We are given y'(i), which is the way we write the derivative of the
       objective function w.r.t. y(i).
       We backprop the derivatives thus:
                x'(i) = y'(i) - 1/N sum_i y'(i)
       .. this is not hard to derive so I won't go through it.

  VARIANCE NORMALIZATION FORWARD PASS:
    (note: it's the y(i) from above that are the input to this, and
    we'll write the output as z(i).

         \Sigma = \epsilon I +  1/N sum_i y(i) y(i)^T
              S = \Sigma^{-0.5}    # computed via symmetric eigenvalue decomposition
           z(i) = S y(i)

  VARIANCE NORMALIZATION BACKWARD PASS (derivation):

     This section does a rather rough derivation of the equations for the
     backward pass.  The inputs to this process are the derivatives z'(i) of the
     objective function w.r.t. z(i), as well the stuff we computed from the
     forward pass: \Sigma, S, y(i) and z(i); we'll later reformulate to only
     require z(i) and not y(i), to save memory.

     The derivative of the objective w.r.t. S, which we write as S', is:
          S' =    \sum_i z'(i) y(i)^T
     and since S is inherently symmetric we don't care about the antisymmetric
     part of the derivative, so to avoid having to keep track of its asymmetry
     later on we symmetrize it to:
         S' = 1/2 \sum_i  z'(i) y(i)^T + y(i) z'(i)^T
     We can propagate back through the equation "S = \Sigma^{-0.5}" to get the
     derivative of the objective w.r.t. \Sigma, as follows:
         \Sigma' = -1/2 \Sigma^{-0.75} S' \Sigma^{-0.75}
     Actually I don't have a proper derivation for the above, I just wrote down some
     plausible-looking formulas for backpropagation through that matrix power,
     and did numerical tests to figure out which one of them was the right one.

     The formula for S' above involves y(i) which is inconvenient.  We reformulate
     using y(i) = S^{-1} z(i) = \Sigma^{0.5} z(i).
          Defining K = 1/N \sum_i z(i) z'(i)^T, we can write:
                  S' =  N/2 (K^T \Sigma^{0.5} + \Sigma^{0.5} K)

     Substituting the expression for S' above into the expression for \Sigma'
     (the derivative of the objective function w.r.t. \Sigma), we have:
         \Sigma' = -N/4 (\Sigma^{-0.75} K^T \Sigma^{-0.25} + \Sigma^{-0.25} K \Sigma^{-0.75})

     The derivative of the objective function w.r.t. y(i) is:
            y'(i) = S z'(i) + 2/N \Sigma' y(i)
     where the first term is the direct term from differentiating z(i) = S y(i),
     and the second term is the term that comes from y(i)'s effct on \Sigma.
     (If we hadn't already symmetrized \Sigma', we'd have to write
      1/N (\Sigma' + \Sigma'^T) in place of 2/N \Sigma').
     Using y(i) = \Sigma^{0.5} z(i) (because we don't want to have to cache y(i)),
     gives us:
            y'(i) = S z'(i) + 2/N \Sigma' \Sigma^{0.5} z(i)
     which we can write as:
            y'(i) = S z'(i) + J z(i)
     where J is defined as 2/N \Sigma' \Sigma^{0.5}, which is:
         J = -1/2 (\Sigma^{-0.75} K^T \Sigma^{0.25} + \Sigma^{-0.25} K \Sigma^{-0.25})
     We can compute J without explicitly computing those 3 powers of \Sigma by making use
     of the symmetric eigenvalue decomposition \Sigma = P L P^T (which we anyway need to do
     to compute S), with \Sigma^x = P L^x P^T for all our powers x.  So:
         J = -1/2 P (L^{-0.75} P^T K^T P L^{0.25} + L^{-0.25} P^T K P L^{-0.25}) P^T
     Defining
         M = P^T K P,
     this can be written as:
         J = -1/2 P (L^{-0.75} M^T L^{0.25} + L^{-0.25} M L^{-0.25}) P^T.


  VARIANCE NORMALIZATION BACKWARD PASS (implementation):

     We are given the outputs z(i) and the objective function derivatives z'(i)
     w.r.t. the outputs, Via the memo we also remember S, and the factors
     P and L in the symmetric eigenvalue decomposition \Sigma = P L P^T.
     (L is diagonal and positive, so its powers are easy to compute).  We compute:

           K = 1/N \sum_i z(i) z'(i)^T
           M = P^T K P
           J = -1/2 P (L^{-0.75} M^T L^{0.25} + L^{-0.25} M L^{-0.25}) P^T

           y'(i) = S z'(i) + J z(i)
  */
void FullBatchNormComponent::ComputeDerived() {
  if (!test_mode_) {
    bias_.Resize(0);
    linear_term_.Resize(0, 0);
    return;
  }
  ...
  if (count_ <= 0.0) {
    ...
    count_ = 1.0;
    stats_sum_.Resize(block_dim_);
    stats_sumsq_.Resize(block_dim_, block_dim_);
    stats_sumsq_.AddToDiag(1.0);
  }

  CuVector<double> mean(stats_sum_);
  CuMatrix<double> var(stats_sumsq_);
  mean.Scale(1.0 / count_);
  var.Scale(1.0 / count_);
  var.AddVecVec(-1.0, mean, mean);
  // now var is the variance.

  // make sure it's symmetric, then copy to CPU.
  CuSpMatrix<double> var_cu_sp(var, kTakeMean);
  SpMatrix<double> var_sp(var_cu_sp);
  Vector<double> s(block_dim_);
  Matrix<double> P(block_dim_, block_dim_);
  // do the eigenvalue decomposition var_sp = P diag(s) P^T.
  var_sp.Eig(&s, &P);
  MatrixIndexT n = s.ApplyFloor(0.0);
  if (n > 0)
    KALDI_WARN << n << " out of " << block_dim_ << " dimensions floored "
        "in data covariance.";
  s.Add(epsilon_);
  s.ApplyPow(-0.5);
  // set var_sp = P diag(s) P after taking eigenvalues to power -0.5;
  // it is now the S matrix which equals \Sigma^{-0.5}.
  var_sp.AddMat2Vec(1.0, P, kNoTrans, s, 0.0);

  var_cu_sp.CopyFromSp(var_sp);
  var.CopyFromSp(var_cu_sp);
  // now var is S, which is \Sigma^{-0.5}

  CuVector<double> bias(block_dim_);
  // bias = - S * mean.
  bias.AddMatVec(-1.0, var, kNoTrans, mean, 0.0);

  bias_.Resize(block_dim_, kUndefined);
  bias_.CopyFromVec(bias);
  linear_term_.Resize(block_dim_, block_dim_, kUndefined);
  linear_term_.CopyFromMat(var);
}

void FullBatchNormComponent::SetTestMode(bool test_mode) {
  test_mode_ = test_mode;
  ComputeDerived();
}

void FullBatchNormComponent::Check() const {
  KALDI_ASSERT(dim_ > 0 && block_dim_ > 0 && dim_ % block_dim_ == 0 &&
               epsilon_ > 0.0);
}

FullBatchNormComponent::FullBatchNormComponent(const FullBatchNormComponent &other):
    dim_(other.dim_), block_dim_(other.block_dim_), epsilon_(other.epsilon_),
    test_mode_(other.test_mode_), count_(other.count_), stats_sum_(other.stats_sum_),
    stats_sumsq_(other.stats_sumsq_) {
  ComputeDerived();
  Check();
}
...
std::string FullBatchNormComponent::Info() const {
  std::ostringstream stream;
  stream << Type() << ", dim=" << dim_ << ", block-dim=" << block_dim_
         << ", epsilon=" << epsilon_
         << ", count=" << count_
         << ", test-mode=" << (test_mode_ ? "true" : "false");
  if (count_ > 0) {
    Vector<double> mean(stats_sum_);
    Matrix<double> var_full(stats_sumsq_);
    SpMatrix<double> var(var_full);
    mean.Scale(1.0 / count_);
    var.Scale(1.0 / count_);
    // subtract mean^2 from var.
    var.AddVec2(-1.0, mean);
    Vector<double> eigs(block_dim_);
    var.Eig(&eigs);
    Vector<BaseFloat> mean_float(mean), eigs_float(eigs);
    stream << ", data-mean=" << SummarizeVector(mean_float)
           << ", var-eigs=" << SummarizeVector(eigs_float);
  }
  return stream.str();
}

void FullBatchNormComponent::InitFromConfig(ConfigLine *cfl) {
  dim_ = -1;
  block_dim_ = -1;
  epsilon_ = 1.0e-03;
  test_mode_ = false;
  ...
  cfl->GetValue("test-mode", &test_mode_);
  if (!ok || dim_ <= 0) {
    KALDI_ERR << "FullBatchNormComponent must have 'dim' specified, and > 0";
  }
  if (block_dim_ == -1)
    block_dim_ = dim_;
  if (!(block_dim_ > 0 && dim_ % block_dim_ == 0 &&
        epsilon_ > 0))
    KALDI_ERR << "Invalid configuration in FullBatchNormComponent";
  if (cfl->HasUnusedValues())
    KALDI_ERR << "Could not process these elements in initializer: "
              << cfl->UnusedValues();
  count_ = 0;
...
void* FullBatchNormComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
                                    const CuMatrixBase<BaseFloat> &in,
                                    CuMatrixBase<BaseFloat> *out) const {
  KALDI_ASSERT(SameDim(in, *out) &&
               (in.NumCols() == dim_ || in.NumCols() == block_dim_));
  ...
  if (!test_mode_) {
    ...
    memo->mean.Resize(dim);
    memo->uncentered_var.Resize(dim, dim);
    memo->P.Resize(dim, dim, kUndefined);
    memo->eigs_powers.Resize(3, dim, kUndefined);
    memo->S.Resize(dim, dim);

    memo->mean.AddRowSumMat(1.0 / num_frames, in, 0.0);
    memo->uncentered_var.SymAddMat2(1.0 / num_frames, in, kTrans, 0.0);
    memo->uncentered_var.CopyLowerToUpper();

    // We'll use memo->S as a temporary for the centered variance \Sigma.
    memo->S.CopyFromMat(memo->uncentered_var);
    memo->S.AddVecVec(-1.0, memo->mean, memo->mean);

    // sigma_cuda will be the CUDA version of the centered variance.
    CuSpMatrix<BaseFloat> sigma_cuda(memo->uncentered_var, kTakeLower);
    sigma_cuda.AddVec2(-1.0, memo->mean);

    // Copy the variance to the CPU so that we can do an eigenvalue
    // decomposition (this is hard to implement fast on the GPU).
    SpMatrix<BaseFloat> sigma(sigma_cuda);
    // For now we're doing the eigenvalue decomposition in double, out of
    // concern about how roundoff affects it; later we can change this back if
    // it's not really an issue.
    SpMatrix<double> sigma_dbl(sigma);
    Matrix<double> P_dbl(dim, dim);
    Vector<double> eigs_dbl(dim);
    sigma_dbl.Eig(&eigs_dbl, &P_dbl);
    // mathematically the flooring operation below is a no-op since sigma is
    // positive semidefinite; it is only necessary because of roundoff errors.
    eigs_dbl.ApplyFloor(0.0);
    eigs_dbl.Add(epsilon_);
    Matrix<BaseFloat> P(P_dbl);
    Vector<BaseFloat> eigs(eigs_dbl);
    memo->P.CopyFromMat(P, kNoTrans);
    CuSubVector<BaseFloat> eigs_powers_0(memo->eigs_powers, 0),
        eigs_powers_1(memo->eigs_powers, 1),
        eigs_powers_2(memo->eigs_powers, 2);
    eigs_powers_0.CopyFromVec(eigs);
    memo->eigs_powers.RowRange(1, 2).CopyRowsFromVec(eigs_powers_0);
    eigs_powers_0.ApplyPow(-0.75);
    eigs_powers_1.ApplyPow(-0.25);
    eigs_powers_2.ApplyPow(0.25);

    CuMatrix<BaseFloat> P_scaled(memo->P);
    P_scaled.MulColsVec(eigs_powers_1);
    // Now P_scaled equals P * L^{-0.25}, where L is the diagonal matrix of
    // eigenvalues of \Sigma.
    // Next we set memo->S to P * L^{-0.25} * L^{-0.25} * P^T,
    // which is \Sigma^{-0.5}.
    memo->S.SymAddMat2(1.0, P_scaled, kNoTrans, 0.0);
    memo->S.CopyLowerToUpper();

    CuVector<BaseFloat> bias(block_dim_);
    bias.AddMatVec(-1.0, memo->S, kNoTrans, memo->mean, 0.0);

    out->CopyRowsFromVec(bias);
    // In the next statement we could use kNoTrans or kTrans, S is symmetric.
    out->AddMatMat(1.0, in, kNoTrans, memo->S, kTrans, 1.0);
    // { // TEMP- check output has less-than-unit uncentered covariance.
    //   CuMatrix<BaseFloat> sigma_out_cuda(block_dim_, block_dim_);
    //   sigma_out_cuda.SymAddMat2(1.0 / out->NumRows(), *out, kTrans, 0.0);
    //   CuSpMatrix<BaseFloat> sigma_out_cuda_sp(sigma_out_cuda, kTakeLower);
    //   SpMatrix<BaseFloat> sigma_out(sigma_out_cuda_sp);
    //   Vector<BaseFloat> eigs_out(block_dim_);
    //   sigma_out.Eig(&eigs_out);
    //   KALDI_LOG << "Eigs of output are in the range " << eigs_out.Min() << " to " << eigs_out.Max();
    // }
    return static_cast<void*>(memo);
  } else {
    if (bias_.Dim() != block_dim_) {
      if (count_ == 0)
        KALDI_ERR << "Test mode set in FullBatchNormComponent, but no stats.";
      else  // why was ComputeDerived() not called?
        KALDI_ERR << "Code error in FullBatchNormComponent";
    }
    out->CopyRowsFromVec(bias_);
    // In the next statement we could use kNoTrans or kTrans, linear_term_ is
    // symmetric.
    out->AddMatMat(1.0, in, kNoTrans, linear_term_, kTrans, 1.0);
    return NULL;
  }
}


// Search above for VARIANCE NORMALIZATION BACKWARD PASS for an explanation of
// the math.
void FullBatchNormComponent::ComputeJ(const Memo &memo,
                                      const CuMatrixBase<BaseFloat> &out_value,
                                      const CuMatrixBase<BaseFloat> &out_deriv,
                                      CuMatrix<BaseFloat> *J) const {
  int32 dim = block_dim_,
      num_rows = out_value.NumRows();
  CuMatrix<BaseFloat> K(block_dim_, block_dim_);
  // Set: K = 1/N \sum_i z(i) z'(i)^T
  K.AddMatMat(1.0 / num_rows, out_value, kTrans, out_deriv, kNoTrans, 0.0);
  CuMatrix<BaseFloat> Mpart(dim, dim),
      M(dim, dim);
  // Mpart = K P.
  Mpart.AddMatMat(1.0, K, kNoTrans, memo.P, kNoTrans, 0.0);
  // M = P^T K P
  M.AddMatMat(1.0, memo.P, kTrans, Mpart, kNoTrans, 0.0);

  // eigs_powers_{0,1,2} are respectively powers -0.75, -0.25, and 0.25
  // of the eigenvalues-- in the math these correspond to
  // L^{-0.75}, L^{-0.25} and L^{0.25} respectively.
  CuSubVector<BaseFloat> eigs_powers_0(memo.eigs_powers, 0),
      eigs_powers_1(memo.eigs_powers, 1),
      eigs_powers_2(memo.eigs_powers, 2);

  // Set Mpart to be the transpose of M.
  Mpart.CopyFromMat(M, kTrans);
  // Set M to what, in the math, is L^{-0.25} M L^{-0.25}.
  M.MulRowsVec(eigs_powers_1);
  M.MulColsVec(eigs_powers_1);
  // Set Mpart to L^{-0.75} M^T M L^{0.25}.
  Mpart.MulRowsVec(eigs_powers_0);
  Mpart.MulColsVec(eigs_powers_2);
  // The next statement sets M to
  // (L^{-0.75} M^T L^{0.25} + L^{-0.25} M L^{-0.25})
  M.AddMat(1.0, Mpart);
  // The next statement sets Mpart to
  // P (L^{-0.75} M^T L^{0.25} + L^{-0.25} M L^{-0.25})
  Mpart.AddMatMat(1.0, memo.P, kNoTrans, M, kNoTrans, 0.0);
  J->Resize(dim, dim);
  // The next statement sets
  // J = -1/2 P (L^{-0.75} M^T L^{0.25} + L^{-0.25} M L^{-0.25}) P^T
  J->AddMatMat(-0.5, Mpart, kNoTrans, memo.P, kTrans, 0.0);
}

void FullBatchNormComponent::Backprop(
    const std::string &debug_info,
    const ComponentPrecomputedIndexes *indexes,
    const CuMatrixBase<BaseFloat> &in_value,  // unused
    const CuMatrixBase<BaseFloat> &out_value,
  ...
  if (!test_mode_) {
    // Search above for VARIANCE NORMALIZATION BACKWARD PASS (implementation)
    // and MEAN NORMALIZATION BACKWARD PASS for a description of what is being
    // implemented here.
    CuMatrix<BaseFloat> J;
    ComputeJ(*memo, out_value, out_deriv, &J);



    // The following three statements compute:
    //  y'(i) = S z'(i) + J z(i)
    // Note: y' occupies in_deriv (although later we'll modify in_deriv
    // by subtracting the mean, to be x'); z' is out_deriv; z is out_value.
    in_deriv->SetZero();
    // It doesn't matter whether memo->S is transposed, it's symmetric.
    in_deriv->AddMatMat(1.0, out_deriv, kNoTrans, memo->S, kTrans, 0.0);
    in_deriv->AddMatMat(1.0, out_value, kNoTrans, J, kTrans, 1.0);

    // OK, the next few statements correspond to
    // MEAN NORMALIZATION BACKWARD PASS
    // in the math: x'(i) = y'(i) - 1/N sum_i y'(i)
    CuVector<BaseFloat> avg_y_deriv(block_dim_);
    avg_y_deriv.AddRowSumMat(1.0 / in_deriv->NumRows(), *in_deriv, 0.0);
    in_deriv->AddVecToRows(-1.0, avg_y_deriv);
  } else {
    KALDI_ASSERT(bias_.Dim() == block_dim_);
    in_deriv->SetZero();
    // Below it doesn't matter whether linear_term_ is transposed; it's
    // symmetric.
    in_deriv->AddMatMat(1.0, out_deriv, kNoTrans, linear_term_, kTrans, 0.0);
  }
}

void FullBatchNormComponent::StoreStats(
    const CuMatrixBase<BaseFloat> &in_value,
    const CuMatrixBase<BaseFloat> &out_value,
    void *memo_in) {
  // in test mode this component does not store stats, it doesn't provide the
  ...
  Memo *memo = static_cast<Memo*>(memo_in);
  BaseFloat num_frames = memo->num_frames;
  KALDI_ASSERT(memo->mean.Dim() == block_dim_ &&
               out_value.NumRows() == num_frames);
  ...
  count_ += num_frames;
  stats_sum_.AddVec(num_frames, memo->mean, 1.0);
  // We could optimize this a bit by making a version of AddMat that does type
  // conversion.
  CuMatrix<double> uncentered_var_double(memo->uncentered_var);
  stats_sumsq_.AddMat(num_frames, uncentered_var_double);
}

void FullBatchNormComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<FullBatchNormComponent>", "<Dim>");
  ReadBasicType(is, binary, &dim_);
  ExpectToken(is, binary, "<BlockDim>");
  ReadBasicType(is, binary, &block_dim_);
  ExpectToken(is, binary, "<Epsilon>");
  ...
  ExpectToken(is, binary, "<StatsVar>");
  stats_sumsq_.Read(is, binary);
  stats_sumsq_.AddVecVec(1.0, stats_sum_, stats_sum_);
  stats_sum_.Scale(count_);
  stats_sumsq_.Scale(count_);
  ExpectToken(is, binary, "</FullBatchNormComponent>");
  ComputeDerived();
  Check();
}

void FullBatchNormComponent::Write(std::ostream &os, bool binary) const {
  Check();
  WriteToken(os, binary, "<FullBatchNormComponent>");
  WriteToken(os, binary, "<Dim>");
  WriteBasicType(os, binary, dim_);
  WriteToken(os, binary, "<BlockDim>");
  WriteBasicType(os, binary, block_dim_);
  ...
  CuMatrix<BaseFloat> var(stats_sumsq_);
  if (count_ != 0) {
    mean.Scale(1.0 / count_);
    var.Scale(1.0 / count_);
    var.AddVecVec(-1.0, mean, mean);
...
void FullBatchNormComponent::Scale(BaseFloat scale) {
  if (scale == 0) {
    count_ = 0.0;
    stats_sum_.SetZero();
    stats_sumsq_.SetZero();
...
void FullBatchNormComponent::Add(BaseFloat alpha, const Component &other_in) {
  const FullBatchNormComponent *other =
      dynamic_cast<const FullBatchNormComponent*>(&other_in);
  count_ += alpha * other->count_;
  stats_sum_.AddVec(alpha, other->stats_sum_);
...
void FullBatchNormComponent::ZeroStats() {
  // We only zero the stats if we're not in test mode.  In test mode, this would
  // be dangerous as the stats are the source for the transform, and zeroing
  // them and then calling ComputeDerived() again would remove the transform
  // parameters (offset_ and scale_).
