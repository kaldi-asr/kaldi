...
void RepeatedAffineComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
  ...
  // of *out repeatly.
	for (int i = 0; i < num_repeats_; i++) {
		out->ColRange(i*block_rows, block_rows).CopyRowsFromVec(bias_params_);
	}
	std::vector<CuSubMatrix<BaseFloat> *> in_batch, out_batch, linear_params_batch;
	CuSubMatrix<BaseFloat>* params_elem = new CuSubMatrix<BaseFloat>(linear_params_.ColRange(0, block_cols));

	for (int i = 0; i < num_repeats_; i++) {
		in_batch.push_back(new CuSubMatrix<BaseFloat>(in.ColRange(i * block_cols, block_cols)));
		out_batch.push_back(new CuSubMatrix<BaseFloat>(out->ColRange(i * block_rows, block_rows)));
...
void RepeatedAffineComponent::Backprop(const std::string &debug_info,
  ...
  if (in_deriv) {
	...
	for (int i = 0; i < num_repeats_; i++) {
		in_deriv_batch.push_back(new CuSubMatrix<BaseFloat>(in_deriv->ColRange(i * block_cols, block_cols)));
		out_deriv_batch.push_back(new CuSubMatrix<BaseFloat>(out_deriv.ColRange(i * block_rows, block_rows)));
		linear_params_batch.push_back(params_elem);	
	}
  ...
  if (to_update != NULL) {
	...
	CuSubMatrix<BaseFloat>* params_deriv_elem = new CuSubMatrix<BaseFloat>(linear_params_.ColRange(0, block_cols));
	
	for (int i = 0; i < num_repeats_; i++) {
		in_value_batch.push_back(new CuSubMatrix<BaseFloat>(in_value.ColRange(i * block_cols, block_cols)));
		out_deriv_batch.push_back(new CuSubMatrix<BaseFloat>(out_deriv.ColRange(i * block_rows, block_rows)));
