...
BaseFloat FilteredSpatialSumsq(const CuMatrix<BaseFloat> &output_data) {
  ...
  int32 num_rows = (data.NumRows() + 15) / 16,
      num_cols = 16;
  double ans = 0.0;
    ...
    for (int32 j = 0; j < num_rows; j++) {
      for (int32 k = 0; k < num_cols; k++) {
        ...
        highpass_image(j, k) = image(j, k) - 0.125 * (
            image(jm1, km1) + image(jm1, 0) + image(jm1, kp1) +
            image(0, km1) + image(0, kp1) +
            image(jp1, km1) + image(jp1, 0) + image(jm1, kp1));
      }
    }
