...
class Component {
 public:
  ...
  /// Backprop function.  "indexes" will be NULL for simple compoonents in_value
  /// and out_value are the input and output of Propagate may be the empty
  /// matrix, and they may be the empty matrix if respectively
  /// Properties()&kBackpropNeedsInput and/or Properties()&kBackpropNeedsOutput
  /// are true.  Model update will be done only if "to_update" is non-NULL, and
  /// will be done to the component pointed to by "to_update", which does not
  /// have to be equal to "this".  If simple_deriv is true, it just does a
  /// gradient computation rather than the normal model update.  If
  /// Properties()&kBackpropInPlace
  virtual void Backprop(const ComponentPrecomputedIndexes *indexes,
                        ...
                        bool simple_deriv, // true if we just want derivative,
  ...
  /// are required at its input.  The default implementation is suitable for any
  /// SimpleComponent (it just copies the output_index to a single identical
  /// input_index).  The misc_info is supplied to handle things that the
  /// framework can't very easily supply, e.g. which time indexes are needed for
  /// AggregateComponent, which time-indexes are available at the input of a
  /// recurrent network, and so on.  (this function maps output-indexes to
  /// required input-indexes, but there are occasions where we'd like to know
  /// what input-indexes are available, and the misc_info lets us do that in
  /// a rather ad-hoc way).
  virtual void GetInputIndexes(const MiscComputationInfo &misc_info,
                               const Index &output_index,
                               std::vector<Index> *input_indexes) const;

  /// A non-simple Component may define its own child-class of
  /// ComponentPrecomputedIndexes, and it can use this to store any indexes or
  /// similar index-dependent information it needs to do the forward and
  /// backward computations.  The default implementation is suitable for
  /// simple Components.
  virtual ComponentPrecomputedIndexes *PrecomputeIndexes(
      const MiscComputationInfo &misc_info,
      const std::vector<FrameIndex> &input_indexes,
      std::vector<FrameIndex> &output_indexes,
      bool need_backprop) const { return NULL;  }

  // each type should return a string such as "SigmoidComponent".
  ...
  /// Return bitmask of the component's properties (generally these depend only on
  /// the component type).  See enum ComponentProperties.
  ...
  // This Read function requires that the Component has the correct type.
...
// SimpleComponent is for things like affine transforms and nonlinearities that
// don't have a direct interaction with things like frame splicing, time and
// space indexes, and so on.  GeneralComponent is the "other" type, that does
// deal with that type of stuff.
class SimpleComponent: public Component {
 ...
 private:
  KALDI_DISALLOW_COPY_AND_ASSIGN(SimpleComponent);
};
...
/// GeneralComponent is for things like frame-splicing and pooling, that may
/// involve temporal and spatial context.
class GeneralComponent: public Component {
  
  int32 NumInputs()
  int32 InputDim(n);  // n = input(n).

  // get frame-indexes of the input component with index 0 <= i < NumInputs(),
  // which we will require to compute this frame of output.
  GetInputIndexes(int32 i,
                  const FrameIndex &output_index,
                  std::vector<FrameIndex> *input_indexes);

  // way to store indexes for a given computation?
  
};
  
/*
  AlternateComponent, chooses different inputs on different frames.

  
  Component...
   Input=10[t-5],10[t+5]

   Input=10[4*(t/4)],10[4*((t/4)+1)]
   ...
   Or... have different versions of final layer with different names, accessed
   as output1, output2, output3, output4?  Labels would be specified as:
     t=10: output1=1420
     t=11: output2=1420
     t=12: output3=1382
