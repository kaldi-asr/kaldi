...
DecodableAmNnetMulti::DecodableAmNnetMulti(
    ...
    mapping_(mapping),
    am_nnet_(am_nnet),
    // priors_vec_(am_nnet_.Priors_vec()),
    feats_(feats),
  ...
  for (int i = 0; i < num_outputs_; i++) {
    priors_vec_.push_back(am_nnet_.Priors(i));
  }
  ...
  for (int i = 0; i < num_outputs_; i++) {
    priors_vec_[i].ApplyLog();
  }
  current_log_post_vec_.resize(num_outputs_);
...
BaseFloat DecodableAmNnetMulti::LogLikelihood(int32 frame,
  ...
  if (frame < current_log_post_offset_ ||
      frame >= current_log_post_offset_ + current_log_post_vec_[0].NumRows())
    EnsureFrameIsComputed(frame);
  int32 pdf_id = trans_model_.TransitionIdToPdf(transition_id);

  const std::vector<int32> &pdf_ids = mapping_[pdf_id];

  std::vector<BaseFloat> scores;
  for (int i = 0; i < num_outputs_; i++) {
    scores.push_back(current_log_post_vec_[i](frame - current_log_post_offset_,
                                              pdf_ids[i]));
  }
...
int32 DecodableAmNnetMulti::GetIvectorDim() const {
  if (ivector_ != NULL)
    return ivector_->Dim();
...
void DecodableAmNnetMulti::EnsureFrameIsComputed(int32 frame) {
  ...
  int32 current_frames_computed = current_log_post_vec_[0].NumRows(), // TODO(hxu) not sure
...
void DecodableAmNnetMulti::GetCurrentIvector(int32 output_t_start,
...
void DecodableAmNnetMulti::DoNnetComputation(
    int32 input_t_start,
  ...
  for (int i = 0; i < num_outputs_; i++) {
    ...
    request.outputs.push_back(
      IoSpecification("output" + os.str(), time_offset + output_t_start,
  ...
  for (int i = 0; i < num_outputs_; i++) {
    ...
    cu_output.AddVecToRows(-1.0, priors_vec_[i]);
    // apply the acoustic scale
    cu_output.Scale(opts_.acoustic_scale);
    current_log_post_vec_[i].Resize(0, 0);
    // the following statement just swaps the pointers if we're not using a GPU.
    cu_output.Swap(&current_log_post_vec_[i]);
    current_log_post_offset_ = output_t_start;
  }
}

void DecodableAmNnetMulti::PossiblyWarnForFramesPerChunk() const {
