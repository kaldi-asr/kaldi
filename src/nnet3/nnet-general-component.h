...
/*
  This component implements convolution in time.  It is primarily intended for
  setups in which the network directly reads the waveform, but the code is quite
  general.  It's a general component, so it interprets the 't' indexes of
  its input and output.


  This paragraph describes the 'normal case' when feature_per_frame_ == 1.
  Suppose the input consists of vectors x_t, of dimension D.  Suppose you have
  filters f_n for 0 <= n < N, which are interpreted as vectors whose dimension
  dimension is some multiple of t.  The output y_{tn} will be the dot product of
  f_n with a suitably spliced set of x_t vectors, depending on left_context and
  right_context.  E.g. if left_context == 2 and right_context == 2, then the
  output will be
     y_{tn} = f_n . (x_{t-2} x_{t-1} x_t x_{t+1} x_{t+2}).
  [note that in the code these are called subsampled_left_context_ and
  subsampled_right_context_, for reasons that relate to the general case we
  discuss below].

  Now we consider the general case when features_per_frame_ > 1.  In this
  case, the input is treated as having a higher time resolution than the 't'
  index captures, so the input dimension (input_dim_) must be a multiple of
  features_per_frame_.  Let shift_dim = input_dim_ / features_per_frame_.
  Then we treat the input x_t as a concatenation of vectors
     x_t = (z_{t0} z_{t1} ... ),
  where the z_{ti} vectors are each of dimension shift_dim, and these are
  considered as forming a sequence over successive values of t, so it's exactly
  as if the 'real' frame rate were higher by a factor of 'features_per_frame_'.
  In this general case, the output dimension is higher by a factor
  of 'features_per_frame_', because we now output values y_{tni} for
  each n and each 0 <= i < features_per_frame_.  For a given t (i.e. for
  a given matrix row), the 'i' index of the output has the larger stride
  (the stride for 'n' is 1) -- this means that in principle you can chain
  this type of component.

 */
class TimeConvolutionComponent: public Component {


  virtual int32 Properties() const {
    return kReordersIndexes|kPropagateAdds|kInputContiguous|kOutputContiguous;
  }
 ...
 private:
  ...
  int32 SubsampledInputDim() { return input_dim_ / features_per_frame_; }

  // The matrix filters_ contains the filters.  Mathematically we view each
  // filter as a vector, but in this matrix the filters are broken up into
  // separate pieces, one per shift; and these are not stored as consecutive
  // rows of filters_, but are interleaved.  The dimension of filters_ is
  // as follows:
  //   filters_.NumRows() ==  num_filters * (subsampled_left_context_ + 1 +
  //                                         subsampled_right_context_)
  // where num_filters is defined by the above equation, and
  //   filters_.NumCols() == (input_dim_ / features_per_frame_)
  //
  // the output dimension (OutputDim()) of this component equals
  // num_filters * features_per_frame_.

  CuMatrix<BaseFloat> filters_;

  int32 input_dim_;
  int32 features_per_frame_;

  int32 subsampled_right_context_;
  int32 subsampled_left_context_;

  bool natural_gradient_;
