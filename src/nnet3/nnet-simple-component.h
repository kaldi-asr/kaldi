...
/*
  FullBatchNormComponent implements a 'full-covariance' form of
  batch normalization: i.e. its output is its input modified to
  be zero mean and unit covariance (modulo the epsilon flooring).
  This is achieved via the symmetric matrix square root, which is
  differentiable.

  For a more detailed explanation of the math, search for the
  string FULL_BATCH_NORM_MATH in nnet-simple-component.cc

  This is a simple component (uses the kSimpleComponent flag), but it is unusual in
  that it will give different results if you call it on half the matrix at a
  time.
    ...
    Accepted configuration values:
           ...
           epsilon      Small term added to the variance that is used to prevent
                        division by zero
 */
class FullBatchNormComponent: public Component {
 public:

  FullBatchNormComponent() { }
  ...
  // constructor using another component
  FullBatchNormComponent(const FullBatchNormComponent &other);
  ...
  virtual std::string Info() const;
  virtual void InitFromConfig(ConfigLine *cfl);
  virtual std::string Type() const { return "FullBatchNormComponent"; }
  virtual int32 Properties() const {
    // If the block-dim is less than the dim, we need the input and output
    // matrices to be contiguous (stride==num-cols), as we'll be reshaping
    // internally.  This is not much of a cost, because this will be used
  ...
  /// Write component to stream
  virtual void Write(std::ostream &os, bool binary) const;
  virtual Component* Copy() const { return new FullBatchNormComponent(*this); }
  ...
  // Members specific to this component type.  Note: the bias and linear term
  // will only be nonempty in 'test mode'; if test_mode_ is false, the bias and
  // offset are computed per minibatch.
  const CuVector<BaseFloat> &Bias() const { return bias_; }
  const CuMatrix<BaseFloat> &LinearTerm() const { return linear_term_; }

 private:

  struct Memo {
    // Note: if memory was a paramount concern we might not keep all of the
    // items here: 'uncentered_var' and 'S' could be reconstructed from
    // other members.
    int32 num_frames;
    // let 'dim' below refer to block_dim_.
    CuVector<BaseFloat> mean;  // data mean, dimension 'dim'
    CuMatrix<BaseFloat> uncentered_var;   // Uncentered data covariance,
                                          // dimension 'dim' by 'dim'.
    CuMatrix<BaseFloat> P;   // Matrix whose rows are the eigenvectors of the
                             // centered data covariance.

    // eigs_powers is of dimension 3 by dim.
    // Different rows of eigs_powers contain different powers of the eigenvalues
    // of the smoothed variance.  We do the decomposition:
    //    (var + epsilon I) = P diag(eigs) P^T
    //  Row 0 contains eigs^{-0.75}.
    //  Row 1 contains eigs^{-0.25}.
    //  Row 2 contains eigs^{0.25}.
    CuMatrix<BaseFloat> eigs_powers;
    CuMatrix<BaseFloat> S; // S equals var^{-0.5}
  };

  void Check() const;

  // computes derived parameters bias_ and linear_term_
  void ComputeDerived();

  // This function, called from Backprop(), computes a matrix that is needed in
  // the backpropagation.  Search in nnet-simple-component.cc for VARIANCE
  // NORMALIZATION BACKWARD PASS for an explanation of the math.
  void ComputeJ(const Memo &memo,
                const CuMatrixBase<BaseFloat> &out_value,
                const CuMatrixBase<BaseFloat> &out_deriv,
                CuMatrix<BaseFloat> *J) const;

  // Dimension of the input and output of this component.
  int32 dim_;
  // This would normally be the same as dim_, but if it's less (it may be any
  // nonzero divisor of dim_), then each separate block of the input of dimension
  // 'block_dim_' is treated like a separate frame for the purposes of
  // normalization.  This can be used to implement spatial batch normalization
  // for convolutional setups-- assuming the filter-dim has stride 1, which it
  // always will in the new code in nnet-convolutional-component.h.
  ...
  // Used to avoid exact-zero variances, we add epsilon times the identity
  ...
  // total count of stats stored by StoreStats().
  double count_;
  // sum-of-data component of stats of input data (will be required, with
  // 'stats_sumsq_', for test mode computation)
  CuVector<double> stats_sum_;
  // sum-of-squared-data component of stats of input data.
  CuMatrix<double> stats_sumsq_;

  // bais_ and linear_term_ are derived from stats_sum_ and stats_sumsq_; they
  // dictate the transform that is done in 'test mode'.  They are set only when
  // reading the model from disk and when calling SetTestMode(true); they are
  // resized to empty when the stats are updated, to ensure that out-of-date
  // values are not kept around.
  CuVector<BaseFloat> bias_;
  CuMatrix<BaseFloat> linear_term_;
};
