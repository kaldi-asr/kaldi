...
/**
   ...
   intended for use in x-vector computation.  It computes x-vectors on fixed-size
   chunks of the input and averages them to get the x-vector for the entire input
   utterance; this not quite the same as how it's canonically supposed to be done
   (it's supposed to be done inside the network), but I don't think it will make
   much difference as long as the chunks are at least a few seconds each.
*/
