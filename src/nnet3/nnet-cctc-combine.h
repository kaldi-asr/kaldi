...
#ifndef KALDI_NNET3_NNET_CCTC_COMBINE_H_
#define KALDI_NNET3_NNET_CCTC_COMBINE_H_
...
class NnetCctcCombiner {
 public:
  ...
  ~NnetCctcCombiner() { delete prob_computer_; }
 private:
  const NnetCombineConfig &config_;

  const std::vector<NnetCctcExample> &egs_;

  const ctc::CctcTransitionModel &trans_mdl_;

  Nnet nnet_;  // The current neural network.

  NnetCctcComputeProb *prob_computer_;
