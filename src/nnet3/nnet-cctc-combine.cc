...
NnetCctcCombiner::NnetCctcCombiner(const NnetCombineConfig &config,
                                   int32 num_nnets,
                                   const std::vector<NnetCctcExample> &egs,
                                   const ctc::CctcTransitionModel &trans_mdl,
                                   const Nnet &first_nnet):
    config_(config),
    egs_(egs),
    trans_mdl_(trans_mdl),
    nnet_(first_nnet),
  ...
  NnetCctcComputeProbOptions compute_prob_opts;
  compute_prob_opts.compute_deriv = true;
  prob_computer_ = new NnetCctcComputeProb(compute_prob_opts, trans_mdl_, nnet_);

}

void NnetCctcCombiner::ComputeUpdatableComponentDims(){
...
void NnetCctcCombiner::AcceptNnet(const Nnet &nnet) {
...
void NnetCctcCombiner::FinishPreprocessingInput() {
  KALDI_ASSERT(num_nnets_provided_ == num_real_input_nnets_ &&
...
void NnetCctcCombiner::PrintParams(const VectorBase<BaseFloat> &params) const {
...
bool NnetCctcCombiner::SelfTestDerivatives() {
...
void NnetCctcCombiner::SelfTestModelDerivatives() {
...
int32 NnetCctcCombiner::ParameterDim() const {
  if (config_.separate_weights_per_component)
...
void NnetCctcCombiner::GetInitialParameters(VectorBase<BaseFloat> *params) const {
...
void NnetCctcCombiner::GetWeights(const VectorBase<BaseFloat> &params,
...
void NnetCctcCombiner::GetParamsDeriv(const VectorBase<BaseFloat> &weights,
...
void NnetCctcCombiner::GetNnetParameters(const Vector<BaseFloat> &weights,
...
void NnetCctcCombiner::GetWeightsDeriv(
...
double NnetCctcCombiner::ComputeObjfAndDerivFromNnet(
  ...
  std::vector<NnetCctcExample>::const_iterator iter = egs_.begin(),
                                                end = egs_.end();
  for (; iter != end; ++iter)
...
double NnetCctcCombiner::ComputeObjfAndDerivFromParameters(
...
void NnetCctcCombiner::GetNormalizedWeights(
...
void NnetCctcCombiner::GetUnnormalizedWeightsDeriv(
    const VectorBase<BaseFloat> &unnorm_weights,
