...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet3-chain-train [options] <model-in> <chain-training-examples-in> (<model-out>|<raw-model-out>)\n"
        "\n"
        "nnet3-chain-train 1.mdl 'ark:nnet3-merge-egs 1.cegs ark:-|' 2.raw\n";

    bool binary_write = true;
    bool write_raw = false;
    std::string use_gpu = "yes";
    NnetChainTrainerOptions train_config;
    ...
    {
      bool binary;
      Input input(chain_nnet_rxfilename, &binary);
      trans_model.Read(input.Stream(), binary);
      nnet.Read(input.Stream(), binary);
    }

    NnetChainTrainer trainer(train_config, trans_model, &nnet);
