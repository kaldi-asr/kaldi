  ...
  try {
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      for (int i = nnet_durmodel.LeftContext();
        ...
        for (int a_left_ctx = left_context; a_left_ctx >= 0; a_left_ctx--) {
          std::vector<int32> phone_context(a_left_ctx + right_context + 1);
          for (int j = i - a_left_ctx; j <= i + right_context; j++) {
            phone_context[j - (i - a_left_ctx)] = pairs[j].first;
          }
          context_to_count[phone_context]++;
          context_to_logprob[phone_context] += logprob;
        }
    ...
    BaseFloat overall_avg = 0.0;
    int32 total_count = 0;
    for (unordered_map<std::vector<int32>, BaseFloat, VectorHasher<int32> >::iterator
          it = context_to_logprob.begin(); it != context_to_logprob.end(); ++it) {
      if (it->first.size() == 2) {
        overall_avg += it->second;
        total_count += context_to_count[it->first];
      }
      it->second /= context_to_count[it->first];
    }
    overall_avg /= total_count;
    AvgPhoneLogProbs avg_logprobs(left_context, right_context, context_to_logprob);
    WriteKaldiObject(avg_logprobs, out_filename, binary_write);
    KALDI_LOG << "Computed logprobs for " << num_examples_processed
              << " phones. Average logprob is "
              << overall_avg << " over all processed examples.";
    return 0;
