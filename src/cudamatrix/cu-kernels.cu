...
template<typename Real>
__global__
static void _calc_max_deriv(Real *deriv, const Real *vec, const Real *maxv,
...
void cudaF_calc_max_deriv(dim3 Gr, dim3 Bl, float *y, const float *x1, 
			    const float *x2, MatrixDim d, int src_stride,
			    int group_size) {
  _calc_max_deriv<<<Gr,Bl>>>(y, x1, x2, d, src_stride, group_size);
}
...
void cudaD_calc_max_deriv(dim3 Gr, dim3 Bl, double*y, const double* x1, 
			    const double* x2, MatrixDim d, int src_stride, 
			    int group_size) {
  _calc_max_deriv<<<Gr,Bl>>>(y, x1, x2, d, src_stride, group_size);
}
