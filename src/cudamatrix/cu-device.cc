...
// We define class CuAllocator inside the .cc file, because we don't want
// to expose it in the header.
class CuAllocator {
 public:
 ...
 private:
  struct MemInfoForSize {
    size_t size; // size of memory that was allocated here.
    size_t countdown; // after countdown (of num freed) reaches zero we will
                      // start to cache things before freeing them.
    size_t cur_allocated; // number that are currently allocated
    std::vector<void*> freed; // number that have been previously freed.
    MemInfoForSize(size_t size): size(size), countdown(0), cur_allocated(0) { }
  };
  struct MemInfoForSizePitch {
    size_t row_bytes;
    size_t num_rows;
    size_t pitch; // the pitch returned by CudaMallocPitch; this code
    // assumes (and checks) that it's a deterministic function
    // of row_bytes and num_rows.
    size_t countdown;
    size_t cur_allocated; // number that are currently allocated
    std::vector<void*> freed; // number that have been previously freed.
    MemInfoForSizePitch(size_t row_bytes, size_t num_rows):
        row_bytes(row_bytes), num_rows(num_rows), pitch(0),
        countdown(0), cur_allocated(0) { }
  };

  inline MemInfoForSize *FindMemInfo(size_t size) {
    KALDI_ASSERT(size != 0);
    unordered_map<size_t, MemInfoForSize*>::iterator iter =
        size_to_list_.find(size);
    if (iter == size_to_list_.end())
      return (size_to_list_[size] = new MemInfoForSize(size));
    else
      return iter->second;
  }

  inline MemInfoForSizePitch *FindMemInfoPitch(size_t row_bytes, size_t num_rows) {
    KALDI_ASSERT(row_bytes != 0 && num_rows != 0);
    std::pair<size_t, size_t> this_pair(row_bytes, num_rows);
    unordered_map<std::pair<size_t, size_t>, MemInfoForSizePitch*>::iterator iter =
        pitch_size_to_list_.find(this_pair);
    if (iter == pitch_size_to_list_.end())
      return (pitch_size_to_list_[this_pair] = new MemInfoForSizePitch(row_bytes, num_rows));
    else
      return iter->second;
  }
  ...
  unordered_map<size_t, MemInfoForSize*> size_to_list_;


  unordered_map<void*, MemInfoForSizePitch*> pitch_addr_to_list_;
  unordered_map<std::pair<size_t, size_t>, MemInfoForSizePitch*> pitch_size_to_list_; 
};

void* CuAllocator::Malloc(size_t size) {
  MemInfoForSize *info = FindMemInfo(size);
  if (!info->freed.empty()) {
    ...
    Cleanup(); // will only do cleanup if it decides it's necessary.
    void *addr;
    CU_SAFE_CALL(cudaMalloc(&addr, size));
    info->cur_allocated++;
    // HERE info->
  }
}
