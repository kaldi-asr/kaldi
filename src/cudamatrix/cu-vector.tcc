...
namespace kaldi {
  ...
  template<typename _ElemT>
  CuVector<_ElemT>& CuVector<_ElemT>::CopyFrom(const CuVector<_ElemT>& src) {
    Resize(src.Dim());
    ...
    if(CuDevice::Instantiate().Enabled()) { 
      ...
      CuDevice::Instantiate().AccuProfile("CuVector::CopyFromD2D",tim.Elapsed());
    } else
    #endif
    {
      vec_.CopyVec(src.vec_);
    }
  ...
  template<typename _ElemT>
  CuVector<_ElemT>& CuVector<_ElemT>::CopyFrom(const Vector<_ElemT>& src) {
    Resize(src.Dim());
    ...
    if(CuDevice::Instantiate().Enabled()) { 
      ...
      CuDevice::Instantiate().AccuProfile("CuVector::CopyFromH2D",tim.Elapsed());
    } else
    #endif
    {
  ...
  template<typename _ElemT>
  void CuVector<_ElemT>::CopyTo(Vector<_ElemT>& dst) const {
    if(dst.Dim() != dim_) {
      dst.Resize(dim_);
    }
    ...
    if(CuDevice::Instantiate().Enabled()) { 
      ...
      CuDevice::Instantiate().AccuProfile("CuVector::CopyToD2H",tim.Elapsed());
    } else
    #endif
    {
  ...
  template<typename _ElemT>
  CuVector<_ElemT>& CuVector<_ElemT>::CopyFrom(const std::vector<_ElemT>& src) {
    Resize(src.size());
  ...
  template<typename _ElemT>
  void CuVector<_ElemT>::CopyTo(std::vector<_ElemT>& dst) const {
    if(dst.Dim() != dim_) {
      dst.resize(dim_);
    }
