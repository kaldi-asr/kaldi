...
namespace kaldi {
  ...
  template<typename _ElemT>
  CuMatrix<_ElemT>& CuMatrix<_ElemT>::CopyFrom(const CuMatrix<_ElemT>& src) {
    ...
    if(CuDevice::Instantiate().Enabled()) { 
      ...
      CuDevice::Instantiate().AccuProfile("CuMatrix::CopyFromD2D",tim.Elapsed());
    } else
    #endif
    {
  ...
  template<typename _ElemT>
  CuMatrix<_ElemT>& CuMatrix<_ElemT>::CopyFrom(const Matrix<_ElemT>& src) {
    Resize(src.NumRows(),src.NumCols());
    ...
    if(CuDevice::Instantiate().Enabled()) { 
      ...
      CuDevice::Instantiate().AccuProfile("CuMatrix::CopyFromH2D",tim.Elapsed());
    } else
    #endif
    {
  ...
  template<typename _ElemT>
  Matrix<_ElemT>& CuMatrix<_ElemT>::CopyTo(Matrix<_ElemT>& dst) const {
    ...
    if(CuDevice::Instantiate().Enabled()) { 
      ...
      CuDevice::Instantiate().AccuProfile("CuMatrix::CopyToD2H",tim.Elapsed());
    } else
    #endif
    {
