...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage = "Computes the Actual Term-Weighted Value and prints it."
        ...
        "     the reference and the hypotheses (found hits), consecutively.\n"
        "     The format is the same for both of them. Each line is of \n"
        "     the following format\n"
        "\n"
        "     <KW-ID> <utterance-id> <start-time-frames> <end-time-frames> <score>\n\n"
        "     e.g.:\n\n"
        "     KW106-189 348 459 560 0.8\n"
        "\n"
        "  b) the alignment-csv file format is equivalent with the alignment file\n"
        "     produced using the F4DE tool. However, we do not set some fields\n"
        "     and the utterance identifiers are numeric. You can use the script\n"
        "     utils/int2sym.pl and the utterance/keyword maps to convert\n" 
        "     the numerical ids into text\n"
