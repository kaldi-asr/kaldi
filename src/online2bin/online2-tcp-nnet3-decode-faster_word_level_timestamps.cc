          ...
          if (eos) {
            ...
            if (decoder.NumFramesDecoded() > 0) {
              ...
              for (int i = word_count; i < message_vector.size(); i++) {
                ...
                currect_hypothesis = currect_hypothesis + "{start: " + str_start + "," + " end: " + str_end + "," + " word: " + word + "} ";
              }

              server.WriteLn(currect_hypothesis + "\n", "\r");
          ...
          if (samp_count > check_count) {
            if (decoder.NumFramesDecoded() > 0) {
              ...
              for (int i = 0; i < words.size(); i++) {
                ...
                std::string str_start = std::to_string(times[i] * frame_shift * frame_subsampling);
                std::string str_end = std::to_string((times[i] + lengths[i]) * frame_shift * frame_subsampling);
                message = message + "{start: " + str_start + "," + " end: " + str_end + "," + " word: " + word + "} ";
