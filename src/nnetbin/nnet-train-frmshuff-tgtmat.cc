...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    CuMatrix<BaseFloat> feats_transf, out_feats_transf, nnet_out, obj_diff;
    ...
    while (!feature_reader.Done()) {
      ...
      for ( ; !feature_reader.Done(); feature_reader.Next()) {
        ...
        Matrix<BaseFloat> tgt_mat = targets_reader.Value();
        ...
        // correct small length mismatch or drop sentence,
        {
          ...
          length.push_back(tgt_mat.NumRows());
          ...
          if (max - min < length_tolerance) {
            ...
            if (tgt_mat.NumRows() != min) tgt_mat.Resize(min, tgt_mat.NumCols(), kCopyData);
            if (weights.Dim() != min) weights.Resize(min, kCopyData);
          } else {
            KALDI_WARN << "Length mismatch! Targets " << tgt_mat.NumRows()
        ...
        out_nnet_transf.Feedforward(CuMatrix<BaseFloat>(tgt_mat), &out_feats_transf);
        ...
        // remove frames with '0' weight from training,
        {
          ...
          if (weight_min == 0.0) {
            ...
            // filter targets,
            CuMatrix<BaseFloat> tmp_out_feats(keep_frames.size(), out_feats_transf.NumCols());
            tmp_out_feats.CopyRows(out_feats_transf, CuArray<MatrixIndexT>(keep_frames));
            tmp_out_feats.Swap(&out_feats_transf);
        ...
        KALDI_ASSERT(feats_transf.NumRows() == out_feats_transf.NumRows());
        feature_randomizer.AddData(feats_transf);
        targets_randomizer.AddData(out_feats_transf);
