...
bool ReadData(SequentialBaseFloatMatrixReader& feature_reader,
              SequentialBaseFloatMatrixReader& target_reader,
  ...
  for ( ; !feature_reader.Done(); feature_reader.Next()) {
    ...
    std::string tgt_utt = target_reader.Key();
    KALDI_VLOG(3) << "Reading " << utt;
    while (tgt_utt != utt) {
        if (target_reader.Done()) {
            target_reader.Close();
            break;
        }
        target_reader.Next();
        tgt_utt = target_reader.Key();
    }
    if (tgt_utt != utt) {//!targets_reader.HasKey(fea_utt)) {
        KALDI_WARN << "No targets for: " << utt;
        (*num_no_tgt_mat)++;
        continue;
    }
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "The training targets are matrices.\n"
        ...
        "e.g.: nnet-train-lstm-streams scp:feature.scp scp:outfeat.scp nnet.init nnet.iter1\n";
    ...
    std::string output_feature_transform;
    po.Register("output-feature-transform", &output_feature_transform,
         "Output feature transform in Nnet format");
    ...
    Nnet nnet_transf, out_nnet_transf;
    if (feature_transform != "") {
      nnet_transf.Read(feature_transform);
    }
    if (output_feature_transform != "") {
      out_nnet_transf.Read(output_feature_transform);
    }
    ...
    SequentialBaseFloatMatrixReader target_reader(targets_rspecifier);
    RandomAccessBaseFloatVectorReader weights_reader;
    if (frame_weights != "") {
    ...
    std::vector<Matrix<BaseFloat> > targets_utt(num_streams);
    std::vector<Vector<BaseFloat> > weights_utt(num_streams);
    std::vector<int32> new_utt_flags(num_streams);

    CuMatrix<BaseFloat> feats_transf, nnet_tgt, nnet_out, obj_diff;
    ...
    while (1) {
      ...
      for (int s = 0; s < num_streams; s++) {
        ...
        if (feats_utt[s].NumRows() == 0) {
          ...
          if (ReadData(feature_reader, target_reader, weights_reader,
            ...
            out_nnet_transf.Feedforward(CuMatrix<BaseFloat>(targets), &nnet_tgt);
            ...
            targets_utt[s] = Matrix<BaseFloat>(nnet_tgt);
      ...
      CuMatrix<BaseFloat> target_mat_host;
      Vector<BaseFloat> weight_host;
      {
        ...
        target_mat_host.Resize(n_streams * batch_size, nnet.OutputDim(), kSetZero);
        ...
        // pack the data,
        {
          ...
          for (int32 s = 0; s < n_streams; s++) {
            const Matrix<BaseFloat>& target_mat_tmp = targets_utt[s];
            for (int32 r = 0; r < frame_num_utt[s]; r++) {
                target_mat_host.Row(r*n_streams + s).CopyFromVec(target_mat_tmp.Row(r));
            }
          }
        ...
        // remove the data we just packed,
        {
          for (int32 s = 0; s < n_streams; s++) {
            ...
            // labels,
            Matrix<BaseFloat>& tm = targets_utt[s];
            if (tm.NumRows() == frame_num_utt[s]) {
              targets_utt[s].Resize(0,0);  // we packed last chunk,
            } else {
              targets_utt[s] = Matrix<BaseFloat>(
                tm.RowRange(frame_num_utt[s], tm.NumRows() - frame_num_utt[s])
              );
            }
            // weights,
      ...
      if (objective_function == "xent") {
        xent.Eval(weight_host, nnet_out, target_mat_host, &obj_diff);
      } else if (objective_function == "mse") {
        mse.Eval(weight_host, nnet_out, target_mat_host, &obj_diff);
      } else {
