...
// a 'sequence' way from the start).  The implementation is efficient for the
// case where the number of symbols is quite small, e.g. not more than a few
// hundred, so tabulating probabilities makes sense.  We don't put too much
// effort in making this the best possible language model and adding bells and
// whistles.  We're implementing the count cutoffs in the way that we feel makes
// the most sense; it probably won't exactly match the way it's done in, say,
// SRILM.  And the way the discounting is done is also not quite the same it's
// done in the original Kneser-Ney publication, as we do it using continuous
...
/**
   ...
   valid history-state).  This ends up mattering later on.
 */
class LanguageModel {
 public:
...
// with the predicted word, is sufficient to work out the probability; it also
// provides a way to work out whether a language model history is a prefix of a
// longer language model history.  It's useful in the training code and in
// conversion to an FST.  Because this isn't something that would normally
