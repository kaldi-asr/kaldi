// chain/chain-supervision-test2.cc
...
void TestSupervisionNumerator(const DenominatorGraph &den_graph,
...
////////HHH////////
  std::cout << "################### Num Seqs: " << supervision.num_sequences << ", sup.fsts.size: " << supervision.fsts.size() << "\n";
  if (supervision.num_sequences == 1 && supervision.fsts.size() == 0) {
    supervision.fsts.push_back(supervision.fst);
  }
  std::cout << "#####-- now --##### Num Seqs: " << supervision.num_sequences << ", sup.fsts.size: " << supervision.fsts.size() << "\n";
//////////////////

  ChainTrainingOptions opts;
  if (RandInt(0, 1) == 1)
    opts.num_leak_coefficient = 0.01;
  NumeratorGraph ng(supervision);
  CuLeakyNumeratorComputation num(opts, ng, den_graph, nnet_output);
  ...
  for (int32 p = 0; p < dim; p++) {
    ...
    CuLeakyNumeratorComputation num2(opts, ng, den_graph, new_nnet_output);
...

  {
    ...
    CuLeakyNumeratorComputation num_mod(opts, ng, den_graph, nnet_output_mod);
...
void TestSupervisionAppend(const ContextDependency &ctx_dep,
                           const TransitionModel &trans_model,
                           Supervision &supervision) {
  ...
  std::cout << "Added weight!\n";
  WriteFstKaldi(supervision.fst, "tmp2");
  ...
  bool compactify = true;//(RandInt(0, 1) == 0);
  ...
  TestSupervisionNumerator(den_graph, output[0]);
...
void ChainSupervisionTest() {
  ...
  TestSupervisionAppend(*ctx_dep, *trans_model, supervision);
