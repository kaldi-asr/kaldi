        ...
        while (true) {
            ...
            while (1) {
                ...
                if (dstate & (decoder.kEndFeats | decoder.kEndUtt)) {
                    ...
                    MinimumBayesRisk mbr(aligned_lat, true);
                    ...
                    if (words_num > 0) {
                        ...
                        sstr << "RESULT:NUM=" << words_num << ",FORMAT=WSEC,RECO-DUR=" << dur
                             << ",INPUT-DUR=" << input_dur;
                        ...
                        for (size_t i = 0; i < word_ids.size(); i++) {
                            ...
                            wstr << word << "," << (frame_shift * (times[i].first+decoder_offset)) << "," << (frame_shift * (times[i].second+decoder_offset))<< "," << conf[i];
                ...
                else
                {
                    ...
                    if (decoder.PartialTraceback(&out_fst)) {
                        GetLinearSymbolSequence(out_fst,static_cast<vector<int32> *>(0),&word_ids, static_cast<LatticeArc::Weight*>(0));
                        for (size_t i = 0; i < word_ids.size(); i++) {
                            if(word_ids[i]!=0) {
                               WriteLine(client_socket,"PARTIAL:"+word_syms->Find(word_ids[i]));
                            }
                        }
                    }
                }
