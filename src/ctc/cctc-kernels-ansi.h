...
extern "C" {
  ...
  // The threadIdx.x and (if necessary) blockIdx.x determine which sequence (0
  // ... num_sequences - 1) we handle.
  ...
  // den-index or num-index, and then sorted first on t and then on the minibatch-element.

  // We could do this by, for each time t, transposing a sub-matrix containing
  // just the relevant time-values.
