...
int main(int argc, char *argv[]) {
  try {
    ...
    bool log_sum_exp = false;

    po.Register("log-sum-exp", &log_sum_exp, "Sum columns considering the "
                "numbers to be stored in log");
    ...
    for (; !mat_reader.Done(); mat_reader.Next()) {
      ...
      if (!log_sum_exp) {
        Matrix<double> mat(mat_reader.Value(), kTrans);
        ...
        num_rows_done += mat.NumCols();
      } else {
        Matrix<double> mat(mat_reader.Value());
        Vector<double> vec(mat.NumRows());
        for (size_t t = 0; t < mat.NumRows(); t++) {
          SubVector<double> mat_t(mat, t);
          vec(t) = mat_t.LogSumExp();
        }
        Vector<BaseFloat> float_vec(vec);
        vec_writer.Write(key, float_vec);
    ...
    KALDI_LOG << "Summed columns for " << num_rows_done << " in " 
              << num_done << " matrices, ";
