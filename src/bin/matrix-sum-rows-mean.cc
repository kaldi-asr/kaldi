...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "table of vectors after averaging\n"
        "\n"
        "Usage: matrix-sum-rows-mean [options] <matrix-rspecifier> <vector-wspecifier>\n"
        "e.g.: matrix-sum-rows-mean ark:- ark:- | vector-sum ark:- sum.vec\n"
    ...
    for (; !mat_reader.Done(); mat_reader.Next()) {
      ...
      vec.AddRowSumMat(1.0/mat.NumRows(), mat, 0.0);
