...
void GetEditionsSingleHyp( const std::string &hyp_rspecifier, 
    ...
    for (; !ref_reader.Done(); ref_reader.Next()) {
      ...
      word_errs = LevenshteinEditDistance(ref_sent, hyp_sent, &num_ins, &num_del, &num_sub);
      edit_word_per_hyp.push_back(std::pair<int32, int32>(word_errs, num_words));
    }

}

void GetEditionsDualHyp(const std::string &hyp_rspecifier, 
    ...
    for (; !ref_reader.Done(); ref_reader.Next()) {
      ...
      word_errs = LevenshteinEditDistance(ref_sent, hyp_sent2, &num_ins, &num_del, &num_sub);
      edit_word_per_hyp2.push_back(std::pair<int32, int32>(word_errs, num_words));
    }

}

void GetBootstrapWERInterval(const std::vector<std::pair<int32, int32> > & edit_word_per_hyp,
        int32 replications, 
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "and outputs overall WER statistics to standard output along with its confidence interval\n"
        "using the bootstrap methos of Bisani and Ney\n"
        "If a second transcription file corresponding to the same reference is provided, a\n"
        "bootstrap comparison of the two transcription is performed to estimate the probability\n"
        "of improvement.\n"
        "\n"
        "Usage: compute-wer-confidence [options] <ref-rspecifier> <hyp-rspecifier> [<hyp2-rspecifier>]\n"
        "E.g.: compute-wer-confidence --replications=10000 --mode=present ark:data/train/text ark:hyp_text\n"
    ...
    po.Register("replications", &replications, "Number of replications to compute the intervals");
    ...
    if(hyp2_rspecifier.empty())
        GetEditionsSingleHyp(hyp_rspecifier, ref_rspecifier, mode, edit_word_per_hyp);
    else
        GetEditionsDualHyp(hyp_rspecifier, hyp2_rspecifier, ref_rspecifier, 
                edit_word_per_hyp, edit_word_per_hyp2);

    //Extract WER for a number of replications of the same size as the hypothesis extracted
    BaseFloat mean_wer = 0.0, interval = 0.0, mean_wer2 = 0.0, interval2 = 0.0, p_improv = 0.0;

    GetBootstrapWERInterval(edit_word_per_hyp, replications, &mean_wer, &interval);

    if(!hyp2_rspecifier.empty()) {
        GetBootstrapWERInterval(edit_word_per_hyp2, replications, &mean_wer2, &interval2);
    ...
    std::cout << "Set1: %WER " << std::fixed << 100*mean_wer << " 95\% Conf Interval [ " << 
              100*mean_wer-100*interval << ", " << 100*mean_wer+100*interval
              << " ]" << '\n'; 

    if(!hyp2_rspecifier.empty()) {
        std::cout << "Set2: %WER " << std::fixed << 100*mean_wer2 << " 95\% Conf Interval [ " << 
                  100*mean_wer2-100*interval2 << ", " << 100*mean_wer2+100*interval2
                  << " ]" << '\n';
