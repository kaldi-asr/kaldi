// bin/compute-confidence-interval-wer.cc
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "supposing that WER of sentences subsamples follow the normal distribution.\n"
        "\n"
        "Usage: compute-confidence-interval-wer [options] <ref-rspecifier> <hyp-rspecifier>\n"
        "E.g.: compute-confidence-interval-wer --replications=10000 --mode=present ark:data/train/text ark:hyp_text\n"
    ...
    BaseFloat mean_wer = wer_accum / (replications-1);
    BaseFloat std_wer = sqrt(wer_mult_accum / (replications-1) - mean_wer*mean_wer);

    // Print the ouptut,
    std::cout.precision(2);
    std::cerr.precision(2);
    std::cout << "%WER " << std::fixed << 100*mean_wer << " 95\% Conf Interval [ " << 
              100*mean_wer-100*1.96*std_wer << ", " << 100*mean_wer+100*1.96*std_wer
              << " ]" << '\n'; 
