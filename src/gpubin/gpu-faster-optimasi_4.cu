...
struct GPUDiagGmm{
  ...
  BaseFloat LogLikelihood(BaseFloat *data, int32 num_data, BaseFloat* loglikes, BaseFloat* data_sq) {
    ...
    for(int i = 0;i < gconsts_.Dim(); ++i){
      BaseFloat incs = 0, decs = 0;
      for(int j = 0;j < num_data; ++j){
        incs += means_invvars_.data[means_invvars_.Index(i, j)] * data[j];
        decs += inv_vars_.data[inv_vars_.Index(i, j)] * data_sq[j];
      }
      loglikes[i] += (incs - 0.5f * decs);
...
int viterbi(gpu_fst &m, 
  ...
  int start_offset = m.input_offsets[EPS_SYM];
  int start_offset_noneps = m.input_offsets[EPS_SYM + 1];
  int end_offset = m.input_offsets.back();
  int EPS_BLOCKS = ceildiv(start_offset_noneps - start_offset, BLOCK_SIZE);
  int NONEPS_BLOCKS = ceildiv(end_offset - start_offset_noneps, BLOCK_SIZE);
  int BLOCKS = ceildiv(end_offset - start_offset, BLOCK_SIZE);
  int LIKELIHOOD_BLOCKS = ceildiv(NUM_PDFS, LIKELIHOOD_BLOCK_SIZE);
  compute_initial <<<EPS_BLOCKS, BLOCK_SIZE>>> (
    m.from_states.data().get(),
    m.to_states.data().get(),
    m.probs.data().get(),
    m.inputs.data().get(),
    start_offset, start_offset_noneps,
    m.initial,
    viterbi.data().get(),
    m.num_states
  );
  ...
  for (int t = NUM_LAYER; !decodable->IsLastFrame(frame_ - 1) && batch_frame < BATCH_SIZE;
    ...
    compute_emitting <<<NONEPS_BLOCKS, BLOCK_SIZE>>> (
      m.from_states.data().get(), 
      m.to_states.data().get(),
      m.probs.data().get(),
      m.inputs.data().get(),
      start_offset_noneps, end_offset, 
      frame_,
