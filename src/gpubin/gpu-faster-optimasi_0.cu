 ...
 */
int viterbi(gpu_fst &m, OnlineDecodableDiagGmmScaled* decodable, GPUOnlineDecodableDiagGmmScaled* gpu_decodable, std::vector<sym_t> &output_symbols, std::vector<sym_t> input_symbols) {
  int verbose=0;
  ...
  try {
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      while(1){
        ...
        int state = viterbi(m, &decodable, gpu_decodable_d, output_symbols, input_symbols);
