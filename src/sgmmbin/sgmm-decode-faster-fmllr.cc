...
int main(int argc, char *argv[]) {
  try {
    ...
    SgmmFmllrGlobalParams fmllr_globals;
    {
      ...
      fmllr_globals.Read(is.Stream(), binary);
    }
    ...
    Matrix<BaseFloat> fmllr_xform(am_sgmm.FeatureDim(), am_sgmm.FeatureDim()+1);
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (fmllr_reader.HasKey(utt_or_spk)) {
        fmllr_xform.CopyFromMat(fmllr_reader.Value(utt_or_spk));
//        logdet = fmllr_xform.Range(0, dim, 0, dim).LogDet();
      } else {
        KALDI_WARN << "Cannot find FMLLR transform for " << utt_or_spk;
        fmllr_xform.SetUnit();
//        logdet = 0.0;
      }
      ...
      DecodableAmSgmmFmllr sgmm_decodable(sgmm_opts, am_sgmm, spk_vars,
