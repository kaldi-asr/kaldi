...
int main(int argc, char *argv[]) {
  try {
    ...
    // If it is not an archive the second argument is required
    if (po.NumArgs() != 2 && in_is_rspecifier) {
      po.PrintUsage();
      exit(1);
    }

    std::string wav_out_fn = "-";
    if (po.NumArgs() > 1) {
      wav_out_fn = po.GetArg(2);
    }
    bool out_is_wspecifier = (ClassifyWspecifier(wav_out_fn, NULL, NULL, NULL)
                              != kNoWspecifier);
