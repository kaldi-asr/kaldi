...
struct NnetChainaTrainingOptions {
  NnetTrainerOptions bottom_nnet_config;
  NnetTrainerOptions top_nnet_config;
  bool train_bottom_nnet;  // True if we will be training the bottom nnet.
  bool train_top_nnet;    // True if we will be training the top nnet.  Either
                          // this or train_bottom_nnet must be true (else, what
                          // are we doing here?)
  ...
  void Register(OptionsItf *opts) {
    // register bottom_nnet_config with the prefix bottom
    ParseO
