...
def main():
    ...
    parser.add_argument(
        "ref", type=str, default="ref.txt", help="path to the reference"
    )
    parser.add_argument(
        "hyp", type=str, default="hyp.txt", help="path to the hypothesis"
    )
    parser.add_argument(
        "--wake-word", type=str, dest="wake_word", default="嗨小问", help="wake word"
    )
    ...
    f = (
        ...
        if args.ref == "-"
        else codecs.open(args.ref, "r", encoding="utf-8")
    )
    lines = f.readlines()
    ref = [
        line.strip().split(None, 1)
        if len(line.strip().split(None, 1)) == 2
        else [line.strip().split(None, 1)[0], ""]
        for line in lines
    ]
    f.close()
    ...
    f = (
        io.TextIOWrapper(sys.stdin.buffer, encoding="utf-8")
        if args.hyp == "-"
        else codecs.open(args.hyp, "r", encoding="utf-8")
    )
    lines = f.readlines()
    hyp = {}
    for line in lines:
        hyp[line.strip().split(None, 1)[0]] = (
    ...
    if len(ref) != len(hyp):
        print(
            "The lengths of reference and hypothesis do not match. ref: {} vs hyp: {}.".format(
                len(ref), len(hyp)
            ),
            file=sys.stderr,
        )
    ...
    for i in range(len(ref)):
        if ref[i][0] not in hyp:
            print(
                "reference {} does not exist in hypothesis.".format(ref[i][0]),
                file=sys.stderr,
            )
            continue
    ...
    print(
        "precision: %.5f  recall: %.5f  FPR: %.5f  FNR: %.5f  FP per hour: %.5f  total: %d"
        % (
            precision,
            recall,
            false_positive_rate,
