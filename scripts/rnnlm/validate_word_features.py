...
parser = argparse.ArgumentParser(
    description="Validates word features file, produced by rnnlm/get_word_features.py.",
...
with open(args.word_features_file, "r", encoding="utf-8") as f:
    for line in f:
        ...
        if len(fields) == 1:
            if word_id != 0:
                sys.exit(
                    sys.argv[0] + ": Only <eps> can have no feature: {0}.".format(line)
                )
        i = 1
        last_feat_id = -1
        while i < len(fields):
            ...
            if feat_id <= last_feat_id:
                sys.exit(
                    sys.argv[0]
                    + ": features must be listed in increasing order: {0} <= {1} in {2}.".format(
                        feat_id, last_feat_id, line
                    )
                )
            ...
            if feat_id > max_feat_id:
                ...
                if len(fields) != 3 and len(fields) != 5:
                    sys.exit(
                        sys.argv[0]
                        + ": Special word can only have one or 2 features: {0}.".format(
                            line
                        )
                    )
                try:
                    ...
                    sys.exit(
                        sys.argv[0]
                        + ": Value of special word feature should be a float number: {0}.".format(
                            line
                        )
                    )
                ...
                if abs(float(feat_value) - constant_feat_value) > 1e-6:
                    sys.exit(
                        sys.argv[0]
                        + ": Value of constant feature is not right: {0}".format(line)
                    )
                ...
                try:
                    ...
                    sys.exit(
                        sys.argv[0]
                        + ": Value of unigram feature should be a float number: {0}.".format(
                            line
                        )
                    )
            i += 2
