...
parser = argparse.ArgumentParser(
    description="This script turns the words into the sparse feature representation, "
...
def get_feature_list(word, idx):
    ...
    if "unigram" in feats:
        if unigram_probs is None:
            sys.exit(
                sys.argv[0] + ": if unigram feature is present, you must specify the "
...
for word, idx in sorted(vocab.items(), key=lambda x: x[1]):
    ...
    print(
        "{0}\t{1}".format(
            idx, " ".join(["%s %.3g" % (f, v) for f, v in sorted(feature_list.items())])
        )
    )

print(
    sys.argv[0] + ": made features for {0} words.".format(len(vocab)), file=sys.stderr
)
