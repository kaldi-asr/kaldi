...
def get_all_data_sources_except_dev(text_dir):
    ...
    if data_sources == {}:
        sys.exit(
            sys.argv[0] + ": data directory {0} contains no .txt files "
...
def distribute_to_outputs(source_filename, weight, output_filehandles):
    ...
    try:
        ...
        sys.exit(
            sys.argv[0]
            + ": failed to open file {0} for reading: {1} ".format(
...
temp_files = [
    "{0}/{1}.tmp".format(args.split_dir, n) for n in range(1, args.num_splits + 1)
]
...
if ret != 0:
    sys.exit(
        sys.argv[0] + ": command '{0}' returned with status {1}".format(command, ret)
    )
