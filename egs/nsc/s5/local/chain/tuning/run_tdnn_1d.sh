...
# This is based on LibriSpeech local/chain/tuning/run_tdnn_1d.sh

# configs for 'chain'
stage=0
decode_nj=80
train_set=train  #_cleaned
gmm=tri6b  #_cleaned
...
if [ $stage -le 15 ]; then
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $dir/egs/storage ]; then
    utils/create_split_dir.pl \
     /export/c{23,24,25,27}/$USER/kaldi-data/egs/swbd-$(date +'%m_%d_%H_%M')/s5c/$dir/egs/storage $dir/egs/storage
  fi
...
if [ $stage -le 16 ]; then
  ...
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 --remove-oov data/lang_test_tg $dir $graph_dir
fi
...
if [ $stage -le 17 ]; then
  ...
  for decode_set in test; do
      ...
      steps/lmrescore_const_arpa.sh \
          ...
          data/${decode_set}_hires $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_{tg,fg} || exit 1
      ) || touch $dir/.error &
  done
  wait
  if [ -f $dir/.error ]; then
