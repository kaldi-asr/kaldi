...
comb_affix=best_path_comb1c  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
if [ $stage -le 8 ]; then
  steps/best_path_weights.sh --cmd "${train_cmd}" --acwt 0.1 --write-words true \
...
if [ $stage -le 9 ]; then
  out_dir=$chaindir/1best_lats_${unsupervised_set}_sp${decode_affix}_fg
...
unsup_lat_dir=${chaindir}/1best_lats_${unsupervised_set}${decode_affix} 
