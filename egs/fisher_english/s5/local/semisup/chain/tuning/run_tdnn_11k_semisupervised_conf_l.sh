...
# This script is same as _f, but uses UNK model.
...
# LM for decoding unsupervised data: 4gram with UNK model
...
egs_affix=${egs_affix}_prun${lattice_prune_beam}_lmwt${lattice_lm_scale}_tol${tolerance}${sil_tolerance:+_sil$sil_tolerance}
...
if [ $stage -le 1 ]; then
  ...
  utils/prepare_lang.sh --unk-fst exp/unk_lang_model/unk_fst.txt \
    data/local/dict "$(cat data/lang/oov.txt)" data/local/lm${graph_affix} data/lang_test${graph_affix}_unk

  cp data/lang_test${graph_affix}/G.fst data/lang_test${graph_affix}_unk/
fi

if [ ! -f data/lang_test${graph_affix}_unk_fg/G.carpa ]; then
  utils/build_const_arpa_lm.sh \
    data/local/lm${graph_affix}/4gram-mincount/lm_unpruned.gz \
    data/lang_test${graph_affix}_unk data/lang_test${graph_affix}_unk_fg
fi

graph_affix=${graph_affix}_unk
...
unsup_lat_dir=${chaindir}/decode_${unsupervised_set}_sp${decode_affix} 
...
sup_lat_dir=${chaindir}_lats_${supervised_set}_sp

if [ $stage -le 7 ]; then
  steps/nnet3/align_lats.sh --nj $decode_nj --cmd "$decode_cmd" --write-best-path-alignments true \
    --acoustic-scale 1.0 --scale-opts "--transition-scale=1.0 --self-loop-scale=1.0" \
    --online-ivector-dir $exp/nnet3${nnet3_affix}/ivectors_${base_train_set}_sp_hires \
    data/${supervised_set}_sp_hires \
    data/lang_test${graph_affix} $chaindir $sup_lat_dir || exit 1
fi
...
if [ $stage -le 8 ]; then
  ...
  steps/subset_ali_dir.sh --cmd "$train_cmd" \
    ...
    $chaindir/best_path_${unsupervised_set}_sp${decode_affix} \
    $chaindir/best_path_${unsupervised_set}${decode_affix} || exit 1
  echo $frame_subsampling_factor > $chaindir/best_path_${unsupervised_set}${decode_affix}/frame_subsampling_factor
  
  steps/subset_ali_dir.sh --cmd "$train_cmd" \
    data/${supervised_set} data/${supervised_set}_sp_hires \
    ${chaindir}_lats_${supervised_set}_sp \
    ${chaindir}_best_path_${supervised_set} || exit 1
  echo $frame_subsampling_factor > ${chaindir}_best_path_${supervised_set}/frame_subsampling_factor

  steps/nnet3/chain/build_tree_multiple_sources.sh \
    --frame-subsampling-factor $frame_subsampling_factor \
    --use-fmllr false \
    --cmd "$train_cmd" 10000 data/lang_chain \
    data/${supervised_set} ${chaindir}_best_path_${supervised_set} \
...
if [ -z "$sup_egs_dir" ]; then
  ...
  if [ $stage -le 12 ]; then
    ...
    steps/nnet3/chain/get_egs.sh --cmd "$decode_cmd" --alignment-subsampling-factor 1 \
               ...
               ${sil_tolerance:+--left-tolerance-silence $sil_tolerance --right-tolerance-silence $sil_tolerance} \
