...
comb_affix=comb1u  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
for dset in $unsupervised_set; do
  if [ $stage -le 3 ]; then
    if [ -f data/${dset}_spEx_hires/feats.scp ]; then
      echo "$0: data/${dset}_spEx_hires/feats.scp exists. Remove it re-run or skip this stage."
      exit 1
    fi

    utils/data/perturb_data_dir_speed_3way.sh data/$dset data/${dset}_sp_tmp_hires
    utils/data/segment_data.sh --nj 30 --cmd "$train_cmd" \
      data/${dset}_sp_tmp_hires data/${dset}_sp_seg_hires

    python utils/data/perturb_speed_to_allowed_lengths.py --only-speed-perturb \
        12 data/${dset}_sp_seg_hires data/${dset}_spEx_hires

    utils/fix_data_dir.sh data/${dset}_spEx_hires

    steps/make_mfcc.sh --mfcc-config conf/mfcc_hires.conf \
        --write-utt2num-frames true --nj 40 --cmd "$train_cmd" \
        data/${dset}_spEx_hires
  ...
  if [ $stage -le 4 ]; then
    steps/compute_cmvn_stats.sh \
      ...
      steps/online/nnet2/copy_data_dir.sh --utts-per-spk-max 2 \
        data/${dset}_spEx_hires data/${dset}_spEx_max2_hires || exit 1;

    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
      data/${dset}_spEx_max2_hires $exp/nnet3${nnet3_affix}/extractor \
      $exp/nnet3${nnet3_affix}/ivectors_${dset}_spEx_hires || exit 1;
  fi

  if [ $stage -le 5 ] && [ ! -f $chaindir/decode_${dset}_spEx${decode_affix}/lat.1.gz ]; then
    echo "$0: getting the decoding lattices for the unsupervised subset using the chain model at: $chaindir"
    steps/nnet3/decode.sh --num-threads 4 --nj $decode_nj --cmd "$decode_cmd" \
              --acwt 1.0 --post-decode-acwt 10.0 \
              --online-ivector-dir $exp/nnet3${nnet3_affix}/ivectors_${dset}_spEx_hires \
              --scoring-opts "--min-lmwt 10 --max-lmwt 10" \
              $graphdir data/${dset}_spEx_hires $chaindir/decode_${dset}_spEx${decode_affix}
  fi

  if [ $stage -le 6 ]; then
    steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" data/lang_test${graph_affix} \
      data/lang_test${graph_affix}_fg data/${dset}_spEx_hires \
      $chaindir/decode_${dset}_spEx${decode_affix} \
      $chaindir/decode_${dset}_spEx${decode_affix}_fg

    ln -s ../final.mdl $chaindir/decode_${dset}_spEx${decode_affix}_fg/final.mdl || true
  fi
done
...
if [ $stage -le 8 ]; then
  steps/best_path_weights.sh --cmd "${train_cmd}" --acwt 0.1 \
    data/${unsupervised_set}_spEx_hires data/lang_chain \
    $chaindir/decode_${unsupervised_set}_spEx${decode_affix} \
    $chaindir/best_path_${unsupervised_set}_spEx${decode_affix}
fi
...
if [ $stage -le 10 ]; then
  steps/subset_ali_dir.sh --cmd "$train_cmd" \
    data/${unsupervised_set} data/${unsupervised_set}_spEx_hires \
    $chaindir/best_path_${unsupervised_set}_spEx${decode_affix} \
  ...
  steps/nnet3/chain/make_weighted_den_fst.sh --num-repeats $lm_weights --cmd "$train_cmd" --lm-opts "$lm_opts" \
...
unsupervised_set=${unsupervised_set}_spEx
