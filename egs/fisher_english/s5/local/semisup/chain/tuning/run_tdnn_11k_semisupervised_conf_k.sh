...
graph_dir=$dir/graph${lang_test_suffix}${unk_prob_scale:+=_unkscale$unk_prob_scale}
if [ $stage -le 17 ]; then
  # Note: it might appear that this $lang directory is mismatched, and it is as
  # far as the 'topo' is concerned, but this script doesn't read the 'topo' from
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 ${unk_prob_scale:+--unk-prob-scale $unk_prob_scale} \
    data/lang_test${lang_test_suffix} $dir $graph_dir
fi
