...
tree_affix=${tree_affix:+${tree_affix}_}${semi_affix}
...
for dset in $unsupervised_set; do
  ...
  if [ $stage -le 5 ]; then
    steps/lmrescore_rnnlm_lat.sh --cmd "$tfrnnlm_cmd --mem 16G" --expand-ngram true --beam 6 --inv-acwt 10 \
...
tree_affix=${tree_affix}.${rnnlm_affix}.lat.3gram.${rnnlm_weight}
