...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 13 ]; then
    ...
    steps/nnet3/chain/get_egs_split_and_convert.sh --cmd "$decode_cmd" --alignment-subsampling-factor 1 \
               ...
               --online-ivector-dir $exp/nnet3${nnet3_affix}/ivectors_${base_train_set}_sp_hires \
               --generate-egs-scp true \
               --splitter-opts "--add-partial-unk-label-left --add-partial-unk-label-right" \
               $unsup_egs_opts \
               data/${unsupervised_set}_hires data/lang_chain $dir \
