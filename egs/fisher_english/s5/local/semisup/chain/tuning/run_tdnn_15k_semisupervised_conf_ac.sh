...
# This script is same as _ab, but uses frames_per_eg 150. This is same as _z with rescoring done correctly.
...
comb_affix=comb1ac  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
if [ -z "$sup_egs_dir" ]; then
  ...
  if [ $stage -le 12 ]; then
    ...
    mkdir -p $sup_egs_dir/
    ! $remove_egs && touch $sup_egs_dir/.nodelete # keep egs around when that run dies.
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 13 ]; then
    ...
    ! $remove_egs && touch $unsup_egs_dir/.nodelete # keep egs around when that run dies.
