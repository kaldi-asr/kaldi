...
arc_scale=1.0  # arc-scale for sausage arcs
...
comb_affix=comb1v  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
egs_affix=${egs_affix}_prun${lattice_prune_beam}_awt${arc_scale}_tol${tolerance}
tree_affix=
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 13 ]; then
    ...
    steps/nnet3/chain/get_egs.sh --cmd "$decode_cmd" --alignment-subsampling-factor 1 \
               ...
               --frame-subsampling-factor $frame_subsampling_factor \
               --cmvn-opts "$cmvn_opts" --lattice-lm-scale 0.0 \
               --use-mbr-decode true --arc-scale $arc_scale \
