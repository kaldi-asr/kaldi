...
# This script is same as _f, but uses a separate silence tolerance.
...
egs_affix=${egs_affix}_prun${lattice_prune_beam}_lmwt${lattice_lm_scale}_tol${tolerance}_sil${sil_tolerance}
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 13 ]; then
    ...
    steps/nnet3/chain/get_egs.sh --cmd "$decode_cmd" --alignment-subsampling-factor 1 \
               ...
               --left-tolerance-silence $sil_tolerance --right-tolerance-silence $sil_tolerance \
               --left-context $left_context --right-context $right_context \
               --left-context-initial $left_context_initial --right-context-final $right_context_final \
