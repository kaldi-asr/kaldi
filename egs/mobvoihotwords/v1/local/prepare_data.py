...
def main():
    ...
    parser.add_argument("wav_dir", type=str, help="dir containing all the wav files")
    parser.add_argument("path", type=str, help="path to the json file")
    parser.add_argument("out_dir", type=str, help="out dir")
    parser.add_argument(
        "--non-wake-word", type=str, default="FREETEXT", help="non-wake word transcript"
    )
    ...
    with open(
        os.path.join(args.out_dir, "wav.scp"), "w", encoding="utf-8"
    ) as f_wav, open(
        os.path.join(args.out_dir, "text"), "w", encoding="utf-8"
    ) as f_text, open(
        os.path.join(args.out_dir, "utt2spk"), "w", encoding="utf-8"
    ) as f_utt2spk:
        for utt, spk, l in zip(utt_id, spk_id, label):
            if spk is None:
                spk = utt  # deal with None speaker
            f_wav.write(
                spk + "-" + utt + " " + os.path.join(abs_dir, utt + ".wav") + "\n"
            )
            if l == 0:
                text = "HiXiaowen"
            elif l == 1:
