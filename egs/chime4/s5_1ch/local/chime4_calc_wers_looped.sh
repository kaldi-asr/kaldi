...
echo ""
mkdir -p $dir/log
for a in `find $dir/decode_looped_tgpr_5k_dt05_real_$enhan/ | grep "\/wer_" | awk -F'[/]' '{print $NF}' | sort`; do
  echo -n "$a "
  if [ -e $dir/decode_looped_tgpr_5k_dt05_simu_$enhan ]; then
    cat $dir/decode_looped_tgpr_5k_dt05_{real,simu}_$enhan/$a | grep WER | awk '{err+=$4} {wrd+=$6} END{printf("%.2f\n",err/wrd*100)}'
  else
    cat $dir/decode_looped_tgpr_5k_dt05_real_$enhan/$a | grep WER | awk '{err+=$4} {wrd+=$6} END{printf("%.2f\n",err/wrd*100)}'
  fi
...
for e_d in $tasks; do
  for task in simu real; do
    rdir=$dir/decode_looped_tgpr_5k_${e_d}_${task}_$enhan
    if [ -e $rdir ]; then
