...
if [ $stage -le 3 ]; then
  local/segmentation/make_sad_tdnn_configs.py \
    ...
    --output-node-parameters "--output-suffix=overlapped_speech --dim=1 --add-final-sigmoid=true --include-log-softmax=false --objective-type=xent-per-dim" \
    $dir/configs
...
if [ -z "$egs_dir" ]; then
  ...
  if [ $stage -le 4 ]; then
    ...
    steps/nnet3/get_egs.py --cmd="$decode_cmd" \
      ...
      --stage=$get_egs_stage \
      --targets-parameters="--output-name=output-snr --target-type=dense --targets-scp=$snr_scp --deriv-weights-scp=$deriv_weights_scp --compress=false" \
      --targets-parameters="--output-name=output-speech --target-type=sparse --dim=2 --targets-scp=$speech_feat_scp --deriv-weights-scp=$deriv_weights_scp --scp2ark-cmd=\"extract-column --column-index=0 scp:- ark,t:- | steps/segmentation/quantize_vector.pl | ali-to-post ark,t:- ark:- |\" --compress=false" \
      --targets-parameters="--output-name=output-overlapped_speech --target-type=dense --dim=1 --targets-scp=$overlapped_speech_labels_scp --deriv-weights-scp=$deriv_weights_for_overlapped_speech_scp --compress=false" \
      --dir=$dir/egs
  fi
fi
