...
extra_egs_copy_cmd="nnet3-copy-egs --keep-outputs=output-speech,output-music,output-speech_music,output-snr ark:- ark:- |"
...
if [ $stage -le -1 ]; then
  utils/combine_data.sh --extra-files "deriv_weights.scp speech_labels.scp irm_targets.scp deriv_weights_for_irm_targets.scp" \
...
if [ -z "$dir" ]; then
  dir=exp/nnet3_stats_sad_music_snr/nnet_tdnn_stats
fi
...
if [ $stage -le 0 ]; then
  ...
  utils/split_data.sh $music_data_dir 40
  utils/split_data.sh $dir/combined_data_dir 40

  $train_cmd JOB=1:40 $dir/log/compute_post_output-speech.JOB.log \
    ali-to-post "scp:utils/filter_scp.pl $dir/combined_data_dir/split40/JOB/utt2spk $dir/combined_data_dir/speech_labels.scp |" ark:- \| \
  ...
  eval vector-sum $dir/post_output-speech.vec.{`seq -s, 40`} $dir/post_output-speech.vec
  rm $dir/post_output-speech.vec.*

  $train_cmd JOB=1:40 $dir/log/compute_post_output-music.JOB.log \
    ali-to-post "scp:utils/filter_scp.pl $music_data_dir/split40/JOB/utt2spk $music_data_dir/music_labels.scp |" ark:- \| \
    post-to-feats --post-dim=2 ark:- ark:- \| \
    matrix-sum-rows ark:- ark:- \| \
    vector-sum ark:- $dir/post_output-music.vec.JOB
  eval vector-sum $dir/post_output-music.vec.{`seq -s, 40`} $dir/post_output-music.vec
  rm $dir/post_output-music.vec.*

  $train_cmd JOB=1:40 $dir/log/compute_post_output-speech_music.JOB.log \
    ali-to-post "scp:utils/filter_scp.pl $dir/combined_data_dir/split40/JOB/utt2spk $dir/combined_data_dir/speech_music_labels.scp |" ark:- \| \
    weight-post ark:- scp:$dir/combined_data_dir/deriv_weights.scp ark:- \| \
    post-to-feats --post-dim=4 ark:- ark:- \| \
    matrix-sum-rows ark:- ark:- \| \
    vector-sum ark:- $dir/post_output-speech_music.vec.JOB
  eval vector-sum $dir/post_output-speech_music.vec.{`seq -s, 40`} $dir/post_output-speech_music.vec
...
if [ $stage -le 1 ]; then
  ...
  num_frames_sad=`vector-to-feat scp:$sad_data_dir/deriv_weights_for_irm_targets.scp ark:- | matrix-sum-rows ark:- ark,t:- | awk '{i+=$3} END{print i}'`
...
for arg in sys.argv[1:]:
  counts_vec.append(float(arg))

min_count = min(counts_vec)

counts_vec = [ x / min_count for x in counts_vec ];
avg_scale = sum(counts_vec) / len(counts_vec)

counts_vec = [ math.pow(x / avg_scale, 0.25) for x in counts_vec ]
print (" ".join([str(x) for x in counts_vec]))' $num_frames_sad $num_frames_music $num_frames_speech_music $num_frames_sad` || exit 1

  echo $scales > $dir/scales
fi 

if [ $stage -le 2 ]; then
  echo "$0: creating neural net configs using the xconfig parser";
  
  scales=`cat $dir/scales`
  
  speech_scale=`echo $scales | awk '{print $1}'`
  music_scale=`echo $scales | awk '{print $2}'`
  speech_music_scale=`echo $scales | awk '{print $3}'`
  snr_scale=`echo $scales | awk '{print $4}'`

  num_snr_bins=`feat-to-dim scp:$sad_data_dir/irm_targets.scp -`
  snr_scale=`perl -e "print $snr_scale / $num_snr_bins"`

  mkdir -p $dir/configs
  cat <<EOF > $dir/configs/network.xconfig
  input dim=`feat-to-dim scp:$sad_data_dir/feats.scp -` name=input

  relu-renorm-layer name=tdnn1 input=Append(-2,-1,0,1,2) dim=$relu_dim add-log-stddev=true
  ...
  stats-layer name=tdnn3_stats config=mean+stddev+count(-99:3:9:99)
  relu-renorm-layer name=tdnn4 input=Append(tdnn3@-6,tdnn3@0,tdnn3@6,tdnn3@12,tdnn3_stats) add-log-stddev=true dim=$relu_dim
  stats-layer name=tdnn4_stats config=mean+stddev+count(-108:6:18:108)
  relu-renorm-layer name=tdnn5 input=Append(tdnn4@-12,tdnn4@0,tdnn4@12,tdnn4@24,tdnn4_stats) dim=$relu_dim
  relu-renorm-layer name=tdnn5-snr input=Append(tdnn3@-6,tdnn3@0,tdnn3@6,tdnn3@12,tdnn5) dim=$relu_dim

  output-layer name=output-speech include-log-softmax=true dim=2 presoftmax-scale-file=$dir/presoftmax_prior_scale_output-speech.txt learning-rate-factor=0.1 objective-scale=$speech_scale input=tdnn5
  output-layer name=output-music include-log-softmax=true dim=2 presoftmax-scale-file=$dir/presoftmax_prior_scale_output-music.txt learning-rate-factor=0.1 objective-scale=$music_scale input=tdnn5
  output-layer name=output-speech_music include-log-softmax=true dim=4 presoftmax-scale-file=$dir/presoftmax_prior_scale_output-speech_music.txt learning-rate-factor=0.1 objective-scale=$speech_music_scale input=tdnn5
  output-layer name=output-snr include-log-softmax=false dim=$num_snr_bins objective-type=quadratic learning-rate-factor=0.1 objective-scale=$snr_scale input=tdnn5-snr
...
if [ -z "$egs_dir" ]; then
  ...
  if [ $stage -le 3 ]; then
    ...
    steps/nnet3/get_egs_multiple_targets.py --cmd="$decode_cmd" \
      ...
      --targets-parameters="--output-name=output-snr --target-type=dense --targets-scp=$sad_data_dir/irm_targets.scp --deriv-weights-scp=$sad_data_dir/deriv_weights_for_irm_targets.scp" \
