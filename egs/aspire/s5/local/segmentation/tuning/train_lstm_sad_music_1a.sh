...
# This script is same as 1e, but removes the stats component in the 3rd layer.
...
music_data_dir=data/train_aztec_unsad_whole_music_corrupted_sp_hires_bp
...
if [ $stage -le 0 ]; then
  utils/combine_data.sh --extra-files "speech_feat.scp deriv_weights.scp irm_targets.scp deriv_weights_manual_seg.scp" \
...
print ($speech_scale / $avg_scale .  " " . $music_scale / $avg_scale)' $num_frames_sad $num_frames_music`

speech_scale=`echo $scales | awk '{print $1}'`
music_scale=`echo $scales | awk '{print $1}'`
...
if [ $stage -le 1 ]; then
  ...
  output-layer name=output-snr include-log-softmax=false dim=$num_snr_bins objective-type=quadratic objective-scale=`perl -e "print (($num_frames_music / $num_frames_sad) ** 0.25) / $num_snr_bins"` input=tdnn5
  output-layer name=output-speech include-log-softmax=true dim=2 objective-scale=`perl -e "print (($num_frames_music / $num_frames_sad) ** 0.25)"` input=tdnn5
  output-layer name=output-music include-log-softmax=true dim=2 input=tdnn5

  output name=output-temp input=Append(input@-2,input@-1,input,input@1,input@2)
EOF
