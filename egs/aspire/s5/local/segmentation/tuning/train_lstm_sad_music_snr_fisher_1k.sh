...
sad_data_dir=data/fisher_train_100k_whole_900_corrupted_spr_hires_bp
music_data_dir=data/fisher_train_100k_whole_900_music_corrupted_spr_hires_bp
...
if [ -z "$dir" ]; then
  dir=exp/nnet3_lstm_sad_music_snr_fisher/nnet_lstm
fi
...
if [ $stage -le 2 ]; then
  ...
  fast-lstmp-layer name=lstm1 cell-dim=$cell_dim recurrent-projection-dim=$projection_dim non-recurrent-projection-dim=$projection_dim delay=-3
  relu-renorm-layer name=tdnn4 input=Append(-6,0,6,12) dim=$relu_dim
  fast-lstmp-layer name=lstm2 cell-dim=$cell_dim recurrent-projection-dim=$projection_dim non-recurrent-projection-dim=$projection_dim delay=-6
  relu-renorm-layer name=tdnn5 input=Append(-12,0,12,24) dim=$relu_dim
  relu-renorm-layer name=tdnn5-snr input=Append(lstm2@-6,lstm2@0,lstm2@6,lstm2@12,tdnn5) dim=$relu_dim
