...
# and learning rate factor 0.01 for the final affine components.
# Decreased learning rate factor of overlapped speech to 0.025 and 0.05 for speech.
# Changed relu-dim to 512
...
if [ $stage -le 1 ]; then
  ...
  relu-renorm-layer name=tdnn2 input=Append(tdnn1@-6, tdnn1, tdnn2_stats) dim=512
  relu-renorm-layer name=tdnn3 input=Append(-9,0,3) dim=512
  relu-renorm-layer name=tdnn4 dim=512

  output-layer name=output-speech include-log-softmax=true dim=2 objective-scale=$speech_scale input=tdnn4 presoftmax-scale-file=$dir/presoftmax_prior_scale_output-speech.txt learning-rate-factor=0.05

  output-layer name=output-snr include-log-softmax=false dim=$num_snr_bins objective-type=quadratic objective-scale=`perl -e "print $speech_scale / $num_snr_bins"` input=tdnn4 max-change=0.75 learning-rate-factor=0.5

  output-layer name=output-overlapped_speech include-log-softmax=true dim=2 objective-scale=$ovlp_scale input=tdnn4 presoftmax-scale-file=$dir/presoftmax_prior_scale_output-overlapped_speech.txt max-change=0.75 learning-rate-factor=0.025
EOF
