...
if [ -z "$dir" ]; then
  dir=exp/nnet3_stats_ovlp/nnet_lstm
fi
...
if [ $stage -le 1 ]; then
  ...
  relu-renorm-layer name=tdnn1 dim=256 input=Append(input@-2, input@-1, input, input@1, input@2)
  lstmp-layer name=lstm1 cell-dim=256 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=-3
  relu-renorm-layer name=tdnn2 input=Append(-6,0,6) dim=256
  lstmp-layer name=lstm2 cell-dim=256 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=-6
  
  output-layer name=output-overlapped_speech include-log-softmax=true dim=2 input=lstm2 presoftmax-scale-file=$dir/presoftmax_prior_scale_output-overlapped_speech.txt max-change=0.75 learning-rate-factor=0.02
EOF
