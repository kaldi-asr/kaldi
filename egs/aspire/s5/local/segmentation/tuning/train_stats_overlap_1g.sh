...
# This scripts is similar to 1f but adds max-change=0.75 and learning-rate-factor=0.1 to the final affine.
...
if [ $stage -le 1 ]; then
  ...
  output-layer name=output-overlapped_speech include-log-softmax=true dim=2 input=tdnn4 presoftmax-scale-file=$dir/presoftmax_prior_scale_output-overlapped_speech.txt max-change=0.75 learning-rate-factor=0.1
EOF
