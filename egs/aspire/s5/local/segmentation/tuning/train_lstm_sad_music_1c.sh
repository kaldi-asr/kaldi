...
print ( ($speech_scale / $avg_scale) ** 0.25.  " " . ($music_scale / $avg_scale) ** 0.25 )' $num_frames_sad $num_frames_music`
...
if [ $stage -le 1 ]; then
  ...
  output-layer name=output-snr include-log-softmax=false dim=$num_snr_bins objective-type=quadratic learning-rate-factor=0.1 objective-scale=`perl -e "print $speech_scale / $num_snr_bins"` input=tdnn3
