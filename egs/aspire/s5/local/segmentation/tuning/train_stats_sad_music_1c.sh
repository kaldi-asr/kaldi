...
if [ $stage -le 3 ]; then
  ...
  relu-renorm-layer name=tdnn2 input=Append(tdnn1@-9, tdnn1@-3, tdnn1, tdnn1@3, tdnn2_stats) dim=256
  stats-layer name=tdnn3_stats config=mean+count(-108:9:27:108)
  relu-renorm-layer name=tdnn3 input=Append(tdnn2@-27, tdnn2@-9, tdnn2, tdnn2@9, tdnn3_stats) dim=256
  relu-renorm-layer name=tdnn4 dim=256

  output-layer name=output-speech include-log-softmax=true dim=2 input=tdnn4 
