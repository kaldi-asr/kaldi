...
ali_dir=/export/a09/vmanoha1/workspace_asr_diarization/egs/ami/s5b/exp/ihm/nnet3_cleaned/tdnn_sp_ali_train_ihmdata_oraclespk
...
dir=exp/sad_ami_sdm1_train/ref
...
if [ $stage -le 1 ]; then
  ( 
  cd $src_dir
  # local/prepare_parallel_train_data.sh --train-set train sdm1

  awk '{print $1" "$2}' $src_dir/data/ihm/train/segments > \
    $src_dir/data/ihm/train/utt2reco
  awk '{print $1" "$2}' $src_dir/data/sdm1/train/segments > \
    $src_dir/data/sdm1/train/utt2reco

  cat $src_dir/data/sdm1/train_ihmdata/ihmutt2utt | \
    utils/apply_map.pl -f 1 $src_dir/data/ihm/train/utt2reco | \
    utils/apply_map.pl -f 2 $src_dir/data/sdm1/train/utt2reco | \
    sort -u > $src_dir/data/sdm1/train_ihmdata/ihm2sdm_reco
  )
fi
...
if [ $stage -le 2 ]; then
  ...
  utils/copy_data_dir.sh $src_dir/data/sdm1/train_ihmdata \
    $src_dir/data/sdm1/train_ihmdata_oraclespk
 
  cat $src_dir/data/ihm/train/utt2spk | \
    utils/apply_map.pl -f 1 $src_dir/data/sdm1/train_ihmdata/ihmutt2utt > \
    $src_dir/data/sdm1/train_ihmdata_oraclespk/utt2spk.temp
    
  cat $src_dir/data/sdm1/train_ihmdata_oraclespk/utt2spk.temp | \
    awk '{print $1" "$2"-"$1}' > \
    $src_dir/data/sdm1/train_ihmdata_oraclespk/utt2newutt
  
  utils/apply_map.pl -f 1 $src_dir/data/sdm1/train_ihmdata_oraclespk/utt2newutt \
    < $src_dir/data/sdm1/train_ihmdata_oraclespk/utt2spk.temp > \
    $src_dir/data/sdm1/train_ihmdata_oraclespk/utt2spk
  
  for f in feats.scp segments text; do
    utils/apply_map.pl -f 1 $src_dir/data/sdm1/train_ihmdata_oraclespk/utt2newutt \
      < $src_dir/data/sdm1/train_ihmdata/$f > \
      $src_dir/data/sdm1/train_ihmdata_oraclespk/$f
  done
      
  rm $src_dir/data/sdm1/train_ihmdata_oraclespk/{spk2utt,cmvn.scp}
...
if [ -z $ali_dir ]; then
  if [ $stage -le 3 ]; then
    ...
    steps/align_fmllr.sh --nj 18 --cmd "$train_cmd" \
      data/sdm1/train_ihmdata_oraclespk data/lang \
      exp/ihm/tri3_cleaned \
      exp/sdm1/tri3_cleaned_train_ihmdata_oraclespk
    )
  fi
  ali_dir=exp/sdm1/tri3_cleaned_ali_train_ihmdata_oraclespk
fi
...
cat $src_dir/data/sdm1/train/reco2file_and_channel | \
...
# single speaker.
cat $src_dir/data/sdm1/train_ihmdata_oraclespk/reco2utt | \
  awk 'BEGIN{i=1} {print $1" "1":"i; i++;}' > \
  $src_dir/data/sdm1/train_ihmdata_oraclespk/reco.txt 
if [ $stage -le 5 ]; then
  utils/data/get_reco2num_frames.sh --frame-shift 0.01 --frame-overlap 0.015 \
    --cmd "$train_cmd" --nj 18 \
    $src_dir/data/sdm1/train
  ...
  $train_cmd $dir/log/interior_regions.log \
    segmentation-init-from-segments --shift-to-zero=false --frame-overlap=0.0 $src_dir/data/sdm1/train/segments ark:- \| \
    segmentation-combine-segments-to-recordings ark:- ark,t:$src_dir/data/sdm1/train/reco2utt ark:- \| \
    segmentation-create-subsegments --filter-label=1 --subsegment-label=1 \
    "ark:segmentation-init-from-lengths --label=0 ark,t:$src_dir/data/sdm1/train/reco2num_frames ark:- |" ark:- ark,t:- \| \
...
if [ $stage -le 6 ]; then
  ...
  $train_cmd $dir/log/get_ref_spk_seg.log \
    ...
    "ark:segmentation-init-from-segments --shift-to-zero=false --frame-overlap=0.0 $src_dir/data/sdm1/train_ihmdata_oraclespk/segments ark:- |" \
    ark,t:$src_dir/data/sdm1/train_ihmdata_oraclespk/reco2utt ark:- \| \
    segmentation-copy --keep-label=1 ark:- ark:- \| \
    segmentation-copy --utt2label-map-rspecifier=ark,t:$src_dir/data/sdm1/train_ihmdata/reco.txt \
    ark:- ark:- \| \
    segmentation-merge-recordings \
    "ark,t:utils/utt2spk_to_spk2utt.pl $src_dir/data/sdm1/train_ihmdata/ihm2sdm_reco |" \
...
if [ $stage -le 7 ]; then
  ...
  $train_cmd $dir/log/get_ref_spk_rttm_manual_seg.log \
    ...
    segmentation-post-process --merge-labels=0 --merge-dst-label=10000 ark:- ark:- \| \
    ...
    --map-to-speech-and-sil=false --no-score-label=10000 ark:- - \| \
  ...
  $train_cmd $dir/log/get_ref_rttm_manual_seg.log \
    export PATH=$KALDI_ROOT/tools/sctk/bin:$PATH '&&' \
    segmentation-get-stats --lengths-rspecifier=ark,t:$src_dir/data/sdm1/train/reco2num_frames \
...
hyp_dir=${nnet_dir}/segmentation_ovlp_ami_sdm1_train_whole_bp/ami_sdm1_train
...
if [ $stage -le 12 ]; then
  steps/segmentation/do_segmentation_data_dir_generic.sh --reco-nj 18 \
    ...
    $src_dir/data/sdm1/train $nnet_dir mfcc_hires_bp $hyp_dir
fi

likes_dir=${nnet_dir}/ovlp_sad_ami_sdm1_train_whole_bp/

hyp_dir=${hyp_dir}_seg
mkdir -p $hyp_dir

seg_dir=${nnet_dir}/segmentation_ovlp_sad_ami_sdm1_train_whole_bp/
lang=${seg_dir}/lang
...
#md-eval.pl -s <( segmentation-init-from-segments --shift-to-zero=false exp/nnet3_sad_snr/nnet_tdnn_j_n4/segmentation_ami_sdm1_train_whole_bp/ami_sdm1_train_seg/segments ark:- | segmentation-combine-segments-to-recordings ark:- ark,t:exp/nnet3_sad_snr/nnet_tdnn_j_n4/segmentation_ami_sdm1_train_whole_bp/ami_sdm1_train_seg/reco2utt ark:- | segmentation-to-ali --length-tolerance=1000 --lengths-rspecifier=ark,t:data/ami_sdm1_train_whole_bp_hires/utt2num_frames ark:- ark:- |
