...
if [ $stage -le 3 ]; then
  local/segmentation/make_sad_tdnn_configs.py \
    ...
    --output-node-parameters "--output-suffix=overlapped_speech_logprob --dim=1 --add-final-sigmoid=false --include-log-softmax=false --objective-type=quadratic" \
...
if [ -z "$egs_dir" ]; then
  ...
  if [ $stage -le 4 ]; then
    ...
    steps/nnet3/get_egs.py --cmd="$decode_cmd" \
      ...
      --stage=$get_egs_stage \
      --targets-parameters="--output-name=output-snr --target-type=dense --targets-scp=$snr_scp --deriv-weights-scp=$deriv_weights_scp --compress=true" \
      ...
      --targets-parameters="--output-name=output-overlapped_speech_logprob --target-type=dense --dim=1 --targets-scp=$overlapped_speech_logprob_scp --deriv-weights-scp=$deriv_weights_for_overlapped_speech_scp --compress=true" \
      --targets-parameters="--output-name=output-overlapped_speech --target-type=sparse --dim=2 --targets-scp=$overlapped_speech_labels_scp --deriv-weights-scp=$deriv_weights_for_overlapped_speech_scp --scp2ark-cmd=\"ali-to-post scp:- ark:- |\" --compress=true" \
      --dir=$dir/egs
  fi
fi
