            ...
            if line.split()[0] == recording_names[recording_index]:
                ...
                while not processed_line:
                    if recording_index >= len(recording_names):
                        raise Exception(
                            "There is a mismatch between the recording_names_file and the ctm file. There are recordings in ctm file which are not present in the recording file."
                        )
                    if line.split()[0] == recording_names[recording_index]:
                        outfile.write(line)
                        processed_line = True
                        ...
                        outfile.write(
                            "{0} 1 0.00  0.01 NOTHINGWASDECODEDHERE\n".format(
                                recording_names[recording_index]
                            )
                        )
                        recording_index += 1
        infile.close()
...
if __name__ == "__main__":
  ...
  that the ctm file has recordings in the same order as the wav.scp file"""
    ...
    try:
        file_names = [
            x.strip()
            for x in open("{0}".format(params.recording_name_file)).readlines()
        ]
    except IOError:
