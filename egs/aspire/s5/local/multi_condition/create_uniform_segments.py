...
def segment(total_length, window_length, overlap=0):
    ...
    segments = [
        (x * increment, min(total_length, (x * increment) + window_length))
...
def get_wave_segments(wav_command, window_length, overlap):
    raw_output = subprocess.check_output(
        wav_command + " sox -t wav - -n stat 2>&1 | grep Length ", shell=True
    )
...
def prepare_segments_file(kaldi_data_dir, window_length, overlap):
    ...
    for i in range(0, len(ids)):
        ...
        for segment in segments:
            segment_string = "{0}-{1:06}-{2:06} {0} {3} {4}".format(
                ids[i],
                int(segment[0] * 1000),
                int(segment[1] * 1000),
                segment[0],
...
if __name__ == "__main__":
  ...
  given the kaldi data directory."""
    ...
    parser.add_argument(
        ...
        type=float,
        default=30.0,
        help="length of the window used to cut the segment",
    )
    parser.add_argument(
        "--overlap", type=float, default=5.0, help="overlap of neighboring windows"
    )
    ...
    segments, segments_per_recording = prepare_segments_file(
