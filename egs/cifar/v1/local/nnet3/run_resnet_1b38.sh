...
# 1b38 is as 1b37 but not putting the residual connection through
# the relu and batch-norm.
# A bit worse.

#local/nnet3/compare.sh exp/resnet1b{37,38}_cifar10
# local/nnet3/compare.sh exp/resnet1b37_cifar10 exp/resnet1b38_cifar10
# System                resnet1b37_cifar10 resnet1b38_cifar10
# final test accuracy:       0.9094      0.9052
# final train accuracy:       0.9984      0.9928
# final test objf:         -0.358765   -0.353428
# final train objf:       -0.0114319  -0.0270446
# num-parameters:            936714      936714


# 1b37 is as 1b32 but adding 4 more residual layers- at the
# early stages, so as not to increase the params too much.
...
if [ $stage -le 1 ]; then
  ...
  res-block name=res4 num-filters=$nf1 height=32 time-period=1 $res_opts
  res-block name=res5 num-filters=$nf1 height=32 time-period=1 $res_opts
  conv-layer name=conv6 height-in=32 height-out=16 height-subsample-out=2 time-offsets=-1,0,1 $common num-filters-out=$nf2
  res-block name=res7 num-filters=$nf2 height=16 time-period=2 $res_opts
  res-block name=res8 num-filters=$nf2 height=16 time-period=2 $res_opts
  res-block name=res9 num-filters=$nf2 height=16 time-period=2 $res_opts
  res-block name=res10 num-filters=$nf2 height=16 time-period=2 $res_opts
  conv-layer name=conv11 height-in=16 height-out=8 height-subsample-out=2 time-offsets=-2,0,2 $common num-filters-out=$nf3
  res-block name=res12 num-filters=$nf3 num-bottleneck-filters=$nb3 height=8 time-period=4 $res_opts
  res-block name=res13 num-filters=$nf3 num-bottleneck-filters=$nb3 height=8 time-period=4 $res_opts
  res-block name=res14 num-filters=$nf3 num-bottleneck-filters=$nb3 height=8 time-period=4 $res_opts
