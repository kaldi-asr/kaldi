...
if [ $stage -le 1 ]; then
  ...
  conv-layer name=conv4 height-in=32 height-out=16 height-subsample-out=2 time-offsets=-1,0,1 $common num-filters-out=$nf2 use-spatial-averages=true
  res-block name=res5 num-filters=$nf2 height=16 time-period=2 $res_opts use-spatial-averages=true
  res-block name=res6 num-filters=$nf2 height=16 time-period=2 $res_opts use-spatial-averages=true
  conv-layer name=conv7 height-in=16 height-out=8 height-subsample-out=2 time-offsets=-2,0,2 $common num-filters-out=$nf3 use-spatial-averages=true
