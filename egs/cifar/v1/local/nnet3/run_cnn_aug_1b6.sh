...
# aug_1b6 is as aug_1b5 but limiting the spatial averaging to just
# the first layer.
#   makes little difference: train 0.9084->0.9128, test 0.8637->0.8613.
# exp/cnn_aug_1b6_cifar10: num-iters=60 nj=1..2 num-params=1.7M dim=96->10 combine=-0.37->-0.35 loglike:train/valid[39,59,final]=(-0.33,-0.25,-0.26/-0.45,-0.40,-0.40) accuracy:train/valid[39,59,final]=(0.89,0.91,0.91/0.85,0.86,0.86)
...
if [ $stage -le 1 ]; then
  ...
  conv-relu-batchnorm-layer name=cnn1 height-in=32 height-out=32 time-offsets=-1,0,1 use-spatial-averages=true $common1
