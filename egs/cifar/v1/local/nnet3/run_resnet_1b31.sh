...
# and a correspondingly different configuration.  We're doing better on train, but
# it doesn't seem to affect validation performance substantially (a little worse).

# local/nnet3/compare.sh exp/resnet1b30_cifar10 exp/resnet1b31_cifar10
# System                resnet1b30_cifar10 resnet1b31_cifar10
# final test accuracy:       0.9093      0.9075
# final train accuracy:       0.9968      0.9976
# final test objf:         -0.355272   -0.383256
# final train objf:       -0.0166483  -0.0102666
# num-parameters:            752010      752010

# local/nnet3/compare.sh exp/resnet1b30_cifar100 exp/resnet1b31_cifar100
# System                resnet1b30_cifar100 resnet1b31_cifar100
# final test accuracy:       0.6585      0.6581
# final train accuracy:       0.9182      0.9388
# final test objf:          -1.34408    -1.37444
# final train objf:        -0.293492   -0.232596
...
if [ $stage -le 1 ]; then
  ...
  conv-layer name=conv1 height-in=40 height-out=38 time-offsets=-1,0,1 $common num-filters-out=$nf1
  res-block name=res2 num-filters=$nf1 height=38 time-period=1 $res_opts
  res-block name=res3 num-filters=$nf1 height=38 time-period=1 $res_opts
  conv-layer name=conv4 height-in=38 height-out=18 height-subsample-out=2 time-offsets=-1,0,1 $common num-filters-out=$nf2
  res-block name=res5 num-filters=$nf2 height=18 time-period=2 $res_opts
  res-block name=res6 num-filters=$nf2 height=18 time-period=2 $res_opts
  conv-layer name=conv7 height-in=18 height-out=8 height-subsample-out=2 time-offsets=-2,0,2 $common num-filters-out=$nf3
  ...
  channel-average-layer name=channel-average input=Append(6,10,14,18,22,24,28,32) dim=$nf3
