...
# 1b23 is as 1b19 but changing to a setup with bottleneck res-blocks,
# in order to test that branch of the code.
...
if [ $stage -le 1 ]; then
  ...
  res-block name=res5 num-filters=$nf2 num-bottleneck-filters=$nb2 height=16 time-period=2 $res_opts
  res-block name=res6 num-filters=$nf2 num-bottleneck-filters=$nb2 height=16 time-period=2 $res_opts
  ...
  relu-batchnorm-layer name=fully_connected1 input=Sum(2,6,10,14,18,22,24,28) dim=96
