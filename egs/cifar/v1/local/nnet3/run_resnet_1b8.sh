...
# does not seem to help: test acc changes from 0.9111 -> 0.9087.
...
if [ $stage -le 1 ]; then
  ...
  conv-layer name=conv9 height-in=16 height-out=8 height-subsample-out=2 time-offsets=-2,0,2 $common num-filters-out=$nf3
  res-block name=res10 num-filters=$nf3 height=8 time-period=4 $res_opts
  res-block name=res11 num-filters=$nf3 height=8 time-period=4 $res_opts
