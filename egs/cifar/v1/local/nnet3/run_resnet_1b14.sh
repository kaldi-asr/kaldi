...
# 1b14 is as 1b13 but changing the non-subsampling conv layer between
# res-blocks to be 1x1.
#    It's slightly worse: 0.9119 -> 0.9097

# 1b13 is as 1b7 (also see 1b12), but having 4 types of
# res-block instead of 3, starting with 32, then going to
# 48 without any sub-sampling.
...
if [ $stage -le 1 ]; then
  ...
  conv-layer name=conv1 height-in=32 height-out=32 time-offsets=-1,0,1 $common num-filters-out=$nf1
  ...
  conv-layer name=conv4 height-in=32 height-out=32 time-offsets=0 height-offsets=0 num-filters-out=$nf2
  res-block name=res5 num-filters=$nf2 height=32 time-period=1 $res_opts
  res-block name=res6 num-filters=$nf2 height=32 time-period=1 $res_opts
  conv-layer name=conv7 height-in=32 height-out=16 height-subsample-out=2 time-offsets=-1,0,1 $common num-filters-out=$nf3
  res-block name=res8 num-filters=$nf3 height=16 time-period=2 $res_opts
  res-block name=res9 num-filters=$nf3 height=16 time-period=2 $res_opts
  conv-layer name=conv10 height-in=16 height-out=8 height-subsample-out=2 time-offsets=-2,0,2 $common num-filters-out=$nf4
  res-block name=res11 num-filters=$nf4 height=8 time-period=4 $res_opts
  res-block name=res12 num-filters=$nf4 height=8 time-period=4 $res_opts
  relu-batchnorm-layer name=fully_connected1 input=Sum(2,6,10,14,18,22,24,28) dim=$nf4
