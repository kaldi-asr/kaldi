...
# 1b3 is as 1b2 but 2x smaller final learning rate

# It seems to make it worse, if anything:
#  test accuracy changes from 0.9026 -> 0.8948.
#  training accuracy changes 0.9948 -> 0.999, so it trains more completely- interesting.

# grep acc exp/cnn_aug_1b3_cifar10/log/compute_prob_valid.final.log
# LOG (nnet3-compute-prob[5.1]:PrintTotalStats():nnet-diagnostics.cc:165) Overall accuracy for 'output' is 0.8566 per frame, over 10000 frames.
