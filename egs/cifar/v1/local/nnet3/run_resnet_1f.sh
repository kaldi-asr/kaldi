...
# 1f is as 1e but removing the last convolutional layer.
# test accuracy improves slightly from 0.9085->0.9112, train 0.993->0.9946.

# 1e is as 1d4 but reducing the number of filters for some layers.
#
...
if [ $stage -le 1 ]; then
  ...
  res_opts="direct-lr-factor=0.1 direct-convolution-source=batchnorm"
  ...
  relu-batchnorm-layer name=fully_connected1 input=Sum(2,6,10,14,18,22,24,28) dim=256
