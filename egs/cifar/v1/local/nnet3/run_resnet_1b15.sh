...
# 1b15 is as 1b12 but adding a new res-block at the start, which
#   was helpful in 1b7->1b9.  Also changing from using 'egs' to
#  'egs2', which made no difference in 1b->1b11, and is faster
#
#  It's a little worse than 1b12: 0.9104, vs. 0.9111 in 1b12.

# 1b12 is as 1b7 but increasing the dimension of the initial blocks from 32
# to 48.
...
if [ $stage -le 1 ]; then
  ...
  conv-layer name=conv5 height-in=32 height-out=16 height-subsample-out=2 time-offsets=-1,0,1 $common num-filters-out=$nf2
  ...
  conv-layer name=conv8 height-in=16 height-out=8 height-subsample-out=2 time-offsets=-2,0,2 $common num-filters-out=$nf3
  res-block name=res9 num-filters=$nf3 height=8 time-period=4 $res_opts
  res-block name=res10 num-filters=$nf3 height=8 time-period=4 $res_opts
