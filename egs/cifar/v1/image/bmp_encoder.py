...
def bmp_encoder(data, width, height):
    ...
    bmp_header[3] = struct.pack(
        "<I", 0
    )  # 4 bytes, This data is reserved but can just be set to 0
    bmp_header[4] = struct.pack(
        "<I", header_size
    )  # 4 bytes, Pixel offset, 54 bytes here
    ...
    bmp_header[8] = struct.pack(
        "<H", 1
    )  # 2 bytes, The number of color planes, must be set to 1
    ...
    bmp_header[11] = struct.pack(
        "<I", 0
    )  # 4 bytes, Size of raw pixel data, fill in later
    bmp_header[12] = struct.pack(
        "<I", 2835
    )  # 4 bytes, horizontal resolution. Just leave it at 2835
    bmp_header[13] = struct.pack(
        "<I", 2835
    )  # 4 bytes, vertical resolution. Just leave it at 2835.
    bmp_header[14] = struct.pack(
        "<I", 0
    )  # 4 bytes, The number of colors, leave at 0 to default to all colors
    bmp_header[15] = struct.pack(
        "<I", 0
    )  # 4 bytes, The important colors, leave at 0 to default to all colors
    ...
    for i in range(height):
        ...
        while len(bmp_body) % 4 != 0:
            bmp_body.append(
                struct.pack("<B", 0)
            )  # pad each row with bytes at the end till its length is a multiple of 4
