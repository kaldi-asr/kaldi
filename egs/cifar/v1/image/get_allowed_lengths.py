...
def get_args():
    parser = argparse.ArgumentParser(
        description="""This script finds a set of
...
def find_allowed_durations(start_len, end_len, args):
    ...
    length = start_len
    with open(
        os.path.join(args.srcdir, "allowed_lengths.txt"), "w", encoding="latin-1"
    ) as fp:
...
def main():
    ...
    logger.info(
        "Lengths in the range [{},{}] will be covered. "
    ...
    logger.info(
        ...
        "for the images.".format(
            int((math.log(float(end_dur) / start_dur)) / math.log(args.factor))
        )
    )
