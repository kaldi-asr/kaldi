...
# This is a modified version of egs/sre08/v1/sid/train_diag_ubm.sh that has
# been specialized for images.  Instead of MFCCs, features are n by m patches
# extracted from the image, and passed through a PCA transform.
# We initialize the GMM using gmm-global-init-from-feats, which sets the means
# to random data points and then does some iterations of E-M in memory.
...
for f in $data/images.scp $dir/final.mat; do
   [ ! -f $f ] && echo "$0: expecting file $f to exist" && exit 1
done

parallel_opts="--num-threads $num_threads"
patch_opts=$(cat $dir/patch_opts)
...
all_feats="ark,s,cs:extract-patches $patch_opts scp:$data/images.scp ark:- | transform-feats $dir/final.mat ark:- ark:- |"

feats="ark,s,cs:extract-patches $patch_opts scp:$sdata/JOB/images.scp ark:- | transform-feats $dir/final.mat ark:- ark:- | subsample-feats --n=$subsample ark:- ark:- |"
