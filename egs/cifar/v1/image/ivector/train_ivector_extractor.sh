...
if [ $# != 3 ]; then
  ...
  echo " e.g.: $0 exp/ubm_2048/final.ubm data/train exp/extractor"
...
for f in $fgmm_model $data/images.scp $srcdir/final.mat $srcdir/patch_opts; do
...
image/split_image_dir.sh $data $nj_full || exit 1;

parallel_opts="--num-threads $[$num_threads*$num_processes]"
## Set up features.
feats="ark,s,cs:extract-patches $patch_opts scp:$sdata/JOB/images.scp ark:- | transform-feats $dir/final.mat ark:- ark:- |"
