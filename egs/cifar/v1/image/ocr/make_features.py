...
def horizontal_pad(im, allowed_lengths=None):
    ...
    if args.num_channels in [1, 4]:
        ...
        im_pad = np.concatenate(
            (255 * np.ones((dim_y, left_padding, args.num_channels), dtype=int), im),
            axis=1,
        )
        im_pad1 = np.concatenate(
            (
                im_pad,
                255 * np.ones((dim_y, right_padding, args.num_channels), dtype=int),
            ),
            axis=1,
        )
    return im_pad1
...
def vertical_shift(im, mode="normal"):
    ...
    im_pad = np.concatenate(
        (
            ...
            - np.random.normal(2, 1, (top, width)).astype(int),
            im,
        ),
        axis=0,
    )
    im_pad = np.concatenate(
        (
            im_pad,
            255 * np.ones((bottom, width), dtype=int)
            - np.random.normal(2, 1, (bottom, width)).astype(int),
        ),
        axis=0,
    )
    return im_pad
