...
def main():
    parser = ArgumentParser(
        description="Convert kaldi data directory to uem dat files",
    ...
    parser.add_argument("--get-text", action="store_true", help="Get text in dat file")
    parser.add_argument(
        "--prefix", type=str, help="Add db file name as db-<prefix>-{utt/spk}.dat"
    )
    parser.add_argument("kaldi_dir", help="Kaldi data directory")
    parser.add_argument("output_dir", help="Directory to store uem dat files")
    parser.usage = (
    ...
    for line in scp_file.readlines():
        ...
        if not m:
            sys.stderr.write(
                "%s does not contain a valid speech file (.wav or .sph)\n"
                % line.strip()
            )
            sys.exit(1)
    ...
    try:
        ...
        repr(e)
        sys.stderr.write(
            "%s: Could not write dat files in %s\n" % (sys.argv[0], options.output_dir)
        )
        sys.exit(1)
    ...
    for line in segments_file.readlines():
        ...
        utt_dat.write(
            "{UTTID %s} {UTT %s} {SPK %s} {FROM %s} {TO %s} {TEXT %s}\n"
    ...
    for spk, utts in list(spk2utt_map.items()):
        try:
            spk_dat.write(
                "{SEGS %s} {ADC %s} {CONV %s.wav} {CHANNEL 1} {DUR }\n"
