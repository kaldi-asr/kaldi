...
def KaldiLauncher(lo, **kwargs):
    ...
    num_failed = 0
    time.sleep(1)
    # Lets wait for a while to give the shared fs time to sync
    error_pending = True
    for logfile in logfiles:
        ...
        for delay in sched_rate:
            ...
            if len(with_status) == 0:
                sys.stderr.write(
                    "The last line(s) of the log-file " + logfile + " does not seem"
                    " to indicate return status as expected\n"
                )
            elif len(with_status) > 1:
                sys.stderr.write(
                    "The last line(s) of the log-file " + logfile + " does seem"
...
def CmdLineParser(argv):
    ...
    if jobstart > jobend:
        sys.stderr.write(
            "lonestar.py: JOBSTART("
            + str(jobstart)
            + ") must be lower than JOBEND("
            + str(jobend)
            + ")\n"
        )
        sys.exit(1)
...
def setup_paths_and_vars(opts):
    ...
    if (
        opts.varname
        and (opts.varname not in opts.logfile)
        and (opts.jobstart != opts.jobend)
    ):
        print(
            "lonestar.py: you are trying to run a parallel job"
    ...
    if taskname == "":
        print(
            "lonestar.py: you specified the log file name in such form "
...
def create_scriptfile(scriptname, opts):
    ...
    f.write(
        "echo '#' Accounting time=$(($time2 - $time1)) threads="
        + str(nof_threads)
        + " >> "
        + logfile
        + "\n"
