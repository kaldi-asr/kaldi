                ...
                file_handle.write(
                    ...
                    % (
                        ...
                        str(
                            sorted(
                                [
                                    str(self.markers[j][i])
                                    + " ("
                                    + str(self.state_count[j][i])
                                    + ")"
...
class JointResegmenter(object):
    def __init__(self, P, A, f, options, phone_map, stats=None, reference=None):
        ...
        self.hard_max_frames = int(
            options.hard_max_segment_length / options.frame_shift
        )
        self.min_inter_utt_nonspeech_length = int(
            options.min_inter_utt_silence_length, options.frame_shift
        )
    ...
    def resegment(self):
        ...
        if self.options.verbose > 1:
            sys.stderr.write(
                "For %s: get_initial_segments took %f sec\n"
        ...
        if self.options.verbose > 1:
            sys.stderr.write(
                "For %s: set_nonspeech_proportion took %f sec\n"
                % (self.file_id, t.interval)
            )
        with Timer() as t:
            self.merge_segments()
    ...
    def get_initial_segments(self):
        ...
        C = self.C
        a = Analysis(
            self.file_id, self.frame_shift, "Analysis after get_initial_segments"
        )
    ...
    def set_nonspeech_proportion(self):
        ...
        if num_speech_frames == 0:
            sys.stderr.write(
                "%s: Warning: no speech found for recording %s\n"
        ...
        target_segment_frames = int(
            num_speech_frames / (1.0 - self.options.silence_proportion)
        )
        ...
        if num_segment_frames < target_segment_frames:
            proportion = (
                float(num_segment_frames - num_speech_frames) / num_segment_frames
            )
            sys.stderr.write(
        ...
        C = self.C
        a = Analysis(
            self.file_id, self.frame_shift, "Analysis after set_nonspeech_proportion"
        )
    ...
    def merge_segments(self):
        ...
        while i < len(segment_starts) and j < len(segment_ends):
            if segment_ends[j] < segment_starts[i]:
                ...
                segment_score = min(
                    segment_starts[i] - segment_starts[i - 1],
        ...
        for b in boundaries:
            ...
            while p <= self.N:
                ...
                p += 1
            assert (
                self.min_inter_utt_nonspeech_length == 0
                or p == self.N
                or self.S[p]
                or self.A[p] in self.THIS_SILENCE_OR_NOISE
            )

            if (
                self.min_inter_utt_nonspeech_length > 0
                and self.A[b[0]] in self.THIS_SILENCE_OR_NOISE
            ):
        ...
        for n in range(0, self.N):
            if self.S[n]:
                ...
                if segment_length > self.hard_max_frames:
                    ...
                    sys.stderr.write(
                        "%s: Warning: for recording %s, " % (sys.argv[0], self.file_id)
    ...
    def remove_silence_only_segments(self):
        for n in range(0, self.N):
            ...
            if self.S[n]:
                ...
                # End if
        if self.reference != None and self.options.verbose > 3:
            a = self.segmentation_analysis(
                "Analysis after remove_silence_only_segments"
            )
            a.write_confusion_matrix()
    ...
    def transition_type(self, j):
        ...
        ) and self.A[j] in (self.THIS_SPEECH_THAT_NOISE + self.THIS_SPEECH_THAT_SIL):
            return 0
        if self.A[j - 1] in self.THIS_SPEECH and self.A[j] in self.THIS_SPEECH:
            return 1
        if self.A[j - 1] in (
            self.THIS_SPEECH
            + self.THIS_NOISE_CONVERT_THAT_SIL
            ...
            return 2
        if self.A[j - 1] in (self.THIS_SPEECH + self.THIS_NOISE_CONVERT) and self.A[
            j
        ] in (self.THIS_SPEECH + self.THIS_NOISE_CONVERT):
            return 3
        if self.A[j - 1] in (
            self.THIS_SPEECH
            + self.THIS_NOISE_CONVERT
            ...
            return 4
        if self.A[j - 1] in (self.THIS_SPEECH + self.THIS_CONVERT) and self.A[j] in (
        ...
        if self.A[j - 1] in self.THIS_SPEECH_PLUS and self.A[j] in (
            ...
            return 7
        if (
            self.A[j - 1] in (self.THIS_SPEECH_PLUS + self.THIS_NOISE)
            and self.A[j] in self.THIS_SPEECH_PLUS
        ):
            return 8
        if (
            self.A[j - 1] in (self.THIS_SPEECH_PLUS + self.THIS_SILENCE)
            and self.A[j] in self.THIS_SPEECH_PLUS
        ):
            return 9
        assert False
    ...
    def print_segments(self, out_file_handle=sys.stdout):
        ...
        if len(segments) == 0:
            sys.stderr.write(
                "%s: Warning: no segments for recording %s\n"
                % (sys.argv[0], self.file_id)
            )
            sys.exit(1)
        ...
        for start, end in segments:
            ...
            utterance_id = "%s%s%s%s%s" % (
                self.file_id,
                self.options.first_separator,
                start_str,
                self.options.second_separator,
                end_str,
            )
            # Output:
        ...
        for x in A1:
            if prev_x == None or x == prev_x:
                ...
                if phone_map[prev_x] == "0":
                    B.extend(["0"] * len_x)
                elif (speech_cap != None and len_x > speech_cap) or phone_map[
                    prev_x
                ] == "1":
                    B.extend(["4"] * len_x)
...
def main():
    ...
    parser.usage = (
        ":".join(parser.format_usage().split(":")[1:])
    ...
    if not (options.silence_proportion > 0.01 and options.silence_proportion < 0.99):
        sys.stderr.write(
            "%s: Error: Invalid silence-proportion value %f\n"
            % options.silence_proportion
        )
        sys.exit(1)

    if not (
        options.remove_noise_only_segments == "false"
    ...
    global_analysis_set_nonspeech_proportion = Analysis(
        "TOTAL_set_nonspeech_proportion",
        options.frame_shift,
    ...
    for f in pred_files:
        ...
        if re.match(".*_" + channel1_file, f) is None:
            if re.match(".*_" + channel2_file, f) is None:
                sys.stderr.write(
                    "%s does not match pattern .*_%s or .*_%s\n"
        ...
        if (
            options.isolated_resegmentation
            or f2 not in pred_files
            or f1 not in pred_files
        ):
            pred_files[f] = True
            try:
                A = (
                    open(os.path.join(prediction_dir, f + ".pred"))
                    .readline()
                    .strip()
                    .split()[1:]
                )
            ...
            if temp_dir != None:
                try:
                    reference = (
                        open(os.path.join(temp_dir, f + ".ref"))
                        .readline()
                        .strip()
                        .split()[1:]
                    )
            ...
            try:
                A1 = (
                    open(os.path.join(prediction_dir, f1 + ".pred"))
                    .readline()
                    .strip()
                    .split()[1:]
                )
            ...
            try:
                A2 = (
                    open(os.path.join(prediction_dir, f2 + ".pred"))
                    .readline()
                    .strip()
                    .split()[1:]
                )
            ...
            if temp_dir != None:
                try:
                    reference1 = (
                        open(os.path.join(temp_dir, f1 + ".ref"))
                        .readline()
                        .strip()
                        .split()[1:]
                    )
            ...
            if temp_dir != None:
                try:
                    reference2 = (
                        open(os.path.join(temp_dir, f2 + ".ref"))
                        .readline()
                        .strip()
                        .split()[1:]
                    )
