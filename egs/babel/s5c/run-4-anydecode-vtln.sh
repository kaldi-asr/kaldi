...
# This scripts is like the run-4-anydecode.sh but has additional stage of  
# decoding vtln model in tri3c, computing the best test speaker warps and 
# extracting features with the VTLN warps and decoding the rest of the models 
# using the vtln-warped features.
...
if [ ! -f  $dataset_dir/.done ] ; then
  ...
  if [ ! -f ${dataset_dir}/.plp.done ]; then
    ...
    echo ---------------------------------------------------------------------
    if $use_vtln; then
      make_plp ${dataset_dir} exp/make_plp/${dataset_id}_nopitch_novtln plp_nopitch_novtln
    else
...
if $use_vtln && [ ! -f $dataset_dir/.vtln.done ]; then
  if [ ! -f exp/tri3c/decode_${dataset_id}/.done ]; then
    utils/mkgraph.sh data/lang exp/tri3c exp/tri3c/graph || exit 1
    steps/decode_lvtln.sh --beam 10 --lattice-beam 4 --skip-scoring true \
      --logdet-scale $logdet_scale \
      --nj $my_nj --cmd "$decode_cmd" "${decode_extra_opts[@]}" \
      exp/tri3c/graph ${dataset_dir} exp/tri3c/decode_${dataset_id} || exit 1
    touch exp/tri3c/decode_${dataset_id}/.done
  fi

  mkdir -p ${dataset_dir}_nopitch_novtln
  cp -rT ${dataset_dir} ${dataset_dir}_nopitch_novtln
  cp exp/tri3c/decode_$dataset_id/final.warp ${dataset_dir}/spk2warp
  
  echo ---------------------------------------------------------------------
  echo "Preparing ${dataset_kind} parametrization files with VTLN in ${dataset_dir} on" `date`
  echo ---------------------------------------------------------------------
  use_pitch=$use_pitch_temp
  make_plp ${dataset_dir} exp/make_plp/${dataset_id}_vtln plp_vtln
