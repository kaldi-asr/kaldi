...
class BnfExpConfig(object):
    def __init__(self):
        ...
        self.finetune_train_data = (
            self.wdir + "train.pfile.gz"
        )  # finetune training data
        self.finetune_valid_data = (
            self.wdir + "valid.pfile.gz"
        )  # finetune validation data
        ...
        self.initial_momentum_epoch = (
            2  # for how many epochs do we use initial_momentum
        )
        ...
        self.lrate = LearningRateExpDecay(
            start_rate=0.04,  # starting learning rate
