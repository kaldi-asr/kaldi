...
# The difference between this script and run-1-main.sh is training VTLN model 
# tri3c, computing VTLN warps for the training speakers and extracting features with warps and 
...
decode_nj=$dev2h_nj
echo ---------------------------------------------------------------------
echo "Starting plp feature extraction for data/train in plp_nopitch_novtln on" `date`
echo ---------------------------------------------------------------------

if [ ! -f data/train/.plp.done ]; then
  steps/make_plp.sh --cmd "$train_cmd" --nj $train_nj data/train exp/make_plp/train_nopitch_novtln plp_nopitch_novtln
  utils/fix_data_dir.sh data/train
  steps/compute_cmvn_stats.sh data/train exp/make_plp/train_nopitch_novtln plp_nopitch_novtln
  utils/fix_data_dir.sh data/train
...
if [ ! -f data/train/.vtln.done ]; then
  steps/train_lvtln.sh --base-feat-type plp \
    --cmd "$train_cmd" --boost-silence $boost_sil \
    --lvtln-iters "2 4 6 8 10 12 14 16 20" --logdet-scale $logdet_scale --max-warp $max_warp \
    $numLeavesTri3 $numGaussTri3 data/train data/lang exp/tri3 exp/tri3c || exit 1 
  mkdir -p data/train_nopitch_novtln
  cp -rT data/train data/train_nopitch_novtln
  cp exp/tri3c/final.warp data/train/spk2warp

  if $use_pitch; then
    steps/make_plp_pitch.sh --cmd "$train_cmd" --nj $train_nj data/train exp/make_plp_pitch/train_vtln plp_vtln
  else
    steps/make_plp.sh --cmd "$train_cmd" --nj $train_nj data/train exp/make_plp/train_vtln plp_vtln
  fi
  utils/fix_data_dir.sh data/train
  steps/compute_cmvn_stats.sh data/train exp/make_plp/train_vtln plp_vtln
  utils/fix_data_dir.sh data/train
