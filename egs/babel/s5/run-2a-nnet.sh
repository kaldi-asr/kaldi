...
if [ ! -f exp/tri6_nnet/decode_dev2h/.done ]; then
  steps/decode_nnet_cpu.sh --cmd "$decode_cmd" --nj $decode_nj \
    ...
    exp/tri5/graph data/dev2h exp/tri6_nnet/decode 
...
if [ ! -f exp/combine_2/decode/.done ]; then
  for iter in 1 2 3 4; do
    local/score_combine.sh --cmd "queue.pl -l mem_free=2.0G,ram_free=1.0G" \
      data/dev2h data/lang exp/tri6_nnet/decode exp/sgmm5_mmi_b0.1/decode_fmllr_it$iter exp/combine_2/decode_it$iter
...
if [ ! -f exp/combine_3/decode/.done ]; then
  for iter in 1 2 3 4; do
    ...
    local/score_combine.sh --cmd "queue.pl -l mem_free=2.0G,ram_free=1.0G" \
      data/dev2h data/lang exp_BNF/sgmm7_mmi_b0.1/decode_fmllr_it$iter:10 \
