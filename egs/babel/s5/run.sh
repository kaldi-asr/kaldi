...
if [[ ! -f data/dev/wav.scp || data/dev/wav.scp -ot "$dev_data_dir" ]]; then
  echo ---------------------------------------------------------------------
...
if [[ ! -f data/dev/glm || data/dev/glm -ot "$glmFile" ]]; then
  echo ---------------------------------------------------------------------
...
decode_si() { 
  ...
  if [ ! -f $dir/decode/.done ]; then
    ...
    steps/decode.sh --nj $decode_nj --cmd "$decode_cmd" \
       ...
       $dir/graph data/dev $dir/decode &> $dir/decode.log || exit 1;

    local/kws_search.sh --cmd "$decode_cmd" --duptime $duptime \
        data/lang data/dev $dir/decode
    touch $dir/decode/.done
    echo "See $dir/mkgraph.log and $dir/decode.log for decoding outcomes"
  fi
}
...
(
  if [ ! -f exp/tri5/decode/.done ]; then
    echo ---------------------------------------------------------------------
    ...
    touch exp/tri5/decode/.done
  fi

  if [ ! -f exp/tri5/decode/kws/.done ]; then
    local/kws_search.sh --cmd "$decode_cmd" --duptime $duptime \
        data/lang data/dev exp/tri5/decode.si
    local/kws_search.sh --cmd "$decode_cmd" --duptime $duptime \
        data/lang data/dev exp/tri5/decode
    touch exp/tri5/decode/kws/.done 
  fi
) &
sleep 5; # Let any "start-up error" messages from the subshell get logged
...
(
  if [ ! -f exp/sgmm5/decode/.done ]; then
    echo ---------------------------------------------------------------------
    echo "Spawning exp/sgmm5/decode_fmllr on" `date`
    echo ---------------------------------------------------------------------
    echo "exp/sgmm5/decode will wait on tri5 decode if necessary"
    while [ ! -f exp/tri5/decode_fmllr/.done ]; do sleep 30; done
    ...
    touch exp/sgmm5/decode_fmllr/.done
  fi

  if [ ! -f exp/sgmm5/decode_fmllr/kws/.done ]; then
    echo ---------------------------------------------------------------------
    echo "Starting exp/sgmm5/decode_fmllr/kws on" `date`
    echo ---------------------------------------------------------------------
    local/kws_search.sh --cmd "$decode_cmd" --duptime $duptime \
        data/lang data/dev exp/sgmm5/decode_fmllr
    touch exp/sgmm5/decode_fmllr/kws/.done
  fi
) &
...
for iter in 1 2 3 4; do
  if [ ! -f exp/sgmm5_mmi_b0.1/decode_fmllr_it$iter/.done ]; then
    echo "Waiting for exp/sgmm5/decode_fmllr_it$iter/.done if necessary"
    while [ ! -f exp/sgmm5/decode_fmllr_it$iter/.done ]; do sleep 30; done
    ...
    touch exp/sgmm5_mmi_b0.1/decode_fmllr_it$iter/.done
  fi
  if [ ! -f exp/sgmm5_mmi_b0.1/decode_fmllr_it$iter/kws/.done ]; then
    local/kws_search.sh --cmd "$decode_cmd" --duptime $duptime \
        data/lang data/dev exp/sgmm5_mmi_b0.1/decode_fmllr_it$iter
    touch exp/sgmm5_mmi_b0.1/decode_fmllr_it$iter/kws/.done
  fi
done
