...
#The following is just a small precaution again screw-ups (especially after 11pm)
if [[ `pwd` != *limited* || $1 != *limited* ]] ; then
...
if $decode_eval ; then
  ...
  #-local/lattice_to_ctm.sh --cmd "$decode_cmd" data/eval.uem data/lang exp/tri4/decode_eval.uem
  #-local/kws_search.sh --skip-scoring true --cmd "$decode_cmd" data/lang data/eval.uem/ exp/tri4/decode_eval.uem.si
  #-local/kws_search.sh --skip-scoring true --cmd "$decode_cmd" data/lang data/eval.uem/ exp/tri4/decode_eval.uem
fi
...
####################################################################
if $decode_eval ; then
  steps/decode_sgmm2.sh --skip-scoring true --use-fmllr true --nj 64 --cmd "$decode_cmd" --transform-dir exp/tri4/decode_eval.uem \
    --num-threads 6 --parallel-opts "-pe smp 6 -l ram_free=0.5G" \
    exp/sgmm5/graph data/eval.uem exp/sgmm5/decode_fmllr_eval.uem |tee  exp/sgmm5/decode_fmllr_eval.uem.log
  #-steps/decode_sgmm2.sh --skip-scoring true --nj 64 --cmd "$decode_cmd" --transform-dir exp/tri4/decode_eval.uem \
...
if $decode_eval ; then
  for iter in 1 2 3 4; do
    #steps/decode_sgmm2_rescore.sh --skip-scoring true\
    #  --cmd "$decode_cmd" --iter $iter --transform-dir exp/tri4/decode_eval.uem \
...
if $decode_nnet ; then
  ...
  if $decode_eval ; then
    steps/decode_nnet_cpu.sh --cmd "$decode_cmd" --nj $decode_nj \
      --num-threads 6 --parallel-opts "-pe smp 6 -l ram_free=0.5G" \
      --transform-dir exp/tri4/decode_eval.uem/ \
  ...
  if $decode_test ; then
    steps/decode_nnet_cpu.sh --cmd "$decode_cmd" --nj $decode_nj \
      --num-threads 6 --parallel-opts "-pe smp 6 -l ram_free=0.5G" \
      --transform-dir exp/tri4/decode_test.uem/ \
  ...
  if $decode_shadow ; then
    steps/decode_nnet_cpu.sh --cmd "$decode_cmd" --nj $decode_nj \
      --num-threads 6 --parallel-opts "-pe smp 6 -l ram_free=0.5G" \
      --transform-dir exp/tri4/decode_shadow.uem/ \
