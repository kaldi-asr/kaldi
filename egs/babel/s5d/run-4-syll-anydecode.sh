...
if [ $# -ne 0 ]; then
  echo "Usage: $(basename $0) --type (dev10h.syll|dev2h.syll|eval.syll|shadow.syll)"
  exit 1
fi
...
if ! $skip_kws ; then
  if  $extra_kws ; then
    L1_lex=data/local/dict.syll/lexiconp.txt
...
if [ ! -f ${decode}/.done ]; then
  ...
  steps/decode_fmllr_extra.sh --skip-scoring true --beam 10 --lattice-beam 4\
    ...
    exp/tri5/graph.syll ${dataset_dir} ${decode} |tee ${decode}/decode.log
  touch ${decode}/.done
fi
...
if [ -f exp/sgmm5/.done ]; then
  ...
  if [ ! -f $decode/.done ]; then
    ...
    steps/decode_sgmm2.sh --skip-scoring true --use-fmllr true --nj $my_nj \
      ...
      exp/sgmm5/graph.syll ${dataset_dir} $decode |tee $decode/decode.log
    touch $decode/.done
