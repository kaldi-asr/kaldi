...
if [ $# -ne 3 ]; then
  echo "Usage: ./local/apply_morphs.sh [opts] <morphs_model_dir> <wordlist> <_word2baseform>"
  echo "  --encoding <encoding_type> # lexicon encoding"
  echo "  --max-len <int> # maximum allowed morpheme length"  
  exit 1
fi
...
[ ! -d $model ] && echo "Model directory does not exist." && exit 1
[ ! -f $word_list ] && echo "Word list does not exist." && exit 1
