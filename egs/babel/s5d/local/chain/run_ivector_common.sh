...
gmm=tri5_cleaned          # This specifies a GMM-dir from the features of the type you're training the system on;
...
if [ $stage -le 4 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 7 --mllt-iters "2 4 6" \
     ...
     $numLeavesMLLT $numGaussMLLT $temp_data_root/${train_set}_hires $langdir \
