...
if [ $# != 8 ]; then
   echo "Usage: steps/train-semi.sh <data-dir> <lang-dir> <ali-dir> <exp-dir> <num-gaussians> <num-tree-leaves> <rho-stats> <rho-reest>"
   echo " e.g.: steps/train-semi.sh data/train data/lang exp/tri1_ali exp/tri1-semi 256 1800 0 0"
   exit 1;
fi
...
rho_stats=$7    # set to > 0 to activate prop/interp of suff. stats. (weights only)
rho_iters=$8    # set to > 0 to actiavte smoothing of new model with prior model (weights only)
