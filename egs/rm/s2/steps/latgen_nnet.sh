...
latgen-faster-mapped --max-active=$max_active --beam=$beam --lattice-beam=$lattice_beam --acoustic-scale=${acousticscale:-0.1429} \
  --allow-partial=true --word-symbol-table=$lang/words.txt \
  $srcdir/transition.mdl $graphdir/HCLG.fst "$feats" "ark:|gzip -c > $dir/lat.gz" \
     2> $dir/latgen.log || exit 1;
