...
#    $morph ="$pos/$acf/$kacf/$kav/$ec/$other"; # Include "other" information
    ...
    $pron_case=1;
    ## Case 1 (**Default and better choice.)
    # e.g. クールエディット+(W (? クーレリットル);クールエディット)+名詞   
    # ->
    # クールエディット+クールエディット+名詞
    ...
    # If you use it, you may obtain better result than not using it.
    # You want to use this processing, you change bllow "" to "1"
    $morph_opt = 1;
    if($morph_opt == 1){
#	$morph =~ s/\/形容詞型//;
#	$morph =~ s/\/文語形容詞型//;
