...
# for instance,
# local/chain/compare_wer.sh exp/chain/tdnn1a exp/chain/tdnn1a_smbr:{1,2,3}


if [ $# == 0 ]; then
  echo "Usage: $0: [--looped] [--online] <dir1> [<dir2> ... ]"
  echo "e.g.: $0 exp/chain/tdnn{1a,1b}"
  echo "or (with epoch numbers for discriminative training):"
  echo "$0 exp/chain/tdnn1a_smbr:{1,2,3}"
  exit 1
fi
...
for n in 0 1 2; do
   ...
   for x in $*; do
    ...
    decode_names=(eval1 eval2 eval3)
