...
if (! -e "$data_base/voxceleb1_test.txt") {
  system("wget -O $data_base/voxceleb1_test.txt http://www.robots.ox.ac.uk/~vgg/data/voxceleb/meta/veri_test.txt");
}
...
foreach (@spkr_dirs) {
  ...
  my @rec_dirs = grep {-d "$data_base/voxceleb1_wav/$spkr_id/$_" && ! /^\.{1,2}$/} readdir($dh);
  closedir $dh;
  foreach (@rec_dirs) {
	my $rec_id = $_;
	opendir my $dh, "$data_base/voxceleb1_wav/$spkr_id/$rec_id/" or die "Cannot open directory: $!";
	my @files = map{s/\.[^.]+$//;$_}grep {/\.wav$/} readdir($dh);
	closedir $dh;
  	foreach (@files) {
      my $name = $_;
      my $wav = "$data_base/voxceleb1_wav/$spkr_id/$rec_id/$name.wav";
      my $utt_id = "$new_spkr_id-$rec_id-$name";
