...
class Table(object):
    ...
    def __str__(self):
        ...
        rows = [
            self.colSep.join([c + " " * gap for c, gap in zip(self.colnames, gaps)])
        ]
        for r in sd:
            gaps = [m - len(c) for (m, c) in zip(colwidth, r)]
            rows.append(self.colSep.join([c + " " * d for c, d in zip(r, gaps)]))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "expath", type=str, action="store", help="Path to experiment directory"
    )
    parser.add_argument(
        "-l",
        "--latex",
    ...
    dev_set_query = (
        "SELECT r.exp, r.lm, r.lm_w FROM results AS r "
