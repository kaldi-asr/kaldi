...
if [ $# != 3 ]; then
  echo "Usage: ami_data_prep_edin.sh /path/to/ICSI <segments> <mic>"
  exit 1; 
fi 
...
find $ICSI_DIR -iname '*icsi_bmf.wav' | sort > $dir/wav.flist
...
awk '{print $1 $2}' $dir/wav.scp | \
  perl -ane '$_ =~ m:^(\S+MDM).*\/([B].*)\.wav$: || die "[rec2file] bad label $_"; 
       print "$1 $2 0\n"; '\
  > $dir/reco2file_and_channel || exit 1;

# we assume we adapt to the session only
awk '{print $1}' $dir/segments | \
  perl -ane '$_ =~ m:^(\S+)([xfm][a-z][0-9]{3})(\S+)$: || die "[segs] bad label $_"; 
          print "$1$2$3 $1\n";'  \
    > $dir/utt2spk || exit 1;
