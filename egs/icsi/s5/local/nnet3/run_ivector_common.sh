...
for f in data/$mic/${train_set}/feats.scp ${gmm_dir}/final.mdl; do
  if [ ! -f $f ]; then
...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc.sh --cmd "$train_cmd" --nj 10 data/$mic/${train_set}_sp || exit 1;
  steps/compute_cmvn_stats.sh data/$mic/${train_set}_sp || exit 1;
...
if [ $stage -le 2 ]; then
  ...
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    data/$mic/${train_set}_sp data/lang $gmm_dir $ali_dir || exit 1
fi
...
if [ $stage -le 3 ]; then
  ...
  utils/data/perturb_data_dir_volume.sh data/$mic/${train_set}_sp_hires || exit 1;

  for datadir in ${train_set}_sp ${test_sets}; do
    steps/make_mfcc.sh --nj 10 --mfcc-config conf/mfcc_hires.conf \
      --cmd "$train_cmd" data/$mic/${datadir}_hires || exit 1;
    steps/compute_cmvn_stats.sh data/$mic/${datadir}_hires || exit 1;
    utils/fix_data_dir.sh data/$mic/${datadir}_hires || exit 1;
  done
fi
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 2 \
     data/$mic/${train_set}_sp_hires exp/$mic/nnet3${nnet3_affix}/diag_ubm \
...
if [ $stage -le 6 ]; then
  ...
  ivectordir=exp/$mic/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $ivectordir/storage ]; then
    utils/create_split_dir.pl /export/b0{5,6,7,8}/$USER/kaldi-data/ivectors/icsi-$(date +'%m_%d_%H_%M')/s5/$ivectordir/storage $ivectordir/storage
  fi
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    data/$mic/${train_set}_sp_hires ${temp_data_root}/${train_set}_sp_hires_max2
