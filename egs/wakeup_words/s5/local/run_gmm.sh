...
if [ $stage -le 2 ]; then
  ...
  # alignment
  steps/align_si.sh --cmd "$train_cmd" --nj $nj data/train data/lang exp/mono exp/mono_ali || exit 1;
fi 

# tri1
if [ $stage -le 3 ]; then
  # training
  steps/train_deltas.sh --cmd "$train_cmd" 4000 32000 data/train data/lang exp/mono_ali exp/tri1 || exit 1;
...
if [ $stage -le 4 ]; then
  # training
  steps/train_deltas.sh --cmd "$train_cmd" 7000 56000 data/train data/lang exp/tri1_ali exp/tri2 || exit 1;
...
if [ $stage -le 5 ]; then
  # training [LDA+MLLT]
  steps/train_lda_mllt.sh --cmd "$train_cmd" 10000 80000 data/train data/lang exp/tri2_ali exp/tri3 || exit 1;

  # decoding
  
  # alignment
  steps/align_si.sh --cmd "$train_cmd" --nj $nj data/train data/lang exp/tri3 exp/tri3_ali || exit 1;
  
  steps/align_si.sh --cmd "$train_cmd" --nj ${nj} data/dev data/lang exp/tri3 exp/tri3_ali_dev || exit 1;
fi
