...
# extract ivector 
dataset=$(basename $dataset_dir)
ivector_dir=exp/$L/nnet3/ivectors_${dataset}${ivector_suffix}
...
if $use_bnf; then
  ...
  dataset=$(basename $dataset_dir)
  multi_ivector_dir=exp/$L/nnet3/ivectors_${dataset}_gb
  ...
  steps/nnet3/dump_bottleneck_features.sh --use-gpu true --nj 100 --cmd "$train_cmd" \
    ...
    $multidir $dump_bnf_dir/$L exp/$L/make_${dataset}_bnf || exit 1; 
  ...
  if [ ! -f $appended_bnf/.done ]; then
    steps/append_feats.sh  --nj 16 --cmd "$train_cmd" \
      ...
      ${dataset_dir}_hires_mfcc${aux_suffix}_bnf exp/$L/append_mfcc${aux_suffix}_bnf \
      mfcc_hires${aux_suffix}_bnf/$L || exit 1;

    steps/compute_cmvn_stats.sh $appended_bnf exp/$L/make_cmvn${aux_suffix}_bnf \
      mfcc_hires${aux_suffix}_bnf/$L || exit 1;
...
if [ -f $nnet3_dir/$L/final.mdl ]; then
  echo "nnet3 decoding"
  decode=$nnet3_dir/$L/decode_${dataset_id}
  rnn_opts=
