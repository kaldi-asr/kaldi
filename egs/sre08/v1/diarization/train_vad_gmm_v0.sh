...
while IFS=$'\n' read line; do
  ...
  run.pl $dir/log/init_speech_model.\$utt_id.log \
    gmm-global-init-from-feats --num-gauss=$speech_num_gauss \
    "\$feats select-voiced-frames ark:- scp:$data/vad.scp ark:- |" \
    $dir/\$utt_id.speech.0.mdl || exit 1
  run.pl $dir/log/init_sil_model.\$utt_id.log \
    gmm-global-init-from-feats --num-gauss=$sil_num_gauss \
  ...
  while [ \$x -lt $num_iters ]; do
    run.pl $dir/log/decode.\$utt_id.\$x.log gmm-decode-simple \
    ...
    run.pl $dir/log/update.\$utt_id.\$x.log gmm-acc-stats-ali \
      $dir/\$utt_id.\$x.mdl "\$feats" \
      ark:$dir/\$utt_id.\$x.ali - \| \
      gmm-est $dir/\$utt_id.\$x.mdl - $dir/\$utt_id.\$[x+1].mdl || exit 1

    objf_impr=\$(cat $dir/log/decode.\$utt_id.\$x.log | grep "GMM update: .* objective function improvements" | perl -pe 's/.*GMM update: Overall (\S+) objective function improvement/\$1/')
  ...
  cp \$utt_id.\$x.mdl $dir/\$utt_id.final.mdl 
...
if [ $stage -le 1 ]; then
  $cmd JOB=1:$nj $dir/log/do_vad_job.JOB.log bash $dir/q/do_vad.JOB.sh || exit 1
fi
