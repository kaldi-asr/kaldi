...
while IFS=$'\n' read line; do
  ...
  select-top-chunks \
    ...
    --weights-next=scp:$dir/zero_crossings.scp --num-select-frames-next=$num_frames_sound_next \
    "${feats}" ark:- | gmm-global-init-from-feats \
  ...
  while [ $x -le $num_iters ]; do
    ...
    loglikes-to-class \
      ...
      \"$feats\" ark:- |" \
      "ark:gmm-global-get-frame-likes $tmpdir/$utt_id.sound.$[x+1].mdl \
      \"$feats\" ark:- |" \
      ark:$dir/$utt_id.speech_likes.bootstrap.ark \
  ...
  
  {
    ...
    gmm-global-copy --binary=false $phase2_dir/$utt_id.sound.0.mdl -;
  ...
  while [ $x -le $num_iters_phase2 ]; do
    ...
    lattice-to-post --acoustic-scale=1.0 \
      ...
      gmm-est --mix-up=$[sil_num_gauss+sound_num_gauss+speech_num_gauss] \
  ...
  loglikes-to-class \
    "ark:gmm-global-get-frame-likes $phase2_dir/$utt_id.silence.$x.mdl \
    \"$feats\" ark:- |" \
    "ark:gmm-global-get-frame-likes $phase2_dir/$utt_id.nonsil.$x.mdl \
    \"$feats\" ark:- |" \
