...
if [ $stage -le -12 ]; then
  ...
  echo "3" >> $dir/local/dict/silence_phones.txt
...
while IFS=$'\n' read line; do
  ...
  { 
    cat $dir/trans_test.mdl 
    echo "<DIMENSION> $feat_dim <NUMPDFS> 3" 
    gmm-global-copy --binary=false $tmpdir/$utt_id.silence.0.mdl
    gmm-global-copy --binary=false $tmpdir/$utt_id.sound.0.mdl 
    gmm-global-copy --binary=false $tmpdir/$utt_id.speech.0.mdl 
