...
if [ $# != 2 ]; then
  echo "Usage: split_vad_data.sh [--per-utt] <data-dir> <num-to-split>"
...
for n in `seq $numsplit`; do
   ...
   spk2utts="$spk2utts $data/split$num_split/$n/spk2utt"
done

# If lockfile is not installed, just don't lock it.  It's not a big deal.
which lockfile >&/dev/null && lockfile -l 60 $data/.split_lock 

utils/split_scp.pl $data/spk2utt $spk2utts || exit 1

for n in `seq $numsplit`; do
  dsn=$data/split$numsplit/$n
  utils/spk2utt_to_utt2spk.pk $dsn/spk2utt > $dsn/utt2spk || exit 1;
done
