  ...
  if [ ! -z "$ali_or_decode_dir" ]; then
    $cmd JOB=1:$nj $dir/log/extract_ivectors.JOB.log \
      ...
      ivector-extract --acoustic-weight=$posterior_scale --compute-objf-change=true \
        --max-count=$max_count \
      $srcdir/final.ie "$feats" ark,s,cs:- ark,scp:$dir/ivectors_utt.JOB.ark,$dir/ivectors_utt.JOB.scp || exit 1;
  else
    $cmd JOB=1:$nj $dir/log/extract_ivectors.JOB.log \
      gmm-global-get-post --n=$num_gselect --min-post=$min_post $srcdir/final.dubm "$gmm_feats" ark:- \| \
      ivector-extract --acoustic-weight=$posterior_scale --compute-objf-change=true \
...
ivector_dim=$[$(copy-vector "scp:head -n 1 $dir/ivectors_utt.1.scp |" ark,t:- | wc -w) - 3] || exit 1;
echo  "$0: iVector dim is $ivector_dim"

if [ $stage -le 5 ]; then
  echo "$0: combining iVectors across jobs"
  for j in $(seq $nj); do cat $dir/ivectors_utt.$j.scp; done >$dir/ivectors_utt.scp || exit 1;
fi
