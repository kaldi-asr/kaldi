...
if [ $stage -le 1 ]; then
  mkdir -p data/train_phonotactics
...
#7.38 8.78 A: an- so the topic is 
  ...
  perl -e ' 
   ...
   open(R, "|sort >data/train_phonotactics/reco2file_and_channel") || die "Opening reco2file_and_channel";
...
if [ $stage -le 2 ]; then
  ...
  cp $tmpdir/text.2 data/train_phonotactics/text
  # create segments file and utt2spk file...
  ! cat data/train_phonotactics/text | perl -ane 'm:([^-]+)-([AB])-(\S+): || die "Bad line $_;"; print "$1-$2-$3 $1-$2\n"; ' > data/train_phonotactics/utt2spk  \
     && echo "Error producing utt2spk file" && exit 1;

  cat data/train_phonotactics/text | perl -ane 'm:((\S+-[AB])-(\d+)-(\d+))\s: || die; $utt = $1; $reco = $2; $s = sprintf("%.2f", 0.01*$3);
                 $e = sprintf("%.2f", 0.01*$4); print "$utt $reco $s $e\n"; ' > data/train_phonotactics/segments

  utils/utt2spk_to_spk2utt.pl <data/train_phonotactics/utt2spk > data/train_phonotactics/spk2utt
fi
...
if [ $stage -le 3 ]; then
  ...
  cat $tmpdir/sph.scp | awk -v sph2pipe=$sph2pipe '{printf("%s-A %s -f wav -p -c 1 %s |\n", $1, sph2pipe, $2); 
    ...
    sort -k1,1 -u  > data/train_phonotactics/wav.scp || exit 1;
fi
...
if [ $stage -le 4 ]; then
  ...
  perl -ane 'm:^\S+ (\S+)\.sph ([fm])([fm]): || die "bad line $_;"; print "$1-A $2\n", "$1-B $3\n"; ' | \
   sort | uniq | utils/filter_scp.pl data/train_phonotactics/spk2utt > data/train_phonotactics/spk2gender

  if [ ! -s data/train_phonotactics/spk2gender ]; then
