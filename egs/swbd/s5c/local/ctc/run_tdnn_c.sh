...
train_stage=-10
speed_perturb=true
dir=exp/ctc/tdnn_c  # Note: _sp will get added to this if $speed_perturb == true.
...
num_jobs_initial=3
num_jobs_final=16
minibatch_size=64  # for GPU memory reasons :-(
frames_per_eg=75
remove_egs=false
...
treedir=exp/ctc/tri5c_tree$suffix
...
if [ $stage -le 11 ]; then
  ...
  steps/train_sat.sh --cmd "$train_cmd" --num-iters 1 \
    ...
    --context-opts "--context-width=3 --central-position=2" \
     10000 20000 data/$train_set data/lang_ctc $ali_dir $treedir
fi
