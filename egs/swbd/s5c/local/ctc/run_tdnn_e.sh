...
# run_tdnn_e.sh is a re-tried version of run_tdnn_c.sh after adding some options
# to reduce the number of questions asked at the leftmost context position, to
# keep the number of history-states to a manageable size.
# Reducing the target num-states from 10k to 7k, and increasing the
# --target-num-history-states for the LM to 4k.

# run_tdnn_c.sh is based on run_tdnn_a.sh, but using double the number of
# context-dependent states (10, not 5k) and larger phonetic left-context (2 left
# phones, not 1).  renaming treedir.

# running with speed perturbation (as I also did run_tdnn_a.sh)
# This was way too slow as the number of history-states blew up.
...
train_stage=-10
speed_perturb=true
dir=exp/ctc/tdnn_e  # Note: _sp will get added to this if $speed_perturb == true.
...
treedir=exp/ctc/tri5e_tree$suffix
...
if [ $stage -le 11 ]; then
  ...
  steps/train_sat.sh --cmd "$train_cmd" --num-iters 1 \
    ...
    --context-opts "--context-width=3 --central-position=2" \
    --compile-questions-opts "--leftmost-questions-truncate=16" \
     7000 20000 data/$train_set data/lang_ctc $ali_dir $treedir
fi
