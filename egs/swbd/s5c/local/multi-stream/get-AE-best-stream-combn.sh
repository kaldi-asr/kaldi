...
python local/multi-stream/get-AE-best-stream-combn.py --alpha=${alpha} --topN=${topN} $outdir $num_streams $tmpdir/feat_to_len_list $outdir/compute_stream_combn_scores.task $outdir/ | copy-feats ark:- ark,scp:$outdir/strm_mask.ark,$outdir/strm_mask.scp || exit 1;
