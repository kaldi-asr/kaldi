...
# c is as b, but with 25% bigger hidden layers.

# System               tdnn_leak6b tdnn_leak6c
# WER on train_dev(tg)      14.26     14.07
# WER on train_dev(fg)      13.09     12.97
# WER on eval2000(tg)        16.8      16.7
# WER on eval2000(fg)        15.3      15.2
# Final train prob     -0.0912405 -0.083505
# Final valid prob      -0.114332 -0.109565
# Final train prob (xent)      -1.30061   -1.2281
# Final valid prob (xent)      -1.39084  -1.32965
...
get_egs_stage=-10
speed_perturb=true
dir=exp/chain/tdnn_leak6c  # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
...
if [ $stage -le 12 ]; then
  ...
  relu-renorm-layer name=prefinal-chain input=tdnn7 dim=$hidden_dim target-rms=0.5
  ...
  relu-renorm-layer name=prefinal-xent input=tdnn7 dim=$hidden_dim target-rms=0.5
