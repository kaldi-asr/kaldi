...
# 7m26h is as 7m26g, but changing how the splicing works, with dim=320 and
#  putting all the splicing in the small-dim layer.
#  It's slightly worse, but not enough to make me want to dismiss this architecture.
#  Trying more in 7m26h2.

# local/chain/compare_wer_general.sh --rt03 tdnn7m26g_sp tdnn7m26h_sp
# System                tdnn7m26g_sp tdnn7m26h_sp
# WER on train_dev(tg)      11.68     11.95
# WER on train_dev(fg)      10.94     10.89
# WER on eval2000(tg)        14.6      14.8
# WER on eval2000(fg)        13.3      13.4
# WER on rt03(tg)            17.9      18.1
# WER on rt03(fg)            15.7      15.8
# Final train prob         -0.066    -0.071
# Final valid prob         -0.084    -0.087
# Final train prob (xent)        -0.851    -0.974
# Final valid prob (xent)       -0.8872   -1.0052
# Num-parameters               22865188  20845860#
...
# 7m26a is as 7m25u but with changes so that it will train slower and more
#  consistently between layers: setting orthonormal constraint to -1 for
#  linear layers, and usign l2 there as well (0.001); and using half the
#  l2 I was using before, to make it train more slowly.
#

# 7m25u is as 7m25t but with even wider context, adding even more small-dim
#  splicing.

# 7m25t is as 7m25q but with wider context; introducing layers that add splicing in
# the small dimension.

# 7m25q is as 7m25p but slightly increased amounts of regularization
# (trying to balance the learning rates of the tdnnX and tdnnXl layers, and
# make the output-affine matrices not too big.)

# local/chain/compare_wer_general.sh --rt03 tdnn7m25l2_sp tdnn7m25l_sp tdnn7m25p_sp tdnn7m25q_sp
# System                tdnn7m25l2_sp tdnn7m25l_sp tdnn7m25p_sp tdnn7m25q_sp
# WER on train_dev(tg)      11.98     11.90     11.98     12.05
# WER on train_dev(fg)      11.00     10.92     10.97     11.08
# WER on eval2000(tg)        14.7      14.7      14.9      14.8
# WER on eval2000(fg)        13.3      13.3      13.4      13.2
# WER on rt03(tg)            18.1      18.0      18.2      18.0
# WER on rt03(fg)            15.8      15.7      15.8      15.7
# Final train prob         -0.078    -0.076    -0.076    -0.076
# Final valid prob         -0.091    -0.091    -0.090    -0.091
# Final train prob (xent)        -0.987    -0.973    -0.974    -0.982
# Final valid prob (xent)       -1.0142   -1.0041   -1.0023   -1.0077
# Num-parameters               22735140  22735140  22735140  22735140

# 7m25p is as 7m25l but after a code and script change, so that
# now we are using GeneralDropoutComponent.    Using all machines now.
...
if [ $stage -le 12 ]; then
  ...
  linear-component name=tdnn2l0 dim=320 $linear_opts
  linear-component name=tdnn2l dim=320 $linear_opts input=Append(-1,0,1)
  relu-batchnorm-dropout-layer name=tdnn2 $opts dim=1280
  linear-component name=tdnn3l0 dim=320 $linear_opts
  linear-component name=tdnn3l dim=320 $linear_opts input=Append(-1,0,1)
  relu-batchnorm-dropout-layer name=tdnn3 $opts dim=1280
  linear-component name=tdnn4l0 dim=320 $linear_opts
  linear-component name=tdnn4l dim=320 $linear_opts input=Append(-1,0,1)
  relu-batchnorm-dropout-layer name=tdnn4 $opts dim=1280
  ...
  linear-component name=tdnn6l0 dim=320 $linear_opts
  linear-component name=tdnn6l dim=320 $linear_opts input=Append(-3,0,3)
  relu-batchnorm-dropout-layer name=tdnn6 $opts dim=1536
  linear-component name=tdnn7l0 dim=320 $linear_opts
  linear-component name=tdnn7l dim=320 $linear_opts input=Append(-3,0,3)
  relu-batchnorm-dropout-layer name=tdnn7 $opts input=Append(tdnn7l,tdnn6l,tdnn4l,tdnn2l) dim=1280
  linear-component name=tdnn8l0 dim=320 $linear_opts
  linear-component name=tdnn8l dim=320 $linear_opts input=Append(-3,0,3)
  relu-batchnorm-dropout-layer name=tdnn8 $opts dim=1536
  linear-component name=tdnn9l0 dim=320 $linear_opts
  linear-component name=tdnn9l dim=320 $linear_opts input=Append(-3,0,3)
  relu-batchnorm-dropout-layer name=tdnn9 $opts input=Append(0,tdnn8l,tdnn6l,tdnn5l) dim=1280
  linear-component name=tdnn10l0 dim=320 $linear_opts
  linear-component name=tdnn10l dim=320 $linear_opts input=Append(-3,0,3)
  relu-batchnorm-dropout-layer name=tdnn10 $opts dim=1536
  linear-component name=tdnn11l0 dim=320 $linear_opts
  linear-component name=tdnn11l dim=320 $linear_opts input=Append(-3,0,3)
  relu-batchnorm-dropout-layer name=tdnn11 $opts input=Append(0,tdnn10l,tdnn9l,tdnn7l) dim=1280
