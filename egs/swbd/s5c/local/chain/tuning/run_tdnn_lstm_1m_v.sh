...
# Changed expt naming scheme to run_tdnn_lstm_1*_v.sh to prevent conflicts
# other experiments.
# run_tdnn_lstm_1m_v.sh is like run_tdnn_lstm_1m.sh
# but the lstm layers are run at 100 Hz
...
get_egs_stage=-10
speed_perturb=true
dir=exp/chain/tdnn_lstm_1m_v # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_nj=50
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmp-layer name=fastlstm2 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-1 $lstm_opts
  fast-lstmp-layer name=fastlstm3 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-1 $lstm_opts
  ...
  output-layer name=output input=tdnn7 output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=1.5
  ...
  output-layer name=output-xent input=tdnn7 output-delay=$label_delay dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=1.5
