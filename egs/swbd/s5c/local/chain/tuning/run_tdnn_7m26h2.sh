...
# 7m26h2 is as 7m26h, but adding splicing to a layer that previously didn't have
# splicing, and adding one more layer, trying to get closer to the context of
# 7m26g.
...
if [ $stage -le 12 ]; then
  ...
  linear-component name=tdnn5l0 dim=320 $linear_opts
  linear-component name=tdnn5l dim=320 $linear_opts input=Append(-1,0,1)
  ...
  linear-component name=tdnn12l0 dim=320 $linear_opts
  linear-component name=tdnn12l dim=320 $linear_opts input=Append(-3,0,3)
  relu-batchnorm-dropout-layer name=tdnn12 $opts  dim=1536
