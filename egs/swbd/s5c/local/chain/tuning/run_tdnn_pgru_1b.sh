...
# 1b is the same as 1a, but with fast-pgru replacing pgru

# ./local/chain/compare_wer_general.sh tdnn_pgru_1a_ld5_sp tdnn_pgru_1b_ld5_sp
# System                tdnn_pgru_1a_ld5_sp tdnn_pgru_1b_ld5_sp
# WER on train_dev(tg)      12.63     12.73
# WER on train_dev(fg)      11.60     11.88
# WER on eval2000(tg)        14.7      14.8
# WER on eval2000(fg)        13.4      13.4
# Final train prob         -0.077    -0.072
# Final valid prob         -0.093    -0.088
# Final train prob (xent)        -0.924    -0.878
# Final valid prob (xent)       -0.9971   -0.9643
...
if [ $stage -le 12 ]; then
  ...
  fast-pgru-layer name=pgru1 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 vars_path="$dir/configs"
  relu-renorm-layer name=tdnn4 input=Append(-3,0,3) dim=1024
  relu-renorm-layer name=tdnn5 input=Append(-3,0,3) dim=1024
  fast-pgru-layer name=pgru2 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 vars_path="$dir/configs"
  relu-renorm-layer name=tdnn6 input=Append(-3,0,3) dim=1024
  relu-renorm-layer name=tdnn7 input=Append(-3,0,3) dim=1024
  fast-pgru-layer name=pgru3 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 vars_path="$dir/configs"
