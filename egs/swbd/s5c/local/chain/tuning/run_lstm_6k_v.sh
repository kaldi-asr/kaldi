...
# 6k_v is same as 6k but with two additional lstm layers
...
get_egs_stage=-10
speed_perturb=true
dir=exp/chain/lstm_6k_v # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_nj=50
...
remove_egs=false
common_egs_dir=exp/chain/lstm_6k_sp/egs/
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmp-layer name=fastlstm4 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 $lstm_opts
  fast-lstmp-layer name=fastlstm5 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 $lstm_opts
