...
# tdnn_blstm_1c is same as tdnn_blstm_1b, but with 100Hz blstm layer
...
remove_egs=false
common_egs_dir=exp/chain/tdnn_blstm_1b_sp/egs
...
if [ $stage -le 12 ]; then
  ...
  fast-lstmp-layer name=blstm1-forward input=tdnn3 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-1 $lstm_opts
  relu-renorm-layer name=tdnn4 input=Append(-1,0,1) dim=512
  fast-lstmp-layer name=blstm1-backward input=tdnn3 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=1 $lstm_opts
  relu-renorm-layer name=tdnn5 input=Append(-1,0,1) dim=512
