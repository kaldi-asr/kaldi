...
set -e
stage=0
augmentation_list="reverb:music:noise:babble:clean"
use_ivectors=true
...
if [ $stage -le 1 ]; then
  ...
  combine_str=""
  aug_list_parsed=`echo $augmentation_list | awk -F ":" '{for (i=1; i<=NF; i++) printf "%s ", $i}'`
  for n in $aug_list_parsed; do
    if [ "$n" == "clean" ]; then
  ...
  utils/combine_data.sh data/${train_set}_ms $combine_str
fi
...
if [ $stage -le 2 ]; then
  ...
  steps/make_mfcc.sh --cmd "$train_cmd" --nj 50 \
                     data/${train_set}_ms exp/make_mfcc/${train_set}_ms $mfccdir
  steps/compute_cmvn_stats.sh data/${train_set}_ms exp/make_mfcc/${train_set}_ms $mfccdir
  utils/fix_data_dir.sh data/${train_set}_ms || exit 1;
fi
...
if [ $stage -le 4 ]; then
  ...
  for dataset in ${train_set}_ms; do
...
if [ "$use_ivectors" == "true" ]; then
  if [ $stage -le 6 ]; then
    ...
    utils/subset_data_dir.sh data/${train_set}_ms_hires 30000 data/${train_set}_ms_30k_hires
    utils/data/remove_dup_utts.sh 200 data/${train_set}_ms_30k_hires data/${train_set}_ms_30k_nodup_hires  # 33hr
    ...
    utils/copy_data_dir.sh data/${train_set}_ms_hires data/${ivector_trainset}_ms_hires
    utils/filter_scp.pl -f 2 data/${ivector_trainset}/utt2spk data/${train_set}_ms_hires/utt2uniq | \
        utils/filter_scp.pl - data/${train_set}_ms_hires/utt2spk > data/${ivector_trainset}_ms_hires/utt2spk
    utils/fix_data_dir.sh data/${ivector_trainset}_ms_hires

    # Since the data size is now increased make a subset of it to bring the duration back to required size (140hr)
    utils/subset_data_dir.sh data/${ivector_trainset}_ms_hires 100000 data/${ivector_trainset}_ms_hires_subset
    utils/data/remove_dup_utts.sh 200 data/${ivector_trainset}_ms_hires_subset data/${ivector_trainset}_ms_hires
    steps/compute_cmvn_stats.sh data/${ivector_trainset}_ms_hires exp/make_hires/${ivector_trainset} $mfccdir;
    utils/fix_data_dir.sh data/${ivector_trainset}_ms_hires
  fi
  ...
  if [ $stage -le 7 ]; then
    ...
    utils/copy_data_dir.sh data/${ivector_trainset}_ms_hires data/${ivector_trainset}_ms
    utils/filter_scp.pl data/${ivector_trainset}_ms/utt2spk data/${train_set}_ms/feats.scp > data/${ivector_trainset}_ms/feats.scp
    utils/fix_data_dir.sh data/${ivector_trainset}_ms
    echo "$0: Creating alignments of aug data by copying alignments of clean data"
    local/copy_ali_dir.sh --nj 40 --cmd "$train_cmd" \
        data/${ivector_trainset}_ms exp/${lda_mllt_ali} exp/${lda_mllt_ali}_ms
    ...
    steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
      ...
      5500 90000 data/${ivector_trainset}_ms_hires \
      data/lang exp/${lda_mllt_ali}_ms exp/nnet3/tri3b
  fi
  ...
  if [ $stage -le 8 ]; then
    ...
    steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 --num-frames 200000 \
      data/${train_set}_ms_30k_nodup_hires 512 exp/nnet3/tri3b exp/nnet3/diag_ubm
  fi
  ...
  if [ $stage -le 9 ]; then
    ...
    steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
      data/${ivector_trainset}_ms_hires exp/nnet3/diag_ubm exp/nnet3/extractor || exit 1;
  fi
  ...
  if [ $stage -le 10 ]; then
    ...
    utils/data/modify_speaker_info.sh --utts-per-spk-max 2 data/${train_set}_ms_hires data/${train_set}_ms_max2_hires

    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
      data/${train_set}_ms_max2_hires exp/nnet3/extractor exp/nnet3/ivectors_${train_set}_ms || exit 1;
