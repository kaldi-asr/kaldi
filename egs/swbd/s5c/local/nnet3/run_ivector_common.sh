...
"| sort -k1,1 -u  > $data_dir/wav.scp_scaled || exit 1;
  ...
  utils/subset_data_dir.sh --last data/train_hires $n data/train_nodev_hires ;

  # Take the first 30k utterances (about 1/8th of the data) this will be used
  # for the diagubm training
  utils/subset_data_dir.sh --first data/train_nodev_hires 30000 data/train_30k_hires
  local/remove_dup_utts.sh 200 data/train_30k_hires data/train_30k_nodup_hires  # 33hr

  # create a 100k subset for the lda+mllt training
  utils/subset_data_dir.sh --first data/train_nodev_hires 100000 data/train_100k_hires;
  local/remove_dup_utts.sh 200 data/train_100k_hires data/train_100k_nodup_hires;
  local/remove_dup_utts.sh 300 data/train_nodev_hires data/train_nodup_hires  # 286hr
fi
...
if [ "$speed_perturb" == "true" ]; then
  ...
  if [ $stage -le 4 ]; then
    ...
    data_dir=data/train_nodup_sp_hires
    mfccdir=mfcc_perturbed
    for x in train_nodup_sp_hires; do
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 --num-frames 200000 \
    data/train_30k_nodup_hires 512 exp/nnet3/tri3b exp/nnet3/diag_ubm
fi
