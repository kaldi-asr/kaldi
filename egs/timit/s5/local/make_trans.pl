...
sub ParseTranscript() {
    ...
    #$trans =~s/^h#/<s>/ ; # first h#
    #$trans =~s/h#/<\\s>/ ; # last h#
    # In this recipe, I've created a phone class h#, and a corresponding word H#
    $trans =~s/h#/sil/ ; # first h#
    $trans =~s/h#$/sil/ ; # last h#
    #($trans !~ m/h#/) || die "Found h# character in transcript, other than start or end.";
    
    close(F);
    # add starting, ending pause
    #return "pau " .$trans . " pau"; 
    return $trans ;
}
