...
# run_tdnn_1d.sh is like run_tdnn_1c.sh but use different structure.
...
# chain_cleaned/tdnn_1d_sp: num-iters=871 nj=3..16 num-params=9.4M dim=40+100->5151 combine=-0.085->-0.085 xent:train/valid[579,870,final]=(-1.09,-1.05,-1.06/-1.19,-1.16,-1.16) logprob:train/valid[579,870,final]=(-0.070,-0.066,-0.066/-0.079,-0.077,-0.077)
...
# WER on dev(fglarge)              3.89      4.01
# WER on dev(tglarge)              4.09      4.16
# WER on dev(tgmed)                5.17      5.18
# WER on dev(tgsmall)              5.81      5.84
# WER on dev_other(fglarge)       10.24     10.51
# WER on dev_other(tglarge)       10.87     11.20
# WER on dev_other(tgmed)         13.23     13.54
# WER on dev_other(tgsmall)       14.63     15.14
# WER on test(fglarge)             4.41      4.42
# WER on test(tglarge)             4.54      4.61
# WER on test(tgmed)               5.56      5.59
# WER on test(tgsmall)             6.21      6.32
# WER on test_other(fglarge)      10.62     10.95
# WER on test_other(tglarge)      11.20     11.41
# WER on test_other(tgmed)        13.64     13.87
# WER on test_other(tgsmall)      15.00     15.19
# Final train prob              -0.0688   -0.0660
# Final valid prob              -0.0771   -0.0771
# Final train prob (xent)       -1.1308   -1.0570
# Final valid prob (xent)       -1.2324   -1.1592
...
if [ $stage -le 14 ]; then
  ...
  relu-batchnorm-layer name=tdnn1 dim=512 input=Append(-1,0,1,2)
  relu-batchnorm-layer name=tdnn2 dim=512 input=Append(-3,0,3)
  relu-batchnorm-layer name=tdnn3 dim=512 input=Append(-3,0,3)
  relu-batchnorm-layer name=tdnn4 dim=512 input=Append(-3,0,3)
  relu-batchnorm-layer name=tdnn5 dim=512 input=Append(-6,-3,0)
  ...
  relu-batchnorm-layer name=prefinal-xent input=tdnn5 dim=512 target-rms=0.5
