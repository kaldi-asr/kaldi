...
# Modified for LibriCSS recipe: 2020  Desh Raj
...
if [ $# != 2 ]; then
   ...
   echo "Usage: local/run_beamformit.sh [options] <wav-in-dir> <wav-out-dir>"
...
basename -a $sdir/* > $expdir/channels_input
output_wavfiles=$expdir/wavfiles.list
cat $expdir/channels_input | sed 's/_CH[0-9]/_CH0/g' | sed 's/.wav//g' | sort -u > $output_wavfiles
...
for x in `cat $output_wavfiles`; do
  echo -n "$x"
  for ch in $bmf; do
    wav_name=$( echo $x | sed 's/_CH0//g' | awk -F'_' -v CHANNEL="$ch" '{print $1"_CH"CHANNEL"_"$2".wav"}' )
    echo -n " $wav_name"
  done
  echo ""
done > $input_arrays
