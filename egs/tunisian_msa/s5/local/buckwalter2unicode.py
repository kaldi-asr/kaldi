...
def parse_ignore_string(string):
    symbols = []
...
for (x, y) in options:
    ...
    if x in ("-o", "--output"):
        outFilename = y
    if x in ("-e", "--input-encoding"):
        inEnc = y
    if x in ("-E", "--output-encoding"):
        outEnc = y
    if x in ("-r", "--reverse"):
        reverse = 1
    if x in ("-g", "--ignore-lines"):
        ignoreChars = y
    if x in ("-c", "--columns"):
        columnRange = y
    if x in ("-d", "--delimiter"):
        ...
        if buck2uni.get(delimiter):
            ...
            print(
                "This will obviously cause much confusion as a delimiter!",
                file=sys.stderr,
            )
...
def get_cols_from_range(c_range):
    columns = []
    hyphen_search = re.compile(r"-")

    range_elements = c_range.split(",")

    for i in range_elements:
        # If it contains a hyphen (e.g., 1-3)
        if hyphen_search.search(i):
            [start, end] = i.split("-")
...
def transliterate(in_string):
    out = ""

    if columnRange:
        columns = get_cols_from_range(columnRange)

        # Split the line on the delimiter
        line_cols = in_string.split(delimiter)
        ...
        for i in range(len(line_cols)):
            ...
            if i == 0:
                if i in columns:
                    out = transliterate_string(line_cols[i])
                else:
                    out = line_cols[i]
            else:
                if i in columns:
                    out = out + delimiter + transliterate_string(line_cols[i])
                else:
                    out = out + delimiter + line_cols[i]

    else:
        out = transliterate_string(in_string)

    return out


def transliterate_string(in_string):
    out = ""

    # For normal Buckwalter -> Unicode transliteration..
    if not reverse:

        # Loop over each character in the string, in_string.
...
for line in lines:
    ...
    try:
        ...
        if not ignoreChars:
            ...
            if line[0] in parse_ignore_string(ignoreChars):
