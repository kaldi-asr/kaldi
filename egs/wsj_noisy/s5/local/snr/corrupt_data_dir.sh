...
# background_noise_files - list of noise wav files
# noise_impulse_* - containes pairs of impulse responses and noise files in 
# the following format
# background_noise_files =<list of noise_wav_files>
...
[ ! -s $impnoise_dir/info/background_noise_files ] && echo "$0: $impnoise_dir/info/noise_files contains no noises" && exit 1
[ ! -s $impnoise_dir/info/foreground_noise_files ] && echo "$0: $impnoise_dir/info/noise_files contains no noises" && exit 1
...
if $pad_silence; then
  ...
  cp $src_dir/wav.scp $tmp_dir/src_wav.scp
fi
...
if [ $stage -le 4 ]; then
  ...
  #else
  utils/split_scp.pl $tmp_dir/corrupt_wav_commands.${random_seed}.list \
    $corrupt_wav_command_lists
  #fi
...
if [ $stage -le 5 ]; then
  cat $tmp_dir/corrupted_${random_seed}.list | sort -k1,1 > $dest_dir/wav.scp
  cat $tmp_dir/clean_${random_seed}.list | sort -k1,1 > $output_clean_dir/wav.scp
  cat $tmp_dir/noise_${random_seed}.list | sort -k1,1 > $output_noise_dir/wav.scp
