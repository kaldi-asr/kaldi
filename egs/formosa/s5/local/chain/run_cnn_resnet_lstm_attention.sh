...
train_stage=-10
get_egs_stage=-10
dir=exp/chain/cnn_resnet_lstm_attention_1a  # Note: _sp will get added to this
decode_iter=
...
if [ $stage -le 10 ]; then
  ...
  conv-relu-batchnorm-layer name=cnn1 input=idct height-in=40 height-out=20 height-subsample-out=2 time-offsets=-1,0,1 height-offsets=-1,0,1 num-filters-out=$nf1 learning-rate-factor=0.333 max-change=0.25
  ...
  conv-relu-batchnorm-layer name=cnn4 input=res3 height-in=20 height-out=20 time-offsets=-1,0,1 height-offsets=-1,0,1 num-filters-out=$nf2
  ...
  conv-relu-batchnorm-layer name=cnn7 input=res6 height-in=20 height-out=20 time-offsets=-1,0,1 height-offsets=-1,0,1 num-filters-out=$nf3
