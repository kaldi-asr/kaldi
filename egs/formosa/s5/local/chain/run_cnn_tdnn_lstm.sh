...
train_stage=-10
get_egs_stage=-10
dir=exp/chain/cnn_tdnn_lstm_1a  # Note: _sp will get added to this
decode_iter=
...
if [ $stage -le 10 ]; then
  ...
  conv-relu-batchnorm-layer name=cnn3 input=cnn2 height-in=20 height-out=20 time-offsets=-1,0,1 height-offsets=-1,0,1 num-filters-out=128

  relu-batchnorm-layer name=affine1 input=lda dim=512
 
  # the first splicing is moved before the lda layer, so no splicing here
  relu-batchnorm-layer name=tdnn1 input=cnn3 dim=1024
