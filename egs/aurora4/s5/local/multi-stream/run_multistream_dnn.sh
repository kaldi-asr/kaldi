...
exp=               # useful, for running multiexpts of multistream
...
if [ $stage -le 1 ]; then
  local/multi-stream/multi-stream_bnfeats_train.sh \
    ...
    --strm-indices $strm_indices --iters-per-epoch 5 --dir exp/dnn5b_multistream${exp}_bottleneck_featureXtractor || exit 1;
fi


if [ $stage -le 2 ]; then
  # Train Performance monitor
  local/multi-stream/multi-stream_autoencoder_perf-monitor_train.sh \
    --train data-multistream-fbank/${train_id} --strm-indices $strm_indices \
    --nnet-dir exp/dnn5b_multistream${exp}_bottleneck_featureXtractor \
    --aann-dir exp/aann_tandem_dnn5b_multistream${exp}_bottleneck_featureXtractor || exit 1;
fi

# Get multi-stream masks
mask_dir=strm-mask/dnn5b_multistream${exp}_bottleneck_featureXtractor
...
if [ $stage -le 3 ]; then
  ...
  local/multi-stream/get_autoencoder-pm_strm-mask.sh --njdec $njdec \
    ...
    --strm-indices $strm_indices --tandem-transf-dir tandem_feats/dnn5b_multistream${exp}_bottleneck_featureXtractor_tandem_dim120/pca_transf \
    --aann-dir exp/aann_tandem_dnn5b_multistream${exp}_bottleneck_featureXtractor/aann \
...
if [ $stage -le 4 ]; then
  ...
  if $decode_bottleneck_featureXtractor == "true"; then
    dir=exp/dnn5b_multistream${exp}_bottleneck_featureXtractor
    graph=$graph_src
    test=data-multistream-fbank/${test_id}
  ...
  nnet_dir=exp/dnn5b_multistream${exp}_bottleneck_featureXtractor
  # train, 
...
# train GMM BNF
dir=exp/dnn5b_multistream${exp}_bn-gmm
ali=$ali_src
if [ $stage -le 5 ]; then
  #Train
...
dir=exp/dnn5c_multistream${exp}_bn_fmllr-gmm
ali=exp/dnn5b_multistream${exp}_bn-gmm_ali
...
gmm=exp/dnn5c_multistream${exp}_bn_fmllr-gmm # fmllr-feats, dnn-targets,
...
dir=exp/dnn5d_multistream${exp}_bn_pretrain-dbn
if [ $stage -le 8 ]; then
...
dir=exp/dnn5e_multistream${exp}_bn_pretrain-dbn_dnn
feature_transform=exp/dnn5d_multistream${exp}_bn_pretrain-dbn/final.feature_transform # re-use
dbn=exp/dnn5d_multistream${exp}_bn_pretrain-dbn/6.dbn
...
dir=exp/dnn5f_multistream${exp}_bn_pretrain-dbn_dnn_smbr
srcdir=exp/dnn5e_multistream${exp}_bn_pretrain-dbn_dnn
acwt=0.1
graph=${gmm}/$(basename $graph_src)
#
if [ $stage -le 10 ]; then
...
dir=exp/dnn5g_multistream${exp}_bn_pretrain-dbn_dnn_smbr_run2
srcdir=exp/dnn5f_multistream${exp}_bn_pretrain-dbn_dnn_smbr
acwt=0.1
#
if [ $stage -le 12 ]; then
  # Generate lattices and alignments
