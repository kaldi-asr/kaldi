...
# adapted from babel/s5d recipe. reduced the dimensions of both the TDNN layers
# and both dimensions of the LSTM layers
# For now, we do not include cleanup and train on all the data

# by default, with cleanup:
# local/chain/run_tdnn_lstm_pitch.sh
# %WER 14.82 [ 1456 / 9827, 196 ins, 244 del, 1016 sub ] exp/chain_pitch/tdnn_flstm_sp_bi_online/decode_test/wer_10_1.0
set -e -o pipefail
...
num_threads_ubm=12
nnet3_affix=_pitch  # cleanup affix for nnet3 and chain dirs, e.g. _cleaned
...
local/chain/run_ivector_common_pitch.sh --stage $stage \
                                  --nj $nj \
...
dir=exp/chain${nnet3_affix}/tdnn_flstm${tdnn_affix}_sp_bi
...
if [ $stage -le 20 ]; then
  steps/online/nnet3/prepare_online_decoding.sh \
    ...
    data/langp_test/ exp/nnet3${nnet3_affix}/extractor $dir ${dir}_online

  steps/online/nnet3/decode.sh --nj 32 --cmd "$decode_cmd" \
