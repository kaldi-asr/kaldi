...
gmm=tri5d         # This specifies a GMM-dir from the features of the type you're training the system on;
                         # it should contain alignments for 'train_set'.
langdir=data/langp/tri5_ali

num_threads_ubm=12
nnet3_affix=_pitch     # affix for exp/nnet3 directory to put iVector stuff in, so it
...
if [ $stage -le 3 ]; then
  ...
  utils/data/combine_short_segments.sh \
     data/${train_set}_sp_hires_nopitch $min_seg_len data/${train_set}_sp_hires_nopitch_comb

  # just copy over the CMVN to avoid having to recompute it.
  cp data/${train_set}_sp_hires_nopitch/cmvn.scp data/${train_set}_sp_hires_nopitch_comb/
  utils/fix_data_dir.sh data/${train_set}_sp_hires_nopitch_comb/
...
if [ $stage -le 4 ]; then
  ...
  utils/data/subset_data_dir.sh --utt-list data/${train_set}/feats.scp \
    data/${train_set}_sp_hires_nopitch $temp_data_root/${train_set}_hires_nopitch

  # note: essentially all the original segments should be in the hires data.
  n1=$(wc -l <data/${train_set}/feats.scp)
  n2=$(wc -l <$temp_data_root/${train_set}_hires_nopitch/feats.scp)
  if [ $n1 != $n1 ]; then
  ...
  steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
    ...
    $temp_data_root/${train_set}_hires_nopitch exp/nnet3${nnet3_affix}/tri5
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    data/${train_set}_sp_hires_nopitch exp/nnet3${nnet3_affix}/diag_ubm exp/nnet3${nnet3_affix}/extractor || exit 1;
fi
...
if [ $stage -le 7 ]; then
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    data/${train_set}_sp_hires_nopitch_comb ${temp_data_root}/${train_set}_sp_hires_nopitch_comb_max2

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
    ${temp_data_root}/${train_set}_sp_hires_nopitch_comb_max2 \
...
if [ $stage -le 11 ]; then
  ...
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
         data/${train_set}_sp_comb $langdir $gmm_dir $ali_dir
fi
