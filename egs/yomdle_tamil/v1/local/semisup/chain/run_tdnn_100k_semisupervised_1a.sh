...
sup_lat_dir=exp/semisup_100k/chain/e2e_train_lats  # Seed model options
sup_tree_dir=exp/semisup_100k/chain/tree_e2e  # tree directory for supervised chain system
...
if [ $stage -le 5 ]; then
  steps/nnet3/decode_semisup.sh --num-threads 4 --nj $nj --cmd "$decode_cmd" --beam 15 \
...
if [ $stage -le 11 ]; then
  ...
  relu-batchnorm-layer name=tdnn1 input=Append(-4,0,4) dim=450 $tdnn_opts
  relu-batchnorm-layer name=tdnn2 input=Append(-4,0,4) dim=450 $tdnn_opts
  relu-batchnorm-layer name=tdnn3 input=Append(-4,0,4) dim=450 $tdnn_opts

  ## adding the layers for chain branch
  relu-batchnorm-layer name=prefinal-chain dim=450 target-rms=0.5 $tdnn_opts
  ...
  relu-batchnorm-layer name=prefinal-xent input=tdnn3 dim=450 target-rms=0.5 $tdnn_opts
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 13 ]; then
    ...
    steps/nnet3/chain/get_egs.sh \
      ...
      --left-context $egs_left_context --right-context $egs_right_context \
      --frames-per-eg $unsup_frames_per_eg --frames-per-iter 1000000 \
      --frame-subsampling-factor $frame_subsampling_factor \
...
if [ $stage -le 18 ]; then
    steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
      ...
      $dir/decode${test_graph_affix}_test
fi
exit 0;
