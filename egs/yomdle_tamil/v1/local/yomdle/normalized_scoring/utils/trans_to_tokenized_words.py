...
currency_symbols = set(
    chr(i) for i in range(sys.maxunicode) if unicodedata.category(chr(i)) == "Sc"
)
digits = set(
    chr(i) for i in range(sys.maxunicode) if unicodedata.category(chr(i)) == "Nd"
)
...
def main():
    ...
    with open(input_file, "r", encoding="utf-8") as fh, open(
        ...
        for utt, uttid in SnorIter(fh):
            for char in utt:
                if (
                    (split_punc and char in punc)
                    or (split_punc and char in currency_symbols)
                    or (split_digits and char in digits)
                ):
                    fh_out.write(" ")
                    fh_out.write(char)
                    fh_out.write(" ")
