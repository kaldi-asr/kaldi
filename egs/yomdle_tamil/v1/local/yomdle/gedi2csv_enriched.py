...
class GEDI2CSV(object):
    ...
    def csvfile(self, coords, polys, baseName, pgrot):
        ...
        for i in coords:
            ...
            M, rot = Rotate2D(contour, np.array([x, y]), old_div(degrees * pi, 180))
            rot = np.int0(rot)
            ...
            if file.lower().endswith(".xml"):
                ...
                for i, pgs in enumerate(child.iterfind("gedi:DL_PAGE", namespaces)):
                    ...
                    for zone in pgs.findall("gedi:DL_ZONE", namespaces):
                        ...
                        if (
                            zone.attrib["gedi_type"] == "Text"
                            and zone.attrib["Type"]
                            in ("Machine_Print", "Confusable_Allograph", "Handwriting")
                            and zone.attrib["Quality"] in qualset
                        ):
                            if zone.get("polygon"):
                                keyCnt += 1
                                polygons.append(
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        "--log", type=str, help="Log directory", default="./GEDI2CSV_enriched.log"
    )
