...
beam=13.0
lattice_beam=8.0 # decode_fmllr uses 6.0
min_active=200
...
if [ $# != 3 ]; then
   ...
   echo ""
   echo "  --model <nnet>                                   # non-default location of transition model (opt.)"
...
if [ $stage -le 0 ]; then
  $cmd --num-threads $((num_threads+1)) JOB=1:$nj $dir/log/decode.JOB.log \
    ...
    $model $graphdir/HCLG.fst "ark:$dir/logprob.JOB.ark" "ark:|gzip -c > $dir/lat.JOB.gz" || exit 1;
fi
