...
for f in $sdata/1/feats.scp $nnet $feature_transform $class_frame_counts $graphdir/HCLG.fst; do
...
# compute log probs
if [ $stage -le 0 ]; then
  $cmd --num-threads $((num_threads+1)) JOB=1:$nj $dir/log/compute_logprobs.JOB.log \
    nnet-forward $nnet_forward_opts --feature-transform=$feature_transform --class-frame-counts=$class_frame_counts --use-gpu=$use_gpu "$nnet" "$feats" "ark:$dir/logprob.JOB.ark" || exit 1;
fi
