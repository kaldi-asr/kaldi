...
# 1b is same as 1a but with changes related to fast-lstmp layer
...
blstm_affix=1c  #affix for BLSTM directory, e.g. "a" or "b", in case we change the configuration.
...
if [ $stage -le 15 ]; then
  ...
  fast-lstmp-layer name=blstm1-forward input=lda cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=-1 $lstm_opts
  fast-lstmp-layer name=blstm1-backward input=lda cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=1 $lstm_opts
  ...
  fast-lstmp-layer name=blstm3-forward input=Append(blstm2-forward, blstm2-backward) cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=-3 $lstm_opts
  fast-lstmp-layer name=blstm3-backward input=Append(blstm2-forward, blstm2-backward) cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=3 $lstm_opts
