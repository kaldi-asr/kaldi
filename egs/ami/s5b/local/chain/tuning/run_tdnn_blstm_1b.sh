...
tblstm_affix=1b  #affix for BLSTM directory, e.g. "a" or "b", in case we change the configuration.
...
if [ $stage -le 15 ]; then
  ...
  fast-lstmp-layer name=blstm1-forward input=tdnn3 cell-dim=512 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 $lstm_opts
  relu-renorm-layer name=tdnn4 input=Append(-3,0,3) dim=256
  fast-lstmp-layer name=blstm1-backward input=tdnn3 cell-dim=512 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=3 $lstm_opts
  relu-renorm-layer name=tdnn5 input=Append(-3,0,3) dim=256

  fast-lstmp-layer name=blstm2-forward input=Append(tdnn4, tdnn5) cell-dim=512 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 $lstm_opts
  relu-renorm-layer name=tdnn6 input=Append(-3,0,3) dim=256
  fast-lstmp-layer name=blstm2-backward input=Append(tdnn4, tdnn5) cell-dim=512 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=3 $lstm_opts
  relu-renorm-layer name=tdnn7 input=Append(-3,0,3) dim=256

  fast-lstmp-layer name=blstm3-forward input=Append(tdnn6, tdnn7) cell-dim=512 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 $lstm_opts
  fast-lstmp-layer name=blstm3-backward input=Append(tdnn6, tdnn7) cell-dim=512 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=3 $lstm_opts
