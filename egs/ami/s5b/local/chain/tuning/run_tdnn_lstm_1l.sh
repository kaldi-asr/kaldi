...
# 1l is same as 1k but with smaller delay on the first two lstm layers
...
if [ $stage -le 15 ]; then
  ...
  fast-lstmp-layer name=lstm2 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-1 $lstm_opts
