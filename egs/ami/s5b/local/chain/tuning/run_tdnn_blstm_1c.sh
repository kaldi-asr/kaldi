...
# Same as run_blstm_1a.sh but with tdnn layers from tdnn_lstm_1a

# --mic sdm1 --use-ihm-ali true --train-set train_cleaned  --gmm tri3_cleaned
#System               tdnn_lstm1j_sp_bi_ihmali_ld5tblstm1a_sp_bi_ihmalitblstm1b_sp_bi_ihmali
#WER on dev        37.3      37.0      36.6
#WER on eval        40.4      40.4      40.5
#Final train prob      -0.118532 -0.128493 -0.120583
#Final valid prob      -0.245593 -0.230034 -0.233008
#Final train prob (xent)      -1.48337  -1.58384  -1.50215
#Final valid prob (xent)      -2.11097  -2.03154  -2.01175
...
tblstm_affix=1c  #affix for BLSTM directory, e.g. "a" or "b", in case we change the configuration.
...
if $use_ihm_ali; then
  ...
  dir=exp/$mic/chain${nnet3_affix}/tblstm${tblstm_affix}_sp_bi_ihmali
  ...
  dir=exp/$mic/chain${nnet3_affix}/tblstm${tblstm_affix}_sp_bi
fi
...
if [ $stage -le 15 ]; then
  ...
  fast-lstmp-layer name=blstm1-forward input=tdnn3 cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=-1 $lstm_opts
  relu-renorm-layer name=tdnn4 input=Append(-3,0,3) dim=256
  fast-lstmp-layer name=blstm1-backward input=tdnn3 cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=1 $lstm_opts
  relu-renorm-layer name=tdnn5 input=Append(-3,0,3) dim=256

  fast-lstmp-layer name=blstm2-forward input=Append(tdnn4, tdnn5) cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=-3 $lstm_opts
  relu-renorm-layer name=tdnn6 input=Append(-3,0,3) dim=256
  fast-lstmp-layer name=blstm2-backward input=Append(tdnn4, tdnn5) cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=3 $lstm_opts
  relu-renorm-layer name=tdnn7 input=Append(-3,0,3) dim=256

  fast-lstmp-layer name=blstm3-forward input=Append(tdnn6, tdnn7) cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=-3 $lstm_opts
  fast-lstmp-layer name=blstm3-backward input=Append(tdnn6, tdnn7) cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=3 $lstm_opts
