...
blstm_affix=1b  #affix for BLSTM directory, e.g. "a" or "b", in case we change the configuration.
...
if [ $stage -le 15 ]; then
  ...
  fast-lstmp-layer name=blstm1-forward input=lda cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=-3 $lstm_opts
  fast-lstmp-layer name=blstm1-backward input=lda cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=3 $lstm_opts
