...
tblstm_affix=1a  #affix for BLSTM directory, e.g. "a" or "b", in case we change the configuration.
...
if [ $stage -le 15 ]; then
  ...
  fast-lstmp-layer name=blstm1-forward input=tdnn3 cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=-3 $lstm_opts
  relu-renorm-layer name=tdnn4 input=Append(-3,0,3) dim=256
  fast-lstmp-layer name=blstm1-backward input=tdnn3 cell-dim=512 recurrent-projection-dim=128 non-recurrent-projection-dim=128 delay=3 $lstm_opts
