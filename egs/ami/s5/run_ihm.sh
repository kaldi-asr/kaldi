...
if [ $stage -le 0 ]; then
  [ -e $AMI_DIR ] && echo "$AMI_DIR already exists, better quit than overwrite..." && exit 1
...
if [ $stage -le 4 ]; then
  # Taking a subset accelerates the initial steps.
  # Full set 77h, reduced set 5h.
  local/remove_dup_utts.sh 20 data/$mic/train data/$mic/train_nodup # remvove uh-huh,
  utils/subset_data_dir.sh --shortest data/$mic/train_nodup 20000 data/$mic/train_20k
fi

if [ $stage -le 5 ]; then
  # Mono,
  steps/train_mono.sh --nj $nj --cmd "$train_cmd" --cmvn-opts "--norm-means=true --norm-vars=false" \
    data/$mic/train_20k data/lang exp/$mic/mono
  steps/align_si.sh --nj $nj --cmd "$train_cmd" \
    data/$mic/train_20k data/lang exp/$mic/mono exp/$mic/mono_ali

  # Deltas,
  steps/train_deltas.sh --cmd "$train_cmd" --cmvn-opts "--norm-means=true --norm-vars=false" \
    3000 40000 data/$mic/train_20k data/lang exp/$mic/mono_ali exp/$mic/tri1
  steps/align_si.sh --nj $nj --cmd "$train_cmd" \
