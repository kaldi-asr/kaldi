...
if [ $stage -le 0 ]; then
  ...
  cat $outdir/unigramcounts.txt | head -n $num_words_in | awk '{print $1,NR}' > $outdir/wordlist.in
  cat $outdir/unigramcounts.txt | head -n $num_words_out | awk '{print $1,NR}' > $outdir/wordlist.out
