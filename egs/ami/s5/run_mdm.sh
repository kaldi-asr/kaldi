...
#            data/local/ami_dev_v1_x.segs dev $mic

#local/ami_${mic}_scoring_data_prep_edin.sh $AMI_DIR \
#            data/local/ami_eval_v1_x.segs eval $mic
...
TSPK=`cut -d" " -f2 data/$mic/train/utt2spk | uniq | wc -l`
#steps/make_denlats.sh --nj $TSPK --cmd "$decode_cmd" --config conf/decode.config \
#  data/$mic/train data/lang exp/$mic/tri3a exp/$mic/tri3a_denlats  || exit 1;

#mkdir -p exp/$mic/tri3a_ali
#steps/align_si.sh --nj $TSPK --cmd "$train_cmd" \
...
for lm_suffix in ami_sw_fsh_tgpr.50k; do
  (
    ...
    for i in `seq 1 4`; do
      SPK=$((`cut -d" " -f2 data/$mic/dev/utt2spk | uniq | wc -l`*3))
      decode_dir=exp/$mic/tri3a_mmi_b0.1/decode_dev_${i}.mdl_${lm_suffix}
      steps/decode.sh --nj $SPK --cmd "$decode_cmd" --config conf/decode.config \
        --iter $i --split-opts "--per-utt"  $graph_dir data/$mic/dev $decode_dir &
    done

    i=3 #simply assummed
    SPK=$((`cut -d" " -f2 data/$mic/eval/utt2spk | uniq | wc -l`*3))
    decode_dir=exp/$mic/tri3a_mmi_b0.1/decode_eval_${i}.mdl_${lm_suffix}
    steps/decode.sh --nj $SPK --cmd "$decode_cmd" --config conf/decode.config \
      --iter $i --split-opts "--per-utt" $graph_dir data/$mic/eval $decode_dir &
  )&
done
