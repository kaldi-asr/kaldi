...
if [ $stage -le 1 ]; then
  dir=exp/$mic/dnn4_fbank_theano-dnn_ExpLrSched
  ali=${gmmdir}_ali

  # Train
  $cuda_cmd $dir/log/train_nnet.log \
    theano-nnet/nnet1/train_ExpLrSched.sh \
    --train-opts "--keep-lr-iters=3" \
    --cmvn-opts "--norm-means=true --norm-vars=true" --delta-opts "--delta-order=2" \
    $data_fbank/train data/lang $ali $dir
  # Decode (reuse HCLG graph)
  theano-nnet/nnet1/decode.sh --nj $nj_decode --cmd "$decode_cmd" --config conf/decode_dnn.conf --acwt 0.1 \
    --num-threads 3 \
