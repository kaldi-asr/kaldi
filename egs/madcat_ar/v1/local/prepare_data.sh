...
if [ $stage -le 1 ]; then
  ...
  echo "Date: $(date)."
  local/process_data.py $download_dir1 $download_dir2 $download_dir3 $data_splits_dir/madcat.dev.raw.lineid data/dev $images_scp_dir/dev/images.scp || exit 1
  local/process_data.py $download_dir1 $download_dir2 $download_dir3 $data_splits_dir/madcat.test.raw.lineid data/test $images_scp_dir/test/images.scp || exit 1
  local/process_data.py $download_dir1 $download_dir2 $download_dir3 $data_splits_dir/madcat.train.raw.lineid data/train $images_scp_dir/train/images.scp || exit 1
