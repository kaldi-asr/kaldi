...
def write_segment(
    start_time, end_time, text, reco_id, segments_fh, utt2spk_fh, text_fh
):
    ...
    utt_id = "{reco_id}-{st:06d}-{end:06d}".format(
        reco_id=reco_id, st=int(start_time * 100), end=int(end_time * 100)
    )

    print(
        "{utt_id} {reco_id} {st} {end}"
        "".format(utt_id=utt_id, reco_id=reco_id, st=start_time, end=end_time),
        file=segments_fh,
    )
    print(
        "{utt_id} {reco_id}" "".format(utt_id=utt_id, reco_id=reco_id), file=utt2spk_fh
    )
    print("{utt_id} {text}" "".format(utt_id=utt_id, text=text), file=text_fh)


def parse_calls_transcript_file(transcript_file, segments_fh, utt2spk_fh, text_fh):
    ...
    for line in open(transcript_file):
        ...
        if i == 0 and not parts[0].startswith("0"):
            raise Exception(
                "Transcript file {0} does not start with 0.000"
...
def parse_non_calls_transcript_file(transcript_file, segments_fh, utt2spk_fh, text_fh):
    base_name = os.path.basename(transcript_file)
    file_id = re.sub(".transcription.txt", "", base_name)
    ...
    with open(transcript_file) as fh:
        ...
        while text != "":
            ...
            if not line.startswith("["):
                raise Exception(
                    "Time-stamp in transcript file {0} does not start with [; error parsing line {1} after text {2}"
            ...
            write_segment(
                start_time, end_time, text, file_id, segments_fh, utt2spk_fh, text_fh
            )
            start_time = end_time
            text = fh.readline()
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print(
            "Usage: {0} <corpus-root-dir> <calls-list> <non-calls-list> <data-dir>",
            file=sys.stderr,
        )
        raise SystemExit(1)
    ...
    reco2file_and_channel_fh = open("{0}/reco2file_and_channel".format(data_dir), "w")
    ...
    for line in calls_list:
        ...
        for channel in [1, 2]:
            ...
            print(
                "{reco_id} sox {wav_file} -r 8000 -b 16 -c 1 -t wav - remix {channel} |"
                "".format(reco_id=reco_id, wav_file=wav_file, channel=channel),
                file=wav_scp_fh,
            )

        parse_calls_transcript_file(transcript_file, segments_fh, utt2spk_fh, text_fh)
    ...
    for line in non_calls_list:
        ...
        print(
            "{file_id} {file_id} 1" "".format(file_id=file_id),
            file=reco2file_and_channel_fh,
        )
        print(
            "{reco_id} sox {wav_file} -r 8000 -b 16 -c 1 -t wav - |"
            "".format(reco_id=file_id, wav_file=wav_file),
            file=wav_scp_fh,
        )

        parse_non_calls_transcript_file(
            transcript_file, segments_fh, utt2spk_fh, text_fh
        )
