...
if [ $stage -le 3 ]; then
  ...
  steps/decode.sh --cmd "$decode_cmd" --config conf/decode.config --nj 10 --stage 1 \
...
# nnet1 setup
#local/nnet/run_dnn.sh --stage $nn_stage
#local/nnet/run_cnn.sh --stage $nn_stage
#local/nnet/run_lstmp.sh --stage $nn_stage

# nnet2 setup
#local/nnet2/run_pnorm.sh
#local/nnet2/run_maxout.sh
