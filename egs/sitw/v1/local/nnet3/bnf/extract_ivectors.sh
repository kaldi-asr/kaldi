...
# Apache 2.0.
#
# Modified version of egs/sre08/v1/sid/extract_ivectors.sh (commit 26b0746f0d601e60e87615c649d919525cbd8d9d)

# This script extracts iVectors for a set of utterances, given
# bottleneck and speaker ID features and a trained iVector extractor.
...
add_bnf=false  # If true, BNF features are appended to speaker ID features for stats computation
...
if [ $# != 4 ]; then
  echo "Usage: $0 <extractor-dir> <data> <data-bnf> <ivector-dir>"
  echo " e.g.: $0 exp/extractor data/train data/train_bnf exp/ivectors_bnf"
...
for f in $srcdir/final.ie $srcdir/final.ubm $srcdir/final_bnf.ubm $data/feats.scp $data_bnf/feats.scp ; do
...
## Set up features.
bnf_feats="ark,s,cs:select-voiced-frames scp:$sdata_bnf/JOB/feats.scp scp,s,cs:$sdata/JOB/vad.scp ark:- |"
...
if [ $stage -le 0 ]; then
  ...
  dubm="fgmm-global-to-gmm $srcdir/final_bnf.ubm -|"

  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/extract_ivectors.JOB.log \
    gmm-gselect --n=$num_gselect "$dubm" "$bnf_feats" ark:- \| \
    fgmm-global-gselect-to-post --min-post=$min_post $srcdir/final_bnf.ubm "$bnf_feats" \
