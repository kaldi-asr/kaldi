...
# Apache 2.0
#
# Based on egs/tedlium/s5_r2/local/prepare_data.sh (commit edb1aae9457f6441a224dbc451bb8c5220dfefc7).
...
for set in dev test train; do
  ...
  # - it 's -> it's : merge words that contain apostrophe (if compound in dictionary, local/nnet3/bnf/tedlium_join_suffix.py)
...
;;'
    ...
    cat db/TEDLIUM_release2/$set/stm/*.stm | sort -k1,1 -k2,2 -k4,4n | \
      sed -e 's:<F0_M>:<o,f0,male>:' \
          ...
          -e 's:([^ ]*)$::' | \
      awk '{ $2 = "A"; print $0; }'
  } | local/nnet3/bnf/tedlium_join_suffix.py > data/tedlium_${set}/stm
  ...
  if [ $set == "train" ]; then
    mv data/tedlium_${set}/segments data/tedlium_${set}/segments.temp
    utils/data/extend_segment_times.py --start-padding=0.15 \
      --end-padding=0.1 <data/tedlium_${set}/segments.temp >data/tedlium_${set}/segments || exit 1
    rm data/tedlium_${set}/segments.temp
  fi
...
utils/combine_data.sh data/tedlium data/tedlium_train data/tedlium_dev data/tedlium_test
