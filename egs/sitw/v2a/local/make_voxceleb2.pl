...
foreach (@spkr_dirs) {
  ...
  foreach (@rec_dirs) {
    ...
    foreach (@files) {
      my $name = $_;
      #my $wav = "/exp/dsnyder/sitw-2018-05-05/egs/sitw/v2a/ffmpeg-4.0/ffmpeg -v 8 -i $data_base/$dataset/$spkr_id/$rec_id/$name.m4a -f wav -acodec pcm_s16le -|";
      my $wav = "ffmpeg -v 8 -i $data_base/$dataset/$spkr_id/$rec_id/$name.m4a -f wav -acodec pcm_s16le -|";
