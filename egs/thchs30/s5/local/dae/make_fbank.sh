...
# Tsinhua University (Author: Dong Wang)
# Apache 2.0
...
compress=true
addnoise_opt="--noise-level 0 --sigma0 10 --seed 32 --verbose 0"
...
if [ $# != 3 ]; then
   ...
   echo "  --addnoise-opt                                   # options for add noise"
...
if [ -f $data/segments ]; then
  ...
  echo "local/dae/add-noise-mod.py $addnoise_opt --wav-src $logdir/wav.JOB.scp | compute-fbank-feats $vtln_opts --verbose=2 --config=$fbank_config ark:- ark:- 
"
  $cmd JOB=1:$nj $logdir/make_fbank_${name}.JOB.log \
    local/dae/add-noise-mod.py $addnoise_opt --wav-src $logdir/wav.JOB.scp \| \
    compute-fbank-feats $vtln_opts --verbose=2 --config=$fbank_config ark:- ark:- \| \
    copy-feats --compress=$compress ark:- \
     ark,scp:$fbankdir/raw_fbank_$name.JOB.ark,$fbankdir/raw_fbank_$name.JOB.scp \
     || exit 1;
