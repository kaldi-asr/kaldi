...
printf "\t dir       : $dir \n"
printf "\t Train-set : $data $(cat $data/feats.scp | wc -l), $target \n"
printf "\t CV-set    : $data_cv $(cat $data_cv/feats.scp | wc -l) $target_cv \n"
echo
...
cat $target/feats.scp | utils/shuffle_list.pl --srand ${seed:-777} > $dir/tgt_train.scp
cp $target_cv/feats.scp $dir/tgt_cv.scp
...
echo
echo "# PREPARING TARGET FEATS"
if [ ! -z "$labels" ]; then
  echo "Using targets '$labels' (by force)"
  labels_tr="$labels"
  labels_cv="$labels"
else
  echo "Using targets from dirs '$target' '$target_cv'"
  labels_tr="ark:copy-feats scp:$dir/tgt_train.scp ark:- |"
  labels_cv="ark:copy-feats scp:$dir/tgt_cv.scp ark:- |"
fi
...
head -n 10000 $dir/tgt_train.scp > $dir/tgt_train.scp.10k
...
if [ ! -z "$cmvn_opts" ]; then
  ...
  labels_tr="$labels_tr apply-cmvn $cmvn_opts --utt2spk=ark:$target/utt2spk scp:$target/cmvn.scp ark:- ark:- |"
  labels_cv="$labels_cv apply-cmvn $cmvn_opts --utt2spk=ark:$target_cv/utt2spk scp:$target_cv/cmvn.scp ark:- ark:- |"
else
...
if [ ! -z "$delta_opts" ]; then
  ...
  labels_tr="$labels_tr add-deltas --delta-order=$delta_order ark:- ark:- |"
  labels_cv="$labels_cv add-deltas --delta-order=$delta_order ark:- ark:- |"
...
if [ ! -z "$feature_transform" ]; then
  ...
  case $feat_type in
    ...
    transf)
      ...
      [ -z $transf ] && transf=$target/final.mat
  ...
  case "$network_type" in
    dnn)
      utils/nnet/make_nnet_proto.py $proto_opts \
        ...
        $num_fea $num_tgt $hid_layers $hid_dim | grep -v "Softmax" >$nnet_proto || exit 1 
      ;;
    cnn1d)
...
labels_tr="$labels_tr feat-to-post ark:- ark:- |"
labels_cv="$labels_cv feat-to-post ark:- ark:- |"
