...
if [ $stage -le 0 ]; then
  ...
  rm -rf data/fbank && mkdir -p data/fbank &&  cp -R data/{train,dev,test,test.ph} data/fbank || exit 1;
  ...
  echo "producing test.fbank.ph"
  cp data/fbank/test/feats.scp data/fbank/test.ph && cp data/fbank/test/cmvn.scp data/fbank/test.ph || exit 1;
...
if [ $stage -le 1 ]; then
  ...
  $cuda_cmd $outdir/log/train_nnet.log \
    steps/nnet/train.sh --copy_feats false --cmvn-opts "--norm-means=true --norm-vars=false" --hid-layers 4 --hid-dim 1024 --learn-rate 0.008 \
    data/fbank/train data/fbank/dev data/lang $alidir $alidir_cv $outdir || exit 1;
  ...
  (
    steps/nnet/decode.sh --nj $nj --cmd "$decode_cmd" --srcdir $outdir --config conf/decode_dnn.config --acwt 0.1 \
      $gmmdir/graph.word data/fbank/test $outdir/decode_test_word || exit 1; 
  )&
  (
   steps/nnet/decode.sh --nj $nj --cmd "$decode_cmd" --srcdir $outdir --config conf/decode_dnn.config --acwt 0.1 \
     $gmmdir/graph.phone data/fbank/test.ph $outdir/decode_test_phone || exit 1; \
  )&
  ...
  for ITER in 3 2 1; do
   (
    steps/nnet/decode.sh --nj $nj --cmd "$decode_cmd" --nnet $outdir/${ITER}.nnet --config conf/decode_dnn.config --acwt $acwt \
      $gmmdir/graph.word data/fbank/test $outdir/decode_test_word_it${ITER} || exit 1;
   )&
   (
   steps/nnet/decode.sh --nj $nj --cmd "$decode_cmd" --nnet $outdir/${ITER}.nnet --config conf/decode_dnn.config --acwt $acwt \
     $gmmdir/graph.phone data/fbank/test.ph $outdir/decode_test_phone_it${ITER} || exit 1; \
   )&
  done
fi
