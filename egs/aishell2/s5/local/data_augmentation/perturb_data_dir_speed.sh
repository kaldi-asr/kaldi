...
if [ -f $srcdir/segments ]; then
  ...
  utils/apply_map.pl -f 1 $destdir/reco_map <$srcdir/wav.scp | sed 's/| *$/ |/' | \
    ...
    awk -v factor=$factor -v m=$mode \
        '{wid=$1; $1=""; if ($NF=="|") {print wid $_ " sox -t wav - -t wav - " m factor " |"}
          else if (match($0, /:[0-9]+$/)) {print wid " wav-copy" $_ " - | sox -t wav - -t wav - " m factor " |" } 
          else  {print wid " sox -t wav" $_ " -t wav - " m factor " |"}}' > $destdir/wav.scp
