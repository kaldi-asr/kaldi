...
num_jobs_final=2   # Number of neural net jobs to run in parallel at the end of training
...
add_layers_period=0 # by default, add new layers every 2 iterations.
stage=-6
...
cleanup=true
egs_dir=
max_lda_jobs=4  # use no more than 10 jobs for the LDA accumulation.
lda_opts=
egs_opts=
transform_dir=     # If supplied, this dir used instead of alidir to find transforms.
cmvn_opts="--norm-means=false"  # will be passed to get_lda.sh and get_egs.sh, if supplied.
...
realign_times=          # List of times on which we realign.  Each time is
                        ...
                        # number.
num_jobs_align=2       # Number of jobs for realignment
...
if [ $# != 5 ]; then
  echo "Usage: $0 [opts] <data> <lang> <ali-dir> <old-exp-dir> <exp-dir>"
...
cp -r $idir/configs $dir/
cp $idir/final.mdl $dir/0.mdl
...
if [ $egs_left_context -lt $left_context ] || [ $egs_right_context -lt $right_context ]; then
	echo "$0: egs in $egs_dir have too little context";
	exit -1;
fi
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    if [ ! -z "${realign_this_iter[$x]}" ]; then
      ...
      list_rm="$list_rm $dir/post.${x}.*.vec"
