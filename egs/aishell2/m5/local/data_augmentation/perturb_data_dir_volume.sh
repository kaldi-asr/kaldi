...
if [ -z "$reco2vol" ]; then
  ...
  wav-to-volume scp:$data/wav.scp ark,t:- | awk -v awk_seed=${local_seed}  \
    -v awk_low=${scale_abslowvol} -v awk_high=${scale_abshighvol}          \
    -v awk_out=${reco2vol}                                                 \
    'BEGIN{ srand(awk_seed); }                                             \
    {                                                                      \
      dst_vol=int(rand() * (awk_high - awk_low) + awk_low);                \
      amp_vol=(dst_vol * 1.0) / ($2 * 1.0);                                \
      print $1,amp_vol > awk_out;                                          \
    }'
fi

cat $data/wav.scp |  perturb_volume.py \
