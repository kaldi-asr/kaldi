#!/usr/bin/perl -w
#sri_gabon_read_rename_wav_filenames.pl - copy wav file name to a unique name
use strict;
use warnings;
use Carp;

BEGIN {
    @ARGV == 2 or croak "USAGE: sri_gabon_read_rename_wav_filenames.pl WAVFILENAMESFILE DIRECTORY
...
LINE: while ( my $line = <$WFN> ) {
    ...
    next LINE unless ( $read_conv eq 'read' );
    my $speakername = basename $path;
    # create the new directory
    mkdir "$dir/sri_gabon_read_$speakername";
    copy "$line", "$dir/sri_gabon_read_$speakername/sri_gabon_read_${speakername}_${file_number}.wav";    
}
