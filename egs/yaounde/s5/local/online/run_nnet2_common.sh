...
if [ $stage -le 1 ]; then
    for datadir in train test answers sri_gabon; do
	utils/copy_data_dir.sh \
	    data/$datadir \
	    data/${datadir}_hires

	steps/make_mfcc.sh \
	    --nj 4 --mfcc-config conf/mfcc_hires.conf --cmd "$train_cmd" \
	    data/${datadir}_hires \
	    exp/make_hires/$datadir \
	    $mfccdir || exit 1;
	...
	--first data/train_hires \
	7138 \
	data/train_hires || exit 1
fi
