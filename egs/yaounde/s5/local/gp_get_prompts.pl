#!/usr/bin/perl -w
#gp_get_prompts.pl - make 2 parts of a prompts file
use strict;
use warnings;
use Carp;
BEGIN {
    @ARGV == 1 or croak "USAGE: gp_get_prompts.pl TRLLISTFILE
output:
2 files
...
my $out_id_file = "data/local/tmp/gp_id_list.txt";
my $out_prompts_file = "data/local/tmp/gp_prompts_list.txt";
...
while ( my $line = <$L> ) {
	...
	if ( $linea =~ /^\;SprecherID\s(FR\d{1,3})/ ) {
	    ...
	    my $n = $1;
	    if ( $n < 10 ) {
		print $I "gp_${spkr}_000${n}\n";
	    } elsif ( $n < 100 ) {
		print $I "gp_${spkr}_00${n}\n";
	    } elsif ( $n < 1000 ) {
		print $I "gp_${spkr}_0${n}\n";
	    } elsif ( $n < 10000 )  {
		print $I "gp_${spkr}_${n}\n";
	    }
	} else {
	    print $P "$linea\n";
	}      
