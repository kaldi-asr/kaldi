...
mkdir -p $tmpdir
test=${src_dir}_test_threegram
mkdir -p $test
cp -r ${src_dir}/* $test
gunzip -c $lm_dir/lm_threegram.arpa.gz | utils/find_arpa_oovs.pl $test/words.txt > $tmpdir/oovs_threegram.txt || exit 1
gunzip -c $lm_dir/lm_threegram.arpa.gz | grep -v '<s> <s>' | grep -v '</s> <s>' | grep -v '</s> </s>' | arpa2fst - | fstprint | utils/remove_oovs.pl $tmpdir/oovs_threegram.txt | utils/eps2disambig.pl | utils/s2eps.pl | fstcompile --isymbols=$test/words.txt --osymbols=$test/words.txt --keep_isymbols=false --keep_osymbols=false | fstrmepsilon | fstarcsort --sort_type=ilabel > $test/G.fst
