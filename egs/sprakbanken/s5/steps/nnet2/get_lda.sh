...
case $feat_type in
  #ASK: Added (p,) to 'scp,p:$sdata/JOB/feats.scp ark:-'
  #ASK: This worked even though feature type is reported as being lda at runtime
  raw) feats="ark,s,cs:apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp,p:$sdata/JOB/feats.scp ark:- |"
   ;;
  lda) 
    splice_opts=`cat $alidir/splice_opts 2>/dev/null`
    cp $alidir/splice_opts $dir 2>/dev/null
    cp $alidir/final.mat $dir    
      feats="ark,p,s,cs:apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:$sdata/JOB/feats.scp ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $dir/final.mat ark:- ark:- |"
    ;;
...
if [ $stage -le 0 ]; then
  ...
  #ASK: Added (p,) to ali-to-post
  $cmd JOB=1:$nj $dir/log/lda_acc.JOB.log \
    ali-to-post "ark,p:gunzip -c $alidir/ali.JOB.gz|" ark:- \| \
