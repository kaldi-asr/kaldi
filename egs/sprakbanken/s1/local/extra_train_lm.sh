...
cat $lexicon | perl -pe 's/\.$//g' | perl -pe 's/^\.| \. / /g'  > $lmdir/lm_input
python3 local/sprak_prep_lm.py $lmdir/lm_input $lmdir/lm_prep
...
text2wfreq < $lmdir/lm_prep | wfreq2vocab -top 30000 > $lmdir/sprak.vocab
...
awk '{if(NF==1){ printf("0 0 %s %s\n", $1,$1); }} END{print "0 0 #0 #0"; print "0";}' \
  < "$lmdir/lm_prep"  >$tmpdir/select_empty.fst.txt
