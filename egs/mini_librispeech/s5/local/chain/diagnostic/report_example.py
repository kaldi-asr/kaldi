...
ss = 4
ax1.scatter(
    model1["tdnn4.affine"]["col-norms-3"], model1["tdnn3.affine"]["row-change"], s=ss
)
ax1.set_title("row-change3 versus  column-norms4", fontsize=fs)

ax2.scatter(
    model1["tdnn4.affine"]["col-norms-3"],
    model1["tdnn3.affine"]["rel-row-change"],
    s=ss,
)
ax2.set_title("rel-row-change3 versus  column-norms4", fontsize=fs)

ax3.scatter(
    model1["tdnn4.affine"]["col-norms-3"], model1["tdnn3.affine"]["row-norms"], s=ss
)
ax3.set_title("row-norms3 versus  column-norms4", fontsize=fs)

ax4.scatter(
    model1["tdnn4.affine"]["col-norms"], model1["tdnn4.affine"]["rel-col-change"], s=ss
)
ax4.set_title("rel-col-change4 versus col-norms4", fontsize=fs)

ax5.scatter(
    model1["tdnn3.batchnorm"]["stats-stddev"],
    model1["tdnn4.affine"]["col-norms-3"],
    s=ss,
)
ax5.set_title("col-norms4 versus batch-norm-stddev3", fontsize=fs)
...
ax6.scatter(
    model2["tdnn3.relu"]["deriv-avg"] * model2["tdnn3.relu"]["oderiv-rms"],
    # model1['tdnn3.relu']['oderiv-rms'],
    model2["tdnn3.affine"]["row-norms"],
    s=ss,
)
ax6.set_xlim(left=0.00, right=0.009)
ax6.set_title("row-norms3 vs ideriv-rms3", fontsize=fs)
