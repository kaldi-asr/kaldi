...
local/fvector/run_fvector_separate.sh --stage 4 --data data/train_clean_5 --noise-data data/noise \
  --egs-dir exp/fvector/egs --fvector-dir exp/fvector_time_shift_only
fi
exit 0
if [ $stage -le 3 ]; then
...
if [ $stage -le 4 ]; then
  local/fvector/run_tdnn.sh --stage 13 --train-stage -10 --affix "1e_fvector"
fi
