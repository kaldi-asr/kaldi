...
import gi
from gi.repository import Gdk, GObject, Gst, Gtk
    ...
    def init_gst(self):
        ...
        if self.asr:
            ...
            if not os.path.isdir(model_dir):
                print(
                    "Model (%s) not downloaded. Run run-simulated.sh first" % model_dir,
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            print(
                "Couldn't create the onlinegmmfasterdecoder element. ", file=sys.stderr
            )
