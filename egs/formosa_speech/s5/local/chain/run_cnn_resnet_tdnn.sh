...
train_stage=-10
get_egs_stage=-10
dir=exp/chain/cnn_resnet_tdnn_1a  # Note: _sp will get added to this
decode_iter=
...
num_jobs_final=8
minibatch_size=64
#frames_per_eg=150,110,90	# not used
remove_egs=true
common_egs_dir=
xent_regularize=0.1

# cnn_tdnn_lstm options
chunk_width=150
...
#  fixed-affine-layer name=lda input=Append(-1,0,1,ReplaceIndex(ivector, t, 0)) affine-transform-file=$dir/configs/lda.mat delay=$label_delay
  ...
  conv-relu-batchnorm-layer name=cnn1 input=idct $a height-in=40 height-out=20 height-subsample-out=2 time-offsets=-1,0,1 height-offsets=-1,0,1 num-filters-out=$nf1 learning-rate-factor=0.333 max-change=0.25
  res-block name=res2 num-filters=$nf1 height=20 time-period=1 $res_opts
  res-block name=res3 num-filters=$nf1 height=20 time-period=1 $res_opts

  conv-relu-batchnorm-layer name=cnn2 input=res3 height-in=20 height-out=20 time-offsets=-1,0,1 height-offsets=-1,0,1 $a num-filters-out=$nf2
  ...
  conv-relu-batchnorm-layer name=cnn3 input=res6 height-in=20 height-out=20 time-offsets=-1,0,1 height-offsets=-1,0,1 $a num-filters-out=$nf3
  res-block name=res8 num-filters=$nf3 num-bottleneck-filters=$nb3 height=20 time-period=4 $res_opts
  res-block name=res9 num-filters=$nf3 num-bottleneck-filters=$nb3 height=20 time-period=4 $res_opts
  res-block name=res10 num-filters=$nf3 num-bottleneck-filters=$nb3 height=20 time-period=4 $res_opts
...
#  relu-batchnorm-layer name=tdnn1 input=channel-average dim=1024
...
#  fast-lstmp-layer name=lstm3 cell-dim=1024 recurrent-projection-dim=256 non-recurrent-projection-dim=256 delay=-3 dropout-proportion=0.0 $lstm_opts
  ...
  output-layer name=output input=channel-average output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=1.5
  ...
  output-layer name=output-xent input=channel-average output-delay=$label_delay dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=1.5
...
if [ $stage -le 11 ]; then
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $dir/egs/storage ]; then
    utils/create_split_dir.pl \
     /export/b0{5,6,7,8}/$USER/kaldi-data/egs/taiwanspeech-$(date +'%m_%d_%H_%M')/s5c/$dir/egs/storage $dir/egs/storage
  fi
