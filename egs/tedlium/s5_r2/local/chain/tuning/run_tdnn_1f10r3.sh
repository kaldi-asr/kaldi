...
# 1f10r3 is as 1f10r2 but using tdnnf2-layer instead of tdnnf-layer...
# it moves the Sum() operation to before the final relu+batchnorm, but
# introduces a NormalizeLayer first, to control the training speed.
# note: bypass-scale=0.5 may not be ideal, I'll try other values.

# A bit worse than r2 but around the same as r.

# local/chain/compare_wer_general.sh exp/chain_cleaned/tdnn1f10r_sp_bi exp/chain_cleaned/tdnn1f10r2_sp_bi exp/chain_cleaned/tdnn1f10r3_sp_bi
# System                tdnn1f10r_sp_bi tdnn1f10r2_sp_bi tdnn1f10r3_sp_bi
# WER on dev(orig)            8.1       8.0       8.2
# WER on dev(rescored)        7.6       7.3       7.6
# WER on test(orig)           8.3       8.2       8.3
# WER on test(rescored)       7.8       7.7       7.8
# Final train prob        -0.0660   -0.0637   -0.0671
# Final valid prob        -0.0893   -0.0885   -0.0928
# Final train prob (xent)   -0.9079   -0.9122   -0.9451
# Final valid prob (xent)   -1.0068   -0.9988   -1.0393
# Num-params                 8642592   8675360   8675360

# 1f10r2 is supposed to be the same as 1f10r but generating the configs a different
#  way, using 'tdnnf-layer'.

# 1f10r is as 1f10q but changing all scales of 0.5 to 0.66, as in i->j (which
# was helpful there).
...
tdnn_affix=1f10r3  #affix for TDNN directory, e.g. "a" or "b", in case we change the configuration.
...
if [ $stage -le 17 ]; then
  ...
  tdnnf_opts="l2-regularize=0.004 dropout-proportion=0.0 bypass-scale=0.5"
  ...
  tdnnf2-layer name=tdnnf2 $tdnnf_opts dim=1024 bottleneck-dim=128 time-stride=1
  tdnnf2-layer name=tdnnf3 $tdnnf_opts dim=1024 bottleneck-dim=128 time-stride=1
  tdnnf2-layer name=tdnnf4 $tdnnf_opts dim=1024 bottleneck-dim=128 time-stride=1
  tdnnf2-layer name=tdnnf5 $tdnnf_opts dim=1024 bottleneck-dim=128 time-stride=0
  tdnnf2-layer name=tdnnf6 $tdnnf_opts dim=1024 bottleneck-dim=128 time-stride=3
  tdnnf2-layer name=tdnnf7 $tdnnf_opts dim=1024 bottleneck-dim=128 time-stride=3
  tdnnf2-layer name=tdnnf8 $tdnnf_opts dim=1024 bottleneck-dim=128 time-stride=3
  tdnnf2-layer name=tdnnf9 $tdnnf_opts dim=1024 bottleneck-dim=128 time-stride=3
  tdnnf2-layer name=tdnnf10 $tdnnf_opts dim=1024 bottleneck-dim=128 time-stride=3
  tdnnf2-layer name=tdnnf11 $tdnnf_opts dim=1024 bottleneck-dim=128 time-stride=3
