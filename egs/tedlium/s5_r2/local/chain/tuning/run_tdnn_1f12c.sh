...
tdnn_affix=1f12c  #affix for TDNN directory, e.g. "a" or "b", in case we change the configuration.
...
if [ $stage -le 17 ]; then
  ...
  attention_opts="l2-regularize=0.004 time-stride=3 num-heads=12 value-dim=60 key-dim=40 num-left-inputs=5 num-right-inputs=2"
