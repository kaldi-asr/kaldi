...
mkdir -p exp/segment_long_utts${affix}_train

# Config saved for reproduction of results
cat <<EOF > exp/segment_long_utts${affix}_train/segment_long_utts.conf
...
steps/cleanup/segment_long_utterances.sh --cmd "$train_cmd" \
  --stage $segment_stage \
  --config exp/segment_long_utts${affix}_train/segment_long_utts.conf \
