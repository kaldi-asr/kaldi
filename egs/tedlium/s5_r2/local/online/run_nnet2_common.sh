...
# added --num-threads default value too high train_diag_ubm.sh
if [ $stage -le 3 ]; then
  mkdir -p exp/nnet2_online
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 --num-frames 700000 \
    --num-threads 7 data/train_hires 512 exp/nnet2_online/tri4 exp/nnet2_online/diag_ubm
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    --num-threads 1 data/train_hires exp/nnet2_online/diag_ubm exp/nnet2_online/extractor || exit 1;
fi
