...
def VB_diarization(
  ...
  """
    ...
    if sp is None:
        sp = old_div(np.ones(maxSpeakers), maxSpeakers)
    else:
        maxSpeakers = len(sp)
    ...
    if q is None:
        ...
        q = old_div(q, q.sum(1, keepdims=True))
    ...
    mixture_sum = coo_matrix(
        (np.ones(C * D), (np.repeat(list(range(C)), D), list(range(C * D)))),
        shape=(C, C * D),
    )
    ...
    F_s = coo_matrix(
        (
            ...
            (
                NN.row.repeat(D),
                NN.col.repeat(D) * D + np.tile(list(range(D)), len(NN.col)),
            ),
        ),
        shape=(nframes, D * C),
    )
    ...
    if downsample is not None:
        ...
        downsampler = coo_matrix(
            (
                ...
                (
                    (np.ceil(old_div(np.arange(nframes), downsample))).astype(int),
                    np.arange(nframes),
                ),
            ),
            shape=(int(np.ceil(old_div((nframes - 1.0), downsample))) + 1, nframes),
        )
        NN = downsampler.dot(NN)
        VtiEF = downsampler.dot(VtiEF)
        G = downsampler.dot(G)
        q = old_div(downsampler.dot(q), downsample)
    else:
    ...
    for ii in range(maxIters):
        ...
        q, tll, lf, lb = forward_backward(
            lls.repeat(minDur, axis=1), tr, ip
        ...
        sp = q[0, ::minDur] + np.exp(
            ...
            + lb[1:, ::minDur]
            + lls[1:]
            + np.log((1 - loopProb) * sp)
            - tll
        ).sum(0)
        sp = old_div(sp, sp.sum())
...
def DER(q, ref, expected=True, xentropy=False):
    ...
    if not expected:
        ...
        q = np.zeros_like(q)
        q[list(range(len(q))), hard_labels] = 1
    ...
    acc = [
        err_mx[perm[: err_mx.shape[1]], list(range(err_mx.shape[1]))].sum()
