...
def create_ref(uttname, utt2num_frames, full_rttm_filename):
    ...
    for line in content:
        ...
        if uttname != uttname_line:
            continue
        start_time, duration = (
            int(float(line_split[3]) * 100),
            int(float(line_split[4]) * 100),
        )
        end_time = start_time + duration
        spkname = line_split[7]
        if spkname not in list(speaker_dict.keys()):
            spk_idx = num_spk + 2
...
def main():
    ...
    parser.add_argument(
        "--channel", type=int, default=0, help="Channel information in the rttm file"
    )
    parser.add_argument(
        "--initialize",
        type=int,
    ...
    assert (
        "<WEIGHTS>" in dubm_para
        and "<MEANS_INVVARS>" in dubm_para
        and "<INV_VARS>" in dubm_para
    )
    DUBM_WEIGHTS, DUBM_MEANS_INVVARS, DUBM_INV_VARS = (
        dubm_para["<WEIGHTS>"],
        dubm_para["<MEANS_INVVARS>"],
        dubm_para["<INV_VARS>"],
    )
    assert "M" in ie_para
    IE_M = np.transpose(ie_para["M"], (2, 0, 1))

    m = old_div(DUBM_MEANS_INVVARS, DUBM_INV_VARS)
    iE = DUBM_INV_VARS
    w = DUBM_WEIGHTS
    V = IE_M
    ...
    for utt in utt_list:
        ...
        if X_voiced.shape[0] == 0:
            print(
                "Warning: {} has no voiced frames in the initialization file".format(
                    utt
                )
            )
            continue
