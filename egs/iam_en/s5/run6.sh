...
nj=10
color=1
data_dir=data_gray_20_words
exp_dir=exp_gray_20_words
...
if [ $stage -le 1 ]; then
  for f in train val_1 val_2 test; do
    local/make_feature_vect.py $data_dir/$f --scale-size 40 --color $color | \
...
if [ $stage -le 3 ]; then
  ...
  #local/prepare_dict.sh $data_dir/test/ $data_dir/test/dict
  #utils/prepare_lang.sh --num-nonsil-states $numStates --position-dependent-phones false \
  #  $data_dir/test/dict "<sil>" $data_dir/lang_test$num_states/temp $data_dir/lang_test$num_states

  local/prepare_lm.sh --grammar-words true $data_dir/train/text  $data_dir/lang_test_${lang_other} 2 || exit 1;
fi
