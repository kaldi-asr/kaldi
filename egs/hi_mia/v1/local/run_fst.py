...
class Token(object):
    ...
    def transform(self, prev, olabel, is_self_arc, is_filler, score):
        if self.active == False or (
            self.active == True and self.score < prev.score + score
        ):
            self.score = prev.score + score
            if is_filler == False:
                t = prev.num_keyword_frames
                average_keyword_score = old_div(
                    (score + prev.average_keyword_score * t), (t + 1)
                )
                num_keyword_frames = t + 1
                if is_self_arc:
                    self.num_frames_of_current_state = (
                        prev.num_frames_of_current_state + 1
                    )
                    self.num_keyword_states = prev.num_keyword_states
                    self.max_score_of_current_state = max(
                        prev.max_score_of_current_state, score
                    )
                    self.average_max_keyword_score_before = (
                    ...
                    self.average_max_keyword_score_before = (
                        prev.average_max_keyword_score
                    )
                self.average_keyword_score = old_div(
                    (
                        self.max_score_of_current_state
                        + self.average_max_keyword_score_before
                        * (self.num_keyword_states - 1)
                    ),
...
class StateMachine(object):
    def __init__(self, stateFile):
        ...
        for line in f.readlines():
            ...
            if len(self.stateList) <= int(line.split()[0]):
                ...
                self.stateList[int(line.split()[0])].addEdge(
                    Edge(
                        line.split()[0],
                        line.split()[1],
                        line.split()[2],
    ...
    def runFst_oneLine(self, line):
        if len(line.split()) == 2:
            ...
            post = np.log(old_div(post, np.sum(post)))
            ...
            for num, tokens in enumerate(self.prev_tokens):
                if tokens.active:
                    for i in self.stateList[num].oedge:
                        ...
                        self.cur_tokens[i.getToState()].transform(
                            tokens, i.osymbol, is_self_arc, is_filler, score
                        )
            best_state = 0
            best_final_state = 0
            ...
            # print best_score
            if reach_final:
                self.postList[self.name] = np.exp(
                    self.cur_tokens[best_final_state].average_keyword_score
                )
                self.keyword_id = self.cur_tokens[best_final_state].keyword
                if (
                    self.cur_tokens[best_final_state].num_keyword_frames >= 0
                    and self.cur_tokens[best_final_state].num_frames_of_current_state
                    >= 5
                    and self.postList[self.name] > 0.5
                ):
                    self.spot = True
            ...
            self.frame_num = self.frame_num + 1
            if (
                self.frame_num > 100 * 60 * 10
                and self.prev_tokens[best_state].is_filler
            ):
                self.prev_tokens = [Token() for i in range(len(self.stateList))]
            ...
            if self.spot == False:
                self.postList[self.name] = np.exp(
                    self.prev_tokens[self.endState_id].average_keyword_score
                )
