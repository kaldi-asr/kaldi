...
cat $dir/scoring_kaldi/test_filt.txt | awk '{{for (i = 2; i <= NF; i++) printf(" %s", $i);} printf("\n"); }' | sed -e 's/\(\S\)/\1 /g' | paste $dir/scoring_kaldi/utt_id - > $dir/scoring_kaldi/test_filt.char || exit 1;
if [ $stage -le 0 ]; then
  ...
  for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    ...
    for LMWT in `seq $min_lmwt $max_lmwt`;do
      cat $dir/scoring_kaldi/penalty_$wip/${LMWT}.txt | awk '{{for (i = 2; i <= NF; i++) printf(" %s", $i);} printf("\n"); }' | sed -e 's/\(\S\)/\1 /g' | paste $dir/scoring_kaldi/utt_id - > $dir/scoring_kaldi/penalty_$wip/${LMWT}.char
    done
    ...
    $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring_kaldi/penalty_$wip/log/score.LMWT.log \
      cat $dir/scoring_kaldi/penalty_$wip/LMWT.char \| \
      compute-wer --text --mode=present \
      ark:$dir/scoring_kaldi/test_filt.char  ark,p:- ">&" $dir/wer_LMWT_$wip || exit 1;
...
if [ $stage -le 1 ]; then
  ...
  if $stats; then
    ...
    $cmd $dir/scoring_kaldi/log/stats1.log \
      cat $dir/scoring_kaldi/penalty_$best_wip/$best_lmwt.char \| \
      align-text --special-symbol="'***'" ark:$dir/scoring_kaldi/test_filt.char ark:- ark,t:- \|  \
      utils/scoring/wer_per_utt_details.pl --special-symbol "'***'" \| tee $dir/scoring_kaldi/wer_details/per_utt \|\
       utils/scoring/wer_per_spk_details.pl $data/utt2spk \> $dir/scoring_kaldi/wer_details/per_spk || exit 1;
    ...
    $cmd $dir/scoring_kaldi/log/wer_bootci.log \
      compute-wer-bootci --mode=present \
        ark:$dir/scoring_kaldi/test_filt.char ark:$dir/scoring_kaldi/penalty_$best_wip/$best_lmwt.char \
        '>' $dir/scoring_kaldi/wer_details/wer_bootci || exit 1;
