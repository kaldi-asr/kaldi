...
# This script has been modified by Yulan Liu to fit into the SWC folder structure. (7 Feb 2017)
...
if $use_ihm_ali; then
  gmm_dir=exp/ihm/$mode/${ihm_gmm}
  ali_dir=exp/${mic}/$mode/${ihm_gmm}_ali_${train_set}_sp_comb_ihmdata
  lores_train_data_dir=data/$mic/$mode/${train_set}_ihmdata_sp_comb
  tree_dir=exp/$mic/$mode/chain${nnet3_affix}/tree_bi${tree_affix}_ihmdata
  lat_dir=exp/$mic/$mode/chain${nnet3_affix}/${gmm}_${train_set}_sp_comb_lats_ihmdata
  dir=exp/$mic/$mode/chain${nnet3_affix}/tdnn${tdnn_affix}_sp_bi_ihmali
  # note: the distinction between when we use the 'ihmdata' suffix versus
  # 'ihmali' is pretty arbitrary.
else
  gmm_dir=exp/${mic}/$mode/$gmm
  ali_dir=exp/${mic}/$mode/${gmm}_ali_${train_set}_sp_comb
  lores_train_data_dir=data/$mic/$mode/${train_set}_sp_comb
  tree_dir=exp/$mic/$mode/chain${nnet3_affix}/tree_bi${tree_affix}
  lat_dir=exp/$mic/$mode/chain${nnet3_affix}/${gmm}_${train_set}_sp_comb_lats
  dir=exp/$mic/$mode/chain${nnet3_affix}/tdnn${tdnn_affix}_sp_bi
fi

train_data_dir=data/$mic/$mode/${train_set}_sp_hires_comb
train_ivector_dir=exp/$mic/$mode/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires_comb
...
if [ $stage -le 18 ]; then
  ...
  for decode_set in dev eval; do
      ...
      steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
          ...
          --online-ivector-dir exp/$mic/$mode/nnet3${nnet3_affix}/ivectors_${decode_set}_hires \
          --scoring-opts "--min-lmwt 5 " \
         $graph_dir data/$mic/$mode/${decode_set}_hires $dir/decode_${decode_set} || exit 1;
      ) || touch $dir/.error &
  done
  wait
  if [ -f $dir/.error ]; then
