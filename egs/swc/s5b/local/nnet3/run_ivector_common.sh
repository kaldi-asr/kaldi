...
#
# This script has been modified by Yulan Liu to fit into SWC folder structure. (7 Feb 2017)
...
num_threads_ubm=32
nnet3_affix=_cleaned     # affix for exp/$mic/$mode/nnet3 directory to put iVector stuff in, so it
                         # becomes exp/$mic/$mode/nnet3_cleaned or whatever.
...
gmmdir=exp/${mic}/$mode/${gmm}


for f in data/${mic}/$mode/${train_set}/feats.scp ${gmmdir}/final.mdl; do
  if [ ! -f $f ]; then
...
if [ $stage -le 2 ] && [ -f data/$mic/$mode/${train_set}_sp_hires/feats.scp ]; then
  echo "$0: data/$mic/$mode/${train_set}_sp_hires/feats.scp already exists."
...
if [ $stage -le 1 ]; then
  ...
  utils/data/perturb_data_dir_speed_3way.sh data/${mic}/$mode/${train_set} data/${mic}/$mode/${train_set}_sp
fi
...
if [ $stage -le 2 ]; then
  ...
  mfccdir=data/$mic/$mode/${train_set}_sp_hires/data
  ...
  for datadir in ${train_set}_sp dev eval; do
    utils/copy_data_dir.sh data/$mic/$mode/$datadir data/$mic/$mode/${datadir}_hires
  done

  # do volume-perturbation on the training data prior to extracting hires
  # features; this helps make trained nnets more invariant to test data volume.
  utils/data/perturb_data_dir_volume.sh data/$mic/$mode/${train_set}_sp_hires

  for datadir in ${train_set}_sp dev eval; do
    steps/make_mfcc.sh --nj $nj --mfcc-config conf/mfcc_hires.conf \
      --cmd "$train_cmd" data/$mic/$mode/${datadir}_hires
    steps/compute_cmvn_stats.sh data/$mic/$mode/${datadir}_hires
    utils/fix_data_dir.sh data/$mic/$mode/${datadir}_hires
  done
fi
...
if [ $stage -le 3 ]; then
  ...
  utils/data/combine_short_segments.sh \
     data/${mic}/$mode/${train_set}_sp_hires $min_seg_len data/${mic}/$mode/${train_set}_sp_hires_comb

  # just copy over the CMVN to avoid having to recompute it.
  cp data/${mic}/$mode/${train_set}_sp_hires/cmvn.scp data/${mic}/$mode/${train_set}_sp_hires_comb/
  utils/fix_data_dir.sh data/${mic}/$mode/${train_set}_sp_hires_comb/
fi
...
if [ $stage -le 4 ]; then
  ...
  temp_data_root=exp/$mic/$mode/nnet3${nnet3_affix}/tri5
  mkdir -p $temp_data_root

  utils/data/subset_data_dir.sh --utt-list data/${mic}/$mode/${train_set}/feats.scp \
          data/${mic}/$mode/${train_set}_sp_hires $temp_data_root/${train_set}_hires

  # note: essentially all the original segments should be in the hires data.
  n1=$(wc -l <data/${mic}/$mode/${train_set}/feats.scp)
  ...
  if [ -e exp/$mic/$mode/nnet3${nnet3_affix}/tri5/final.mdl ]; then
    # we don't want to overwrite old stuff, ask the user to delete it.
    echo "$0: exp/$mic/$mode/nnet3${nnet3_affix}/tri5/final.mdl already exists: "
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 7 --mllt-iters "2 4 6" \
     ...
     3000 10000 $temp_data_root/${train_set}_hires data/lang \
      $gmmdir exp/$mic/$mode/nnet3${nnet3_affix}/tri5
fi
...
if [ $stage -le 5 ]; then
  ...
  mkdir -p exp/$mic/$mode/nnet3${nnet3_affix}/diag_ubm
  temp_data_root=exp/$mic/$mode/nnet3${nnet3_affix}/diag_ubm
  ...
  num_utts_total=$(wc -l <data/$mic/$mode/${train_set}_sp_hires/utt2spk)
  num_utts=$[$num_utts_total/4]
  utils/data/subset_data_dir.sh data/$mic/$mode/${train_set}_sp_hires \
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    ...
    exp/$mic/$mode/nnet3${nnet3_affix}/tri5 exp/$mic/$mode/nnet3${nnet3_affix}/diag_ubm
fi
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    data/$mic/$mode/${train_set}_sp_hires exp/$mic/$mode/nnet3${nnet3_affix}/diag_ubm exp/$mic/$mode/nnet3${nnet3_affix}/extractor || exit 1;
fi
...
if [ $stage -le 7 ]; then
  ...
  ivectordir=exp/$mic/$mode/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires_comb
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    data/${mic}/$mode/${train_set}_sp_hires_comb ${temp_data_root}/${train_set}_sp_hires_comb_max2

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
    ${temp_data_root}/${train_set}_sp_hires_comb_max2 \
    exp/$mic/$mode/nnet3${nnet3_affix}/extractor $ivectordir

  # Also extract iVectors for the test data, but in this case we don't need the speed
  # perturbation (sp) or small-segment concatenation (comb).
  for data in dev eval; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj "$nj" \
      data/${mic}/$mode/${data}_hires exp/$mic/$mode/nnet3${nnet3_affix}/extractor \
      exp/$mic/$mode/nnet3${nnet3_affix}/ivectors_${data}_hires
  done
fi
