...
# notes: This script has been modified by Yulan Liu to fit into the SWC folder structure. (7 Feb 2017)
...
data=data/$mic/$mode/train
cleaned_data=data/$mic/$mode/train_${cleanup_affix}
srcdir=exp/$mic/$mode/$gmm
dir=exp/$mic/$mode/${gmm}_${cleanup_affix}_work
...
if [ $stage -le 2 ] && [ $mic != "ihm" ]; then
  ...
  local/prepare_parallel_train_data.sh $mic $mode
  ...
  utils/data/subsegment_data_dir.sh --segment-end-padding $padding \
    data/$mic/$mode/train_ihmdata $dir/segments $dir/text data/$mic/$mode/train_${cleanup_affix}_ihmdata
  # note, there will be no feats or CMVN in these directories.
fi

if [ $stage -le 3 ]; then
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    data/$mic/$mode/train_${cleanup_affix} data/lang exp/$mic/$mode/$gmm exp/$mic/$mode/${gmm}_ali_${cleanup_affix}
fi

if [ $stage -le 4 ]; then
  steps/train_sat.sh --cmd "$train_cmd" --stage "$train_stage" \
    5000 80000 data/$mic/$mode/train_${cleanup_affix} data/lang exp/$mic/$mode/${gmm}_ali_${cleanup_affix} exp/$mic/$mode/${gmm}_${cleanup_affix}
fi
...
if [ $stage -le 5 ]; then
  graph_dir=exp/$mic/$mode/${gmm}_${cleanup_affix}/graph_$LM
  nj_dev=$(cat data/$mic/$mode/dev/spk2utt | wc -l)
  nj_eval=$(cat data/$mic/$mode/eval/spk2utt | wc -l)

  $decode_cmd $graph_dir/mkgraph.log \
    utils/mkgraph.sh data/lang_$LM exp/$mic/$mode/${gmm}_${cleanup_affix} $graph_dir
  steps/decode_fmllr.sh --nj $nj --cmd "$decode_cmd" --config conf/decode.conf \
    $graph_dir data/$mic/$mode/dev exp/$mic/$mode/${gmm}_${cleanup_affix}/decode_dev_$LM
  steps/decode_fmllr.sh --nj $nj --cmd "$decode_cmd" --config conf/decode.conf \
    $graph_dir data/$mic/$mode/eval exp/$mic/$mode/${gmm}_${cleanup_affix}/decode_eval_$LM
fi
