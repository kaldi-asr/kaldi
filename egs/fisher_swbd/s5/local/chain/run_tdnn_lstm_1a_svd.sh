...
# First we train LSTMP-TDNN config chain model for few epochs on the multien data,
# Then, we perform SVD based refactoring of all the Affine components in the final.mdl,
...
# Commands used for performing  WER decoding on a dataset:
...
#         $graph_dir ${testset}_hires exp/chain/tdnn_lstm_1a_sp
