...
if [ $stage -le 5 ]; then
  ...
  # The self attention layer. In contrast to the stats-layer, the self-layer will
  # compute a weighted mean and sd of the whole input segment. The summation weights along the
  # segment are computed by the self-attention mechanism. 
  # Here, the self-attention mechanism is implemented by a two layer perceptron, which takes the
  # whole series of frames as input then output the weights. The wegiths are dotted with the
  # same series of frames and result in a fixed length vector.
  self-layer name=attention config=mean+stddev(0:1:1:${max_chunk_size}) num-heads=1 affine-dim=500

  # This is where we usually extract the embedding (aka xvector) from.
  relu-batchnorm-layer name=tdnn6 dim=512 input=attention
