...
lang_num= # how many speakers + 1 silence
...
# Check some files.
for f in $data/feats.scp $alidir/ali.ark $alidir/frame_counts $alidir/lang_num; do
  [ ! -f $f ] && echo "$0: no such file $f" && exit 1;
done


# Set some variables.
lang_num=`cat $alidir/lang_num` || exit 1;
num_leaves_lang=$lang_num
[ -z $num_leaves_lang ] && echo "\$num_leaves_lang is unset" && exit 1
[ "$num_leaves_lang" -eq "0" ] && echo "\$num_leaves_lang is 0" && exit 1

nj=30  # number of jobs in alignment dir...
...
if [ $stage -le -5 ]; then
  ...
  # create the config files for nnet initialization for lang recognition
...
if [ $stage -le -4 ] && [ -z "$egs_dir" ]; then
  ...
  echo "$0: calling get_egs_olr.sh "
  local/olr/nnet3/get_egs_olr.sh $egs_opts "${extra_opts[@]}" \
      --samples-per-iter $samples_per_iter --stage $get_egs_stage \
      --cmd "$cmd" $egs_opts \
      --frames-per-eg $frames_per_eg --lang-num $lang_num \
...
if [ $stage -le -2 ]; then
  echo "$0: lang recognition, preparing initial vector for FixedScaleComponent before softmax"
  ...
  awk -v power=$presoftmax_prior_scale_power -v smooth=0.01 \
     ...
     $alidir/frame_counts > $dir/presoftmax_prior_scale.vec
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    ( # this sub-shell is so that when we "wait" below,
      ...
      for n in $(seq $this_num_jobs); do
        ...
        $gpu_cmd $train_queue_opt $dir/log/train.$x.$n.log \
...
if [ $stage -le $num_iters ]; then
  ...
  $gpu_cmd $combine_queue_opt $dir/log/combine.log \
