...
# on top of speaker-independent features.  For more info on SGMMs, see the paper "The
...
# Check some files.
for f in $data/feats.scp $lang/L.fst $alidir/ali.1.gz $alidir/final.{mdl,mat} $ubm; do
...
cmvn_opts=`cat $alidir/cmvn_opts 2>/dev/null` # CMVN options.
