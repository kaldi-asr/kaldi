...
train_set_name=train_si284
dbn_dir=exp/dnn5b_pretrain-dbn
ce_dnn_dir=exp/dnn5b_pretrain-dbn_dnn
smbr_dnn_dir=exp/dnn5b_pretrain-dbn_dnn_smbr
#
...
if [ $stage -le 0 ]; then
  ...
  # train
  dir=$data_fmllr/$train_set_name
  steps/nnet/make_fmllr_feats.sh --nj 10 --cmd "$train_cmd" \
     --transform-dir ${gmmdir}_ali_si284 \
     $dir data/$train_set_name $gmmdir $dir/log $dir/data || exit 1
...
if [ $stage -le 1 ]; then
  ...
  $cuda_cmd $dir/log/pretrain_dbn.log \
    steps/nnet/pretrain_dbn.sh --rbm-iter 3 $data_fmllr/$train_set_name $dir || exit 1;
fi
...
if [ $stage -le 2 ]; then
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    ...
    $data_fmllr/${train_set_name}_tr90 $data_fmllr/${train_set_name}_cv10 data/lang $ali $ali $dir || exit 1;
...
if [ $stage -le 3 ]; then
  ...
  steps/nnet/align.sh --nj 100 --cmd "$train_cmd" \
    $data_fmllr/$train_set_name data/lang $srcdir ${srcdir}_ali || exit 1;
  steps/nnet/make_denlats.sh --nj 100 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt $acwt \
    $data_fmllr/$train_set_name data/lang $srcdir ${srcdir}_denlats || exit 1;
fi

if [ $stage -le 4 ]; then
  # Re-train the DNN by 1 iteration of sMBR
  steps/nnet/train_mpe.sh --cmd "$cuda_cmd" --num-iters 5 --acwt $acwt --do-smbr true \
    $data_fmllr/$train_set_name data/lang $srcdir ${srcdir}_ali ${srcdir}_denlats $dir || exit 1
  # Decode (reuse HCLG graph)
