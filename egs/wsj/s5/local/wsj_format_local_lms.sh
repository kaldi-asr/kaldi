...
for d in data/lang_test_bd_{tg,tgpr,tgconst,fg,fgpr,fgconst}; do
  rm -r $d 2>/dev/null
  cp -r data/lang_bd $d
done
...
gunzip -c $lm_srcdir_3g/lm_unpruned.gz | \
  ...
  --unk-symbol=$unk - data/lang_test_bd_tgconst/G.carpa || exit 1
...
gunzip -c $lm_srcdir_4g/lm_unpruned.gz | \
  utils/map_arpa_lm.pl $lang/words.txt | \
  arpa-to-const-arpa --bos-symbol=$bos --eos-symbol=$eos \
  --unk-symbol=$unk - data/lang_test_bd_fgconst/G.carpa || exit 1
