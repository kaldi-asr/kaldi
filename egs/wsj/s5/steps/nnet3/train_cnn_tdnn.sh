...
case $feat_type in
  globalraw) feat_dim=$(feat-to-dim --print-args=false scp:$data/feats.scp -) || \
...
if [ $stage -le -4 ]; then
  ...
  python steps/nnet3/make_cnn_tdnn_configs.py  \
    --cnn-indexes "$cnn_indexes"  \
    --cnn-reduced-dim $cnn_reduced_dim \
    --add-delta $conv_add_delta \
    --use-mfcc $use_mfcc \
...
if [ $stage -le -1 ]; then
  # Add the first layer; this will add in the
...
if $use_gpu; then
  parallel_suffix=""
  train_queue_opt="--gpu 1 -l mem_free=80G,ram_free=80G"
