...
def GetUttPartitionInfoParser():
    ...
    parser.add_argument("--ivector-dim", type=int, required=True)
...
def VerifyEgsDir(egs_dir, feat_dim, ivector_dim,
    ...
    chunk_widths_copy = copy.deepcopy(chunk_widths)
    utt_part_info_file = '{0}/info/utt_partition_info'.format(egs_dir)
    if os.path.exists(utt_part_info_file):
        utt_parser = GetUttPartitionInfoParser()
        for line in open(utt_part_info_file, 'r').readlines():
            print line
            args = utt_parser.parse_args(line.split())
            if (not((min_left_context <= args.chunk_left_context) and
                    (args.chunk_left_context <= max_left_context))
                or
                not((min_right_context <= args.chunk_right_context) and
                    (args.chunk_right_context <= max_right_context))):
