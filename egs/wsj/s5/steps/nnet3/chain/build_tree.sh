...
# This script builds a tree for use in the 'chain' systems.  This is just like
# the first stages of a standard system, like 'train_sat.sh', except it does
# 'convert-ali' to convert alignments to a monophone topology just created (in
# case the topology is different from where you got the system's alignments
# from), and it stops after the tree-building and model-initialization stage,
# without re-estimating the Gaussians or training the transitions.
