...
# --speed-perturbation=true/false --max-speed-perturb-rate=float
# --add-noise=float --max-snr=float --min-snr=float
# --volume-perturbation=true/false --max-volume-variance=float
#noise_option="--max-snr=$max_snr --min-snr=$min_snr"
noise_option="--time-shift=true --speed-perturbation=false --add-noise=-1 --volume-perturbation=false"
#noise_option="--time-shift=false --speed-perturbation=true --add-noise=-1 --volume-perturbation=false"
#noise_option="--time-shift=false --speed-perturbation=false --add-noise=0.85 --max-snr=$max_snr --min-snr=$min_snr --volume-perturbation=false"
#noise_option="--time-shift=false --speed-perturbation=false --add-noise=-1 --volume-perturbation=true"
  ...
  if [ $stage -le $x ]; then
    ...
    if [ $[$x%$diagnostic_period] == 0 ]; then
      ...
      $cmd JOB=1:$num_diagnostic_archives $dir/log/compute_prob_valid.$x.JOB.log \
        ...
        "ark:fvector-add-noise-separate $noise_option scp:$egs_dir/valid_diagnostic_egs.JOB.scp scp:$egs_dir/valid_diagnostic_egs.noise.JOB.scp ark:- | fvector-get-egs --left-padding=$left_padding --right-padding=$right_padding ark:- ark:- | nnet3-merge-egs --measure-output-frames=false ark:- ark:- |" &
      $cmd JOB=1:$num_diagnostic_archives $dir/log/compute_prob_train.$x.JOB.log \
        nnet3-xvector-compute-prob --compute-accuracy=${compute_accuracy} $dir/$x.raw \
        "ark:fvector-add-noise-separate $noise_option scp:$egs_dir/train_diagnostic_egs.JOB.scp scp:$egs_dir/train_diagnostic_egs.noise.JOB.scp ark:- | fvector-get-egs --left-padding=$left_padding --right-padding=$right_padding ark:- ark:- | nnet3-merge-egs --measure-output-frames=false ark:- ark:- |" &
    fi
    if [ $x -gt 0 ]; then
    ...
    ( # this sub-shell is so that when we "wait" below,
      ...
      for n in $(seq $this_num_jobs); do
        ...
        $cmd $train_queue_opt $dir/log/train.$x.$n.log \
          fvector-add-noise-separate $noise_option scp:"utils/shuffle_list.pl --srand $k $egs_dir/egs.$archive.scp |" scp:"utils/shuffle_list.pl --srand $k $egs_dir/egs.noise.$archive.scp |" ark:- \| \
