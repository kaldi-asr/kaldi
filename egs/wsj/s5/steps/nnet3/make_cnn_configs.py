...
for l in range(1, num_hidden_layers + 1):
    ...
    if l == 1:
        if args.ivector_dim > 0:
            print('component name=iv-affine type=NaturalGradientAffineComponent '
        ...
        for cl in range(0, num_cnn_layers):
            ...
            print('component name=L{0}_maxp type=Maxpooling3dComponent '
                  'input-x-dim={1} input-y-dim={2} input-z-dim={3} '
        ...
        cnn_affine_ouput = args.cnn_affine_dim;
        print('component name=cnn-affine type=NaturalGradientAffineComponent '
              'input-dim={0} output-dim={1}'.format(maxp_output, cnn_affine_ouput), file=f)

    cur_dim = (nonlin_output_dim * len(splice_array[l-1]) if l > 1 else cnn_affine_ouput + args.ivector_dim)
    ...
    if l == 1:
        ...
        print('component-node name=cnn-affine component=cnn-affine input={0}'.
              format(cur_input), file=f)
        cur_input='cnn-affine'

        if args.ivector_dim > 0:
            print('component-node name=iv-affine component=iv-affine input=ReplaceIndex(ivector, t, 0)',
                  file=f)
            cur_input='Append({0}, {1})'.format(cur_input, 'iv-affine')
    else:
