...
for f in $data/feats.scp ${alidir}_0/ali.1.gz ${alidir}_0/final.mdl ${alidir}_0/tree $extra_files; do
...
num_ali_jobs=$(cat ${alidir}_0/num_jobs) || exit 1;
...
[ -z "$transform_dir" ] && transform_dir=$alidir-0
...
case $feat_type in
  raw) feats="ark,s,cs:utils/filter_scp.pl --exclude $dir/valid_uttlist $sdata/JOB/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:- ark:- |"
   ...
   ;;
  lda)
    splice_opts=`cat $alidir-0/splice_opts 2>/dev/null`
    # caution: the top-level nnet training script should copy these to its own dir now.
    cp $alidir-0/{splice_opts,cmvn_opts,final.mat} $dir || exit 1;
    [ ! -z "$cmvn_opts" ] && \
...
if [ $stage -le 2 ]; then
  ...
  for i in `seq 0 $[$num_outputs-1]`; do
    for id in $(seq $num_ali_jobs); do gunzip -c ${alidir}_$i/ali.$id.gz; done | \
      copy-int-vector ark:- ark,scp:$dir/ali-$i.ark,$dir/ali-$i.scp || exit 1;
  done
fi
...
for i in `seq 0 $[$num_outputs-1]`; do
  num_pdfs_vec[$i]=$(tree-info --print-args=false ${alidir}_$i/tree | grep num-pdfs | awk '{print $2}')
done
if [ $stage -le 3 ]; then
  ...
  for i in `seq 0 $[$num_outputs-1]`; do
    ...
    <$dir/ali-$i.scp >$dir/ali_special_$i.scp
  done

  input_string=
  for i in `seq 0 $[$num_outputs-1]`; do
    input_string="$input_string \"ark,s,cs:ali-to-pdf ${alidir}_$i/final.mdl scp:$dir/ali_special_$i.scp ark:- | ali-to-post ark:- ark:- |\" ${num_pdfs_vec[$i]}"
  done

  echo input string is $input_string

  $cmd $dir/log/create_valid_subset.log \
    nnet3-get-egs-multi $valid_ivector_opt $valid_egs_opts "$valid_feats" \
    $input_string \
    "ark:$dir/valid_all.egs" || touch $dir/.error &
  $cmd $dir/log/create_train_subset.log \
    nnet3-get-egs-multi $train_subset_ivector_opt $valid_egs_opts "$train_subset_feats" \
    $input_string \
...
if [ $stage -le 4 ]; then
  ...
  input_string=
  for i in `seq 0 $[$num_outputs-1]`; do
    input_string="$input_string \"ark,s,cs:filter_scp.pl $sdata/JOB/utt2spk $dir/ali-$i.scp | ali-to-pdf ${alidir}_$i/final.mdl scp:- ark:- | ali-to-post ark:- ark:- |\" ${num_pdfs_vec[$i]}"
  done

  $cmd JOB=1:$nj $dir/log/get_egs.JOB.log \
    nnet3-get-egs-multi $ivector_opt $egs_opts --num-frames=$frames_per_eg "$feats" \
    $input_string ark:- \| \
