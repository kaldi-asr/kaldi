...
def Main():
  ...
  for lang in range(num_langs):
    ...
    logger.info("Number of examples for language {0} is {1}".format(lang, lang2len[lang]))
  ...
  for job_index in range(args.num_jobs):
    for archive_index in range(num_archives):
      ...
      logger.debug("Processing archive {0} for job {1}".format(archive_index + 1, job_index + 1))
      ...
      while num_egs <= this_num_egs_per_archive:
        ...
        if rem_egs > 0:
          ...
          if lang_len[lang_id] < args.minibatch_size:
            lang_len[lang_id] = 0
            logger.debug("Run out of data for language {0}".format(lang_id))
        else:
          logger.debug("Run out of data for all languages.")
          break
  ...
  for archive in range(num_archives):
    logger.debug("Processing archive {0} by combining all jobs.".format(archive + 1))
    this_ranges = []
        ...
        for line in scpfile:
            try:
              ...
              print("{0} {1}".format(scp_line[0], lang2weight[int(scp_line[2])]), file=w)
                ...
                logger.error("Failed processing line %s in scp %s", line,
                             scpfile.name)
                raise
      os.remove(scp)
