...
def  process_multilingual_egs(args):
    """ This script generates egs.archive.scp and ranges.* used for multilingual setup.
        ...
        To avoid loading whole files in memory, egs.scp for is processed
        line by line using readline() for input langs and to have more
        randomization across different archives,
    ...
    for job in range(args.num_jobs):
        for archive_index in range(num_archives):
            ...
            while num_egs <= this_num_egs_per_archive:
                ...
                if rem_egs > 0:
                    lang_id = select_random_lang(lang_len, rem_egs, rand_select)
...
def main():
    try:
        ...
        if args.email is not None:
            message = ("Generating egs.*.scp for multilingual training {dir} "
