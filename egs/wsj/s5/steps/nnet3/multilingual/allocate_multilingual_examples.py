...
def process_multilingual_egs(args):
    ...
    for lang in range(num_langs):
        ...
        logger.info("The proportion of egs from lang {} is {:.2f}. The number of blocks "
                    ...
                    "{}".format(lang, old_div(lang_to_num_examples[lang], tot_num_egs),
    ...
    for archive_index in range(num_archives + 1):  #  +1 is because we write to the last archive in two rounds
        ...
        num_remaining_blocks = old_div(num_remaining_egs, args.block_size)

        last_round = (archive_index == num_archives)
        if not last_round:
            num_blocks_this_archive = int(round(old_div(num_remaining_blocks, num_remaining_archives)))
        ...
        for block_index in range(num_blocks_this_archive):
            ...
            remaining_proportions = [old_div(remain, tot) for remain, tot in zip(lang_to_num_remaining_egs, lang_to_num_examples)]
