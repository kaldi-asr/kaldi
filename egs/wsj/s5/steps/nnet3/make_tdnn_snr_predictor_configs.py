...
if (sum([1 for x in [args.relu_dim, args.relu_dim, args.sigmoid_dims, args.sigmoid_dim, args.pnorm_input_dims, args.pnorm_input_dim] if x]) > 1
    or sum([1 for x in [args.relu_dim, args.relu_dim, args.sigmoid_dims, args.sigmoid_dim, args.pnorm_output_dims, args.pnorm_output_dim] if x]) > 1):
...
for l in range(1, num_hidden_layers + 1):
    ...
    if positivity_opts != '' or sparsity_opts != '':
        print('component name=affine{0} type=NaturalGradientPositiveAffineComponent '
              ...
              format(l, cur_dim, nonlin_input_dims[l-1], args.max_change_per_sample, positivity_opts, sparsity_opts), file=f)
    else:
        print('component name=affine{0} type=NaturalGradientAffineComponent '
              ...
              format(l, cur_dim, nonlin_input_dims[l-1], args.max_change_per_sample), file=f)
    if args.relu_dims is not None:
    ...
    print('component name=renorm{0} type=NormalizeComponent dim={1}'.format(
         l, nonlin_output_dims[l-1]), file=f)
    ...
    if positivity_opts != '' or sparsity_opts != '':
        print('component name=final-affine type=NaturalGradientPositiveAffineComponent '
              ...
              nonlin_output_dims[l-1], args.num_targets, args.max_change_per_sample, positivity_opts, sparsity_opts), file=f)
    else:
        print('component name=final-affine type=NaturalGradientAffineComponent '
              ...
              nonlin_output_dims[l-1], args.num_targets, args.max_change_per_sample), file=f)
