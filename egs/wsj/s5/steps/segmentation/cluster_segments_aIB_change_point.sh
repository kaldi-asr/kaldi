...
data_uniform_seg=$dir/${data_id}_uniform_seg_window${window}_ovlp${overlap}
...
if [ $stage -le 4 ]; then
  ...
  $cmd JOB=1:$reco_nj $seg_dir/log/compute_scores.JOB.log \
    ib-scoring-dense --input-factor=0 $clustering_opts \
...
if [ $stage -le 5 ]; then
  $cmd JOB=1:$reco_nj $seg_dir/log/calibrate.JOB.log \
    /export/a12/vmanoha1/kaldi-diarization-v2/src/ivectorbin/compute-calibration \
      ark,t:$seg_dir/scores.JOB.txt $seg_dir/threshold.JOB.txt 
  
  threshold=$(for n in `seq $reco_nj`; do cat $seg_dir/threshold.$n.txt; done | \
...
if [ $stage -le 6 ]; then
  $cmd JOB=1:$reco_nj $seg_dir/log/cluster_segments.JOB.log \
    agglomerative-cluster-ib --input-factor=0.0 $clustering_opts \
