...
# the "parallel7" script is as parallel6 but supports shrinking (by default
# will shrink on each iter.)
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    $cmd $parallel_opts $dir/log/compute_prob.$x.log \
      nnet-compute-prob $dir/$x.mdl ark:$dir/valid.egs || exit 1 &
    ...
    if $shrink; then
      $cmd $parallel_opts $dir/log/shrink.$x.log \
         nnet-shrink $dir/$[$x+1].mdl ark:$dir/valid.egs $dir/$[$x+1].mdl || exit 1;
    fi
...
$cmd $parallel_opts $dir/log/combine.log \
  nnet-am-combine $nnets_list ark:$dir/valid.egs $dir/final.mdl || exit 1;
