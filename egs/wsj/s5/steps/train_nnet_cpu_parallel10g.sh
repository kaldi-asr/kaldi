...
while [ $x -lt $num_iters ]; do
  ...
  if [ $stage -le $y ]; then # start off randomization for next time.
    ...
    echo "Spawning the process for randomization for iteration $y"
    (
      ...
      ! $cmd $dir/log/get_egs.$y.log \
        nnet-get-egs $nnet_context_opts "$feats" \
         ...
         nnet-copy-egs --srand=$y --keep-proportion=$keep_proportion ark:- $egs_list && touch $dir/egs_error && exit 1;
