...
if [ $stage -le -4 ]; then
  ...
  $cmd $dir/log/init_sgmm.log \
    sgmm2-init --spk-dep-weights=true --self-weight=$self_weight --pdf-map=$dir/pdf2group.map --phn-space-dim=$phn_dim \
...
while [ $x -lt $num_iters ]; do
   ...
   [ $x -ge $first_spkvec_iter ] && flags=${flags}u; # update spk-weight projections.
