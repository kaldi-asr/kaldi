...
hid_dim=2048   #number of units per layer or list hid_dimensions, separated by :
...
# num_hid = length of hid_dim
hid_dims=(${hid_dim//:/ })
if [ ${#hid_dims[@]} -gt 1 ]; then
  nn_depth=${#hid_dims[@]}
else
  for i in $(seq 1 $nn_depth); do
    hid_dims[$((i-1))]=$hid_dim
  done
fi
...
for depth in $(seq 1 $nn_depth); do
  ...
  if [ "$depth" == "1" ]; then
    num_vis=$num_fea
    num_hid=${hid_dims[$((depth-1))]}

    # This is usually Gaussian-Bernoulli RBM (not if CNN layers are part of input transform)
    # initialize
    echo "Initializing '$RBM.init'"
    echo "<NnetProto>
    <Rbm> <InputDim> $num_vis <OutputDim> $num_hid <VisibleType> $input_vis_type <HiddenType> bern <ParamStddev> $param_stddev_first
    </NnetProto>
    " > $RBM.proto
    ...
    echo "Initializing '$RBM.init'"
    echo "<NnetProto>
    <Rbm> <InputDim> $num_vis <OutputDim> $num_hid <VisibleType> bern <HiddenType> bern <ParamStddev> $param_stddev <VisibleBiasCmvnFilename> $dir/$depth.cmvn
    </NnetProto>
    " > $RBM.proto
    nnet-initialize $RBM.proto $RBM.init 2>$dir/log/nnet-initialize.$depth.log || exit 1
    #pre-train
