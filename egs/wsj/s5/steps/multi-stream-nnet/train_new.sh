...
if [ $# != 8 ]; then
   echo "Usage: $0 <data-train> <data-dev> <lang-dir> <ali-train> <ali-dev> <stream-indices> <stream-combination-list> <exp-dir>"
   echo " e.g.: $0 data/train data/cv data/lang exp/mono_ali_train exp/mono_ali_cv 0:84:168:252:336:420 stream_combination_ascii.list exp/mono_nnet"
   echo ""
   ...
   echo " Multi-stream: begin and end+1 feature indices of each stream"
   echo ""
...
cp $stream_combn_list $dir/stream_combination.list
...
if [ ! -z $feature_transform ]; then
  ...
  echo "# cmvn_opts='$cmvn_opts' delta_opts='$delta_opts'"
fi
...
steps/multi-stream-nnet/train_scheduler_new.sh --cmd "${cmd}" \
  ${scheduler_opts} \
  ...
  $nnet_init "$feats_tr" "$feats_cv" "$labels_tr" "$labels_cv" $stream_indices $dir || exit 1
