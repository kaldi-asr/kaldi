...
[ ! -e $dir/stream_combination.list ] && echo "$dir/stream_combination.list doesn't exist ... check" && exit 1;
stream_combn_list=$dir/stream_combination.list

##############################
# number of stream combinations
num_streams=`echo $stream_indices | awk -F ":" '{print NF-1}'`
num_stream_combns=`cat $stream_combn_list | wc -l`
stream_combns=`cat $stream_combn_list | tr -s '\n' ':' | rev | cut -c 2- | rev`
...
echo "stream combinations to train: `cat $stream_combn_list | tr -s '\n' ' '`"
...
stream_combn_str="\`sed -n JOBp $stream_combn_list\`"
dollar_sign='$'
if [ ! -e $dir/.done_iter00.initial.cv ]; then
  $cmd JOB=1:$num_stream_combns $dir/log/iter00.initial.comb.JOB.log \
  stream_combn=$stream_combn_str \; $train_tool --cross-validate=true --randomize=false --verbose=$verbose $train_tool_opts \
      ${frame_weights:+ "--frame-weights=$frame_weights"} \
      ${utt_weights:+ "--utt-weights=$utt_weights"} \
      "$feats_cv nnet-forward $feature_transform ark:- ark:- | apply-feature-stream-mask-new --cross-validate=true --stream-combinations=${dollar_sign}stream_combn $stream_indices ark:- ark:- |" "$labels_cv" $mlp_best || exit 1;
  ...
  if [ ! -e $dir/.done_iter${iter}.cv ]; then
    $cmd JOB=1:$num_stream_combns $dir/log/iter${iter}.cv.comb.JOB.log \
      stream_combn=$stream_combn_str \; $train_tool --cross-validate=true --randomize=false --verbose=$verbose $train_tool_opts \
        ${frame_weights:+ "--frame-weights=$frame_weights"} \
        ${utt_weights:+ "--utt-weights=$utt_weights"} \
        "$feats_cv nnet-forward $feature_transform ark:- ark:- | apply-feature-stream-mask-new --cross-validate=true --stream-combinations=${dollar_sign}stream_combn $stream_indices ark:- ark:- |" "$labels_cv" $mlp_next || exit 1
