...
if [ $stage -le 2 ]; then
  if [ ! -z "$ali_or_decode_dir" ]; then
    $cmd JOB=1:$nj $dir/log/extract_ivectors.JOB.log \
      ...
      weight-post --length-tolerance=2 ark:- "ark,s,cs:gunzip -c $dir/weights.gz|" ark:- \| \
      ivector-extract --length-tolerance=2 --acoustic-weight=$posterior_scale --compute-objf-change=true \
