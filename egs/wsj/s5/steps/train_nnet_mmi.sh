...
nnet_i=$nnet; nnet=$dir/0.nnet_nosoftmax;
...
while [ $x -le $num_iters ]; do
  ...
  if [ -f $dir/$x.nnet_nosoftmax ]; then
    echo "Skipped, file $dir/$x.nnet_nosoftmax exists"
  else
    $cmd $dir/log/mmi.$x.log \
     nnet-train-mmi-sequential \
       ...
       $cur_mdl $alidir/final.mdl "$feats" "$lats" "$ali" $dir/$x.nnet_nosoftmax || exit 1
  fi
  cur_mdl=$dir/$x.nnet_nosoftmax
  #add softmax to the nnet, so it can be used with decode/align script
  { cat $dir/$x.nnet_nosoftmax
    echo "<softmax> $num_tgt $num_tgt"
  } > $dir/$x.nnet
