...
num_valid_frames_shrink=5000 # a subset of the frames in "valid_utts", used only
                             # for diagnostics in this recipe, and for optimizing
                            # combination weights at the very end.
...
# use the previous iteration's shrinking parameters.
last_normal_shrink_iter=$[($num_hidden_layers-$initial_num_hidden_layers+1)*$add_layers_period]
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    if $shrink; then
      if [ $x -le $last_normal_shrink_iter ]; then
        # For earlier iterations, while we've recently beeen adding layers, just do shrinking
        # normally.
        $cmd $parallel_opts $dir/log/shrink.$x.log \
          nnet-shrink $dir/$[$x+1].mdl ark:$dir/valid_shrink.egs $dir/$[$x+1].mdl || exit 1;
      else
        wait $last_shrink_process;
        # Get the shrinking parameters from the last shrinking log.
        scales=`grep 'scale factors per layer are' $dir/log/shrink.$[$x-1].log | \
        ...
        $cmd $parallel_opts $dir/log/shrink.$x.log \
          nnet-shrink $dir/$[$x+1].mdl ark:$dir/valid_shrink.egs /dev/null &
        last_shrink_process=$!  # process-id of the process what we just spawned.
      fi
    fi
