...
# the parallel12 script is as parallel11 (building the tree), but now building
# a two level tree. 

# the parallel11 script is as parallel10, but building the tree ourselves,
# again.  This means we can control the #leaves.  Currently this is just a single
# level tree, but the plan is to go to multi-level tree and use the mixture-weights.
...
if [ $# != 6 ]; then
  echo "Usage: steps/train_nnet_cpu_parallel12.sh <data> <lang> <ali-dir> <exp-dir> <num-leaves-1level> <num-leaves-2level>"
  echo " e.g.: steps/train_nnet_cpu_parallel12.sh data/train_si84 data/lang exp/tri3b_ali exp/tri4y36_nnet 1000 4000"
  echo ""
...
# in this dir we'll have a different #jobs for neural net stuff.
sdata=$data/split$nj
...
if [ $stage -le -7 ]; then
  ...
  $cmd $dir/log/build_tree.log \
    build-tree-two-level --binary=false --verbose=1 --max-leaves-first=$num_leaves_first \
       --max-leaves-second=$num_leaves_second \
...
if [ $stage -le -6 ]; then
  # Use the alignment-dir alignments for the LDA (should make no difference,
  # but more convenient as we don't yet have a .mdl file in this directory.)
...
if [ $stage -le -5 ]; then
  ...
  if [ ! -z "$alpha" ]; then
    utils/nnet-cpu/make_nnet_config_preconditioned.pl --alpha $alpha $nnet_config_opts \
      ...
      $feat_dim $actual_num_leaves $num_hidden_layers $num_parameters \
  ...
  if [ $stage -le $y ]; then # start off randomization for next time.
    ...
    $cmd $parallel_opts $dir/log/randomize.$y.log \
     nnet-randomize-frames $nnet_context_opts --num-samples=$[$samples_per_iteration*$num_jobs_nnet] \
      --srand=$y "$feats" \
       "ark,cs:gunzip -c $dir/ali.*.gz | ali-to-pdf $dir/0.mdl ark:- ark:- |" ark:- \| \
