...
def AddOverlappingSpeech(room,  # the room selected
    ...
    if (len(speech_segments_list) > 0 and random.random() < overlapping_speech_addition_probability
        ...
        for k in range(1, random.randint(min_overlapping_speech_segments, max_overlapping_speech_segments) + 1):
            ...
            overlapping_speech_descriptor['start_times'].append(
                round(random.random()
                      * max(speech_dur - speech_segment.duration, 0), 2))
...
def GenerateReverberationAndOverlappedSpeechOpts(
    ...
    assert len(noise_addition_descriptor['noise_io']) == len(noise_addition_descriptor['noise_ids'])
...
def GenerateReverberatedWavScpWithOverlappedSpeech(
    ...
    for i in range(start_index, num_replicas+1):
        for recording_id in keys:
            ...
            max_overlapping_segments_recording = math.ceil(max_overlapping_segments_per_minute * speech_dur / 60)
...
def CreateReverberatedCopy(input_dir,
    ...
    GenerateReverberatedWavScpWithOverlappedSpeech(
               ...
               num_replicas = num_replicas, include_original=include_original, prefix = prefix,
    ...
    data_lib.CopyDataDirFiles(input_dir, output_dir, num_replicas, include_original=include_original, prefix=prefix)

    if output_reverb_dir is not None:
        data_lib.CopyDataDirFiles(input_dir, output_reverb_dir, num_replicas, include_original=include_original, prefix=prefix)

    if output_additive_noise_dir is not None:
        data_lib.CopyDataDirFiles(input_dir, output_additive_noise_dir, num_replicas, include_original=include_original, prefix=prefix)
