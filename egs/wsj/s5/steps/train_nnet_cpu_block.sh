...
# as train_nnet_cpu.sh but block-diagonal matrices, and randomization
# of order... (random sparsity).
...
if [ $stage -le -4 ]; then
  ...
  utils/nnet-cpu/make_nnet_config_block.pl  \
      --initial-num-hidden-layers $initial_num_hidden_layers $dir/hidden_layer.config \
     $feat_dim $num_leaves $num_hidden_layers $num_blocks $num_parameters \
