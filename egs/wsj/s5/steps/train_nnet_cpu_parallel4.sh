...
# this "parallel4" script is like the "parallel" script but modifying it
# so the randomization of frames is done in parallel with the training.
# It's like parallel3.sh in that respect, but that script does not update
# the learning rates.  We're also adding by default a couple of options
...
while [ $x -lt $num_iters ]; do
  ...
  wait # wait for any randomization from last time.
  y=$[$x+1];
  if [ $stage -le $y ] && [ $y -ge $start_parallel ]; then # start off randomization for next time.
    egs_list=
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    if [ $x -lt $start_parallel ]; then # Not parallel; just train in the standard way.
      ...
      $cmd $parallel_opts $dir/log/combine.$x.log \
         nnet-combine --verbose=3 \
            --num-bfgs-iters=10 \
           "$mdl" $nnets_list ark:$dir/valid.egs $dir/$[$x+1].mdl.tmp || exit 1;
      ...
      rm $dir/$[$x+1].mdl.tmp $nnets_list $egs_list
    fi
  fi
  x=$[$x+1]
done
