...
# *batch7.sh is as *batch5.sh, but adding in the nnet-am-fix stuff; also using
# the faster version of nnet-combine and adding the --num-threads option to it.
...
while [ $x -lt $num_tot_iters ]; do
  if [ $stage -le $x ]; then
    ...
    $cmd $parallel_opts $dir/log/combine.$x.log \
      nnet-subset-egs --n=$valid_frames_subset ark:$dir/valid.egs ark:- \| \
      nnet-combine-fast --minibatch-size=500 --num-threads=$num_threads \
        --verbose=3 --initial-model=0 "${all_models[@]}" ark:- - \| \
