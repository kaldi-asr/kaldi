...
def OneEMIter(args, word, stats, prons, pron_probs, debug=False):
    ...
    for i in range(len(pron_probs)):
        pron_probs[i] = old_div(pron_probs[i], s)
    log_like = 0.0
    for (utt, start_frame) in stats[word]:
        ...
        for i in range(len(prons[word])):
            prob_acc[i] += old_div(prob[i], sum(prob))
    ...
    for word in prons:
        ...
        while len(active_indexes) > 1:
            ...
            for i in active_indexes:
                ...
                pron_probs_mod = [
                    old_div(s, sum(pron_probs_mod)) for s in pron_probs_mod
                ]
                log_like2 = 1.0
                log_like2_last = -1.0
                num_iters2 = 0
            ...
            for i in range(len(pron_probs)):
                if i in active_indexes:
                    pron_probs[i] += 0.01
            pron_probs = [old_div(s, sum(pron_probs)) for s in pron_probs]
                    ...
                    if loss_abs > thr:
                        print(
                            "    or beta_{pd} is smaller than {}".format(
                                (old_div(loss_abs, thr) - 1) * len(stats[word])
                            ),
                            file=sys.stderr,
                        )
