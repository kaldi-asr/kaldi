...
"""This script contains some generic functions to work on data directories.
Some of these are required for nnet3 egs creation.
"""
...
def get_frame_shift(data_dir):
    """Reads frame-shift from the features in data directory and returns it"""
...
def generate_utt2dur(data_dir):
    """Adds utt2dur file to data_dir"""
    common_lib.run_kaldi_command(
        "utils/data/get_utt2dur.sh {0}".format(data_dir))


def get_utt2dur(data_dir):
    """Returns durations of utterances as a dictionary indexed by the
    utterance-id"""
...
def get_utt2uniq(data_dir):
    """Parses the file utt2uniq, if it exists, and returns two dictionaries:
        uniq2utt = { unique_utterance_id : list-of-utterances }
        utt2uniq = { utterance_id : unique_utterance_id }
    """
...
def get_num_frames(data_dir, utts = None):
    """Returns the total number of frames in the data directory"""
    generate_utt2dur(data_dir)
    frame_shift = get_frame_shift(data_dir)
    total_duration = 0
    utt2dur = get_utt2dur(data_dir)
    if utts is None:
        utts = utt2dur.keys()
    for utt in utts:
...
def create_data_links(file_names):
    """Wrapper around create_data_link.pl
