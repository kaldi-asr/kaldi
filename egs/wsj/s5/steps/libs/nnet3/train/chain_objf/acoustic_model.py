...
def create_phone_lm(dir, tree_dir, run_opts, lm_opts=None):
    ...
    This method trains a weighted phone LM for chain training generated by
    weighted combination of counts in alignments
    in "tree_dir"
    "tree_dir" is a list of comma-separated alignments directories with
    same tree and phone sets (each containing tree and ali.*.gz)
    and the colon-separated integer weight appended
    to each alignment dir.(if none, the weight is 1)
    i.e. ali_dir1:w1,ali_dir2:w2
    """
    tree_dirs = tree_dir.split(",")
    ali_dirs = []
    scales = []
    for tree_dir in tree_dirs:
        ali_and_weights = tree_dir.split(":")
        ali_dirs.append(ali_and_weights[0])
        if (len(ali_and_weights) == 2):
            scales.append(str(int(ali_and_weights[1])))
        else:
            scales.append(1)
    phone_lists = []
    for ali_dir in ali_dirs:
        ...
        assert(filecmp.cmp('{0}/tree'.format(ali_dirs[0]), '{0}/tree'.format(ali_dir)))

        alignments=' '.join(['{0}/ali.{1}.gz'.format(ali_dir, job)
                             for job in range(1, num_ali_jobs + 1)])
        phone_lists.append("""'ark:gunzip -c {0} | ali-to-phones {1}/final.mdl ark:- ark:-|'""".format(alignments, ali_dir))

    phone_lists_str=' '.join(phone_lists)
    if len(scales) > 0:
    ...
    chain-est-phone-lm {lm_opts} {phone_lists} {dir}/phone_lm.fst""".format(
        command=run_opts.command, dir=dir,
        lm_opts=lm_opts if lm_opts is not None else '',
        phone_lists=phone_lists_str))
