...
def create_phone_lm(dir, tree_dir, run_opts, alignment_dirs=None, lm_opts=None):
    ...
    This method trains a phone LM usingalignments.
    If alignment_dirs is non empty, it trains weighted phone LM for chain
    training generated by weighted combination of counts in alignments provided
    in "alignments_dir", which is a list of comma-separated alignment directories
    with same phone sets (each containing tree , final.mdl and ali.*.gz)
    and the colon-separated integer weights appended
    to each alignment dir.(if none, the weight is 1)
    i.e. alignment_dirs="ali_dir1:w1,ali_dir2:w2"
    If "alignment_dirs is empty, phone LM generated using alignments in tree_dir.
    """
    ali_dirs = []
    scales = []
    if alignment_dirs is not None:
        ali_weight_dirs = alignment_dirs.split(",")
        for ali_weight_dir in ali_weight_dirs:
            ali_and_weights = ali_weight_dir.split(":")
            ali_dirs.append(ali_and_weights[0])
            if (len(ali_and_weights) == 2):
                assert(ali_and_weights[1].isdigit())
                scales.append(ali_and_weights[1])
            else:
                scales.append("1")
    else:
        ali_dirs.append(tree_dir)

    phone_alignments_list = []
    for ali_dir in ali_dirs:
        try:
            f = open(ali_dir + "/num_jobs", 'r')
            num_ali_jobs = int(f.readline())
            assert num_ali_jobs > 0
        except:
            raise Exception("""There was an error getting the number of alignment
                            jobs from {0}/num_jobs""".format(ali_dir))

        common_lib.execute_command(
            """ utils/lang/check_phones_compatible.sh \
                    {0}/phones.txt {1}/phones.txt""".format(tree_dir, ali_dir))

        alignments=' '.join(['{0}/ali.{1}.gz'.format(ali_dir, job)
                             for job in range(1, num_ali_jobs + 1)])

        phone_alignments_list.append("""'ark:gunzip -c {0} | ali-to-phones {1}/final.mdl ark:- ark:-|'""".format(alignments, ali_dir))

    phone_alignments_list_str=' '.join(phone_alignments_list)
    if len(scales) > 0:
        scales_str=','.join(scales)
        lm_opts="{0} --scales={1}".format(lm_opts, scales_str)

    common_lib.execute_command(
        """{command} {dir}/log/make_phone_lm.log \
    chain-est-phone-lm {lm_opts} {phone_alignments_list} {dir}/phone_lm.fst""".format(
        command=run_opts.command, dir=dir,
        lm_opts=lm_opts if lm_opts is not None else '',
        phone_alignments_list=phone_alignments_list_str))
