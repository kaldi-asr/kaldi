...
def prepare_initial_network(dir, run_opts, srand=-3):
    """ This function prepares "0.raw" model by adding layers
        in final.config.
        If dir/init.raw exists, the layers in "final.config" added to "init.raw",
        otherwise the new model is initialized using final.config.
        If {dir}/configs/edit.config exists, the intermediate model "0.pre-edited.raw"
        is generated by modifying "init.raw" by adding layers in "final.config" and
        this model is eddited using edits.config as "0.raw".
        edits.config applied in cases where the initial network "init.raw"
        is copied from another setup (i.e. Weight transfer and we need to rename
        output nodes) and there is no init.config in config dir to generate "init.raw".
    ...
    """
    if os.path.exists("{0}/init.raw".format(dir)):
        ...
        assert(os.path.exists("{0}/configs/edits.config".format(dir)) or
                os.path.exists(edits_config_file))
