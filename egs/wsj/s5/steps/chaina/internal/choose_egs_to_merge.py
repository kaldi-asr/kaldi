...
def choose_egs(args):
    ...
    for sublist in chunk_to_sublist.values():
        ...
        num_groups = (len(sublist) +
                      args.chunks_per_spk - 1) // args.chunks_per_spk
        for i in range(num_groups):
            group = sublist[i*args.chunks_per_spk : (i+1)*args.chunks_per_spk]
            groups.append(group)
            if args.num_repeats == 2:
                group = sublist[i * args.chunks_per_spk + 2 :
                                (i + 1) * args.chunks_per_spk + 2]
                if group:
                    groups.append(group)
