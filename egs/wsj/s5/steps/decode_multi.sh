...
if [ $# != 4 ]; then
   echo "Usage: steps/decode.sh [options] <graph-dir> <data-dir> <decode-dir> <mapping>"
...
if [ -z "$model" ]; then # if --model <mdl> was not specified on the command line...
  if [ -z $iter ]; then model=$srcdir/model; 
...
for f in $sdata/1/feats.scp $sdata/1/cmvn.scp $graphdir/HCLG.fst; do
...
echo splice $splice_opts
echo cmvn opts $cmvn_opts
...
if [ $stage -le 0 ]; then
  $cmd $parallel_opts JOB=1:$nj $dir/log/decode.JOB.log \
    gmm-latgen-faster-multi --max-active=$max_active --beam=$beam --lattice-beam=$lattice_beam \
    --acoustic-scale=$acwt --allow-partial=true --word-symbol-table=$graphdir/words.txt --num-trees=$numtrees \
    $model $srcdir/tree $mapping $graphdir/HCLG.fst "$feats" "ark:|gzip -c > $dir/lat.JOB.gz" || exit 1;
fi
