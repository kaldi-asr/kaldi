...
if (@ARGV != 4) {
  print STDERR "Usage: make_nnet_config.pl  [options] <feat-dim> <num-leaves> " .
               "<num-hidden-layers> <num-parameters>  >config-file\n";
  exit(1);
}
...
for ($hidden_layer = 0; $hidden_layer < $num_hidden_layers; $hidden_layer++) {
