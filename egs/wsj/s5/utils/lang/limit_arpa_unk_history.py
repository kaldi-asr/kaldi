...
parser = argparse.ArgumentParser(
    description = "This script takes an existing ARPA lanugage model and limits the <unk> history to make it suitable for downstream <unk> modeling.\nIt supports up to 5-grams.", 
    usage = "utils/lang/limit_arpa_unk_history.py [--gzip] <oov_dict_entry> <input_arpa >output_arpa")

parser.add_argument('oov_dict_entry', help='oov identifier, for example <unk>', type=str)
parser.add_argument('--gzip', dest='compressed', action='store_true', help='whether the input/output streams are .gz compressed')
...
def get_ngram_stats(old_lm_lines):
    ...
    if len(ngram_counts) == 0:
        sys.exit("Couldn't get counts per ngram section. The input doesn't seem to be a valid ARPA language model.")
...
def find_and_replace_unks(old_lm_lines,max_ngrams,skip_rows):
    ...
    unk_pattern = re.compile("[0-9.-]+(?:[\s\\t]\S+){1,3}[\s\\t]"+args.oov_dict_entry+"[\s\\t](?!-[0-9]+\.[0-9]+).*")
    ...
    sys.stderr.write("Upadting the language model .. \n")
    ...
    sys.stderr.write("Removed {} lines including {} as second-to-last term\n".format(unk_row_count, args.oov_dict_entry))
    sys.stderr.write("Removed backoff probabilties from {} lines\n".format(backoff_row_count))

    return new_lm_lines, ngram_diffs


def read_old_lm():
    sys.stderr.write("Reading ARPA LM frome input stream .. \n")
    
    if args.compressed:
        input_stream = gzip.open(sys.stdin.buffer, 'rt')
    else:
...
def write_new_lm(new_lm_lines,ngram_counts,ngram_diffs):
    ...
    for i in range(10):
        ...
        if g:
            n = int(g.group(1))
            if n in ngram_diffs:
                new_num_ngrams = ngram_counts[n] + ngram_diffs[n] #ngram_diffs contains negative values
                new_lm_lines[i]="ngram {}={}\n".format(n,new_num_ngrams)
    
    if args.compressed:
        output_stream = gzip.open(sys.stdout.buffer, 'wt')
    else:
...
def main():
    ...
    new_lm_lines, ngram_diffs = find_and_replace_unks(old_lm_lines,max_ngrams,skip_rows)
