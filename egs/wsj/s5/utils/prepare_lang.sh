...
share_silence_phones=false  # if true, then share pdfs of different silence
                            # phones together.
share_state_roots=true      # if set, single root per phoneme (all states) is created. False, create root for each phoneme state
sil_prob=0.5
...
if [ $# -ne 4 ]; then
  ...
  echo "     --share-state-roots (true|false)                # default: true"
  echo "                                                     # if true, single decission tree (all states) for each phoneme is created"
  echo "                                                     # if false, create separate decission tree roots for each phoneme state"
...
if $share_silence_phones; then
  ...
  if $share_state_roots; then
    ...
    cat $dir/phones/sets.txt | awk '{print "not-shared", "split", $0;}' > $dir/phones/roots.txt
  fi
fi
