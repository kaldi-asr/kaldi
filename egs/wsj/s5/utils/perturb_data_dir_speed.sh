...
if [ -f $srcdir/segments ]; then
  ...
  awk '{wid=$1; $1=""; if ($NF=="|") {print wid $_ " sox -t wav - -t wav - speed '$factor' |"} else {print wid " sox -t wav" $_ " -t wav - speed '$factor' |"}}' $srcdir/wav.scp > $destdir/wav.scp
  if [ -f $srcdir/reco2file_and_channel ]; then
    cp $srcdir/reco2file_and_channel $destdir/
  fi
else # no segments->wav indexed by utt.
  if [ -f $srcdir/wav.scp ]; then
    awk '{wid=$1; $1=""; if ($NF=="|") {print "'$utt_prefix'" wid $_ " sox -t wav - -t wav - speed '$factor' |"} else {print "'$utt_prefix'" wid " sox -t wav" $_ " -t wav - speed '$factor' |"}}' $srcdir/wav.scp > $destdir/wav.scp
  fi
fi
...
echo "$0: generate speed-perturbed data from $srcdir to $destdir"
