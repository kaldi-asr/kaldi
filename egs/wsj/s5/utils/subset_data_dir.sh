...
if $perspk; then
  ...
  [ -f $srcdir/wav.scp ] && utils/filter_scp.pl $destdir/feats.scp <$srcdir/wav.scp >$destdir/wav.scp
  [ -f $srcdir/text ] && utils/filter_scp.pl $destdir/feats.scp <$srcdir/text >$destdir/text
  ...
  if $shortest; then
    ...
    # create feats.scp
    utils/subset_scp.pl $first_opt $numutt $srcdir/feats.scp > $destdir/feats.scp || exit 1;
  fi
 
  if [ -f $srcdir/wav.scp ]; then
    utils/filter_scp.pl $destdir/feats.scp $srcdir/wav.scp > $destdir/wav.scp || exit 1;
  else
    rm $destdir/wav.scp 2>/dev/null
  fi

  if [ -f $srcdir/utt2spk ]; then
    utils/filter_scp.pl $destdir/feats.scp $srcdir/utt2spk > $destdir/utt2spk|| exit 1;
