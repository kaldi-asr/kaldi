...
if [ $# != 3 ]; then
   echo "Usage: scripts/decode_tri2i.sh <graph> <decode-dir> <job-number>"
   exit 1;
fi
...
mat=exp/tri2j/final.mat
graph=$1
dir=$2
job=$3
scp=$dir/$job.scp
feats="ark:add-deltas --delta-order=3 scp:$scp ark:- | transform-feats $mat ark:- ark:- |"
