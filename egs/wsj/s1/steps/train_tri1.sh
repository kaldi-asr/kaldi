...
# tri1 is triphone training using 3500 utterances 
# from the si-84 training set, and starting
# from the monophone models.  
...
realign_iters="5 10 20";
numleaves=2000
numgauss=3000 # initial num-gauss
...
# This one uses half the SI-84 data.  We don't bother with temporary archives
# here as the effect on disk I/O is smaller; also, because we use sub-parts of
# the features, the scripting would get more complicated.

head -7138 data/train.scp | scripts/subset_scp.pl 3500 - > $dir/train.scp
head -7138 data/train.tra | scripts/subset_scp.pl 3500 - > $dir/train.tra
...
while [ $x -lt $numiters ]; do
   ...
   gmm-acc-stats-ali --binary=false $dir/$x.mdl "$feats" "ark:gunzip -c $dir/cur?.ali.gz|" $dir/$x.acc 2> $dir/acc.$x.log  || exit 1;
