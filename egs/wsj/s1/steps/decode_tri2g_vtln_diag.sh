...
# tri2g_vtln is doing normal, feature-level VTLN (with diagonal
# fMLLR).
...
if [ $# != 3 ]; then
   echo "Usage: steps/decode_tri2g.sh <graph> <decode-dir> <job-number>"
   exit 1;
fi
...
model=exp/tri2g/final.mdl # Model to decide which transform to use.
...
feats="ark:compute-mfcc-feats $utt2spk_opt --vtln-low=100 --vtln-high=-600 --vtln-map=ark:$dir/$job.factor --config=conf/mfcc.conf scp:$dir/${job}_wav.scp ark:- | add-deltas ark:- ark:- |"
