...
if [ $# != 3 ]; then
   echo "Usage: scripts/decode_tri2d.sh <graph> <decode-dir> <job-number>"
   exit 1;
fi
...
mat=exp/tri2d/final.mat
graph=$1
dir=$2
job=$3
scp=$dir/$job.scp
feats="ark:add-deltas --print-args=false scp:$scp ark:- | transform-feats $mat ark:- ark:- |"

filenames="$scp $model $mat $graph data/words.txt"
