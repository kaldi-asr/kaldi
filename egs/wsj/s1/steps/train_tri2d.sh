...
# tri2d is as tri2a but with MLLT on top of the delta+delta-delta features.
# See tri3d for a full-size (si-284) system that builds from this.
...
while [ $x -lt $numiters ]; do
   ...
   if echo $mllt_iters | grep -w $x >/dev/null; then # Do MLLT update.
     ...
     cur_mllt=$dir/$x.mat
     feats="ark,s,cs:add-deltas --print-args=false scp:$dir/train.scp ark:- | transform-feats $cur_mllt ark:- ark:- |" 
     for n in 1 2 3; do
       featspart[$n]="ark,s,cs:add-deltas --print-args=false scp:$dir/train$n.scp ark:- | transform-feats $cur_mllt ark:- ark:- |" 
     done
   else # do GMM update
