...
if [ $# != 3 ]; then
   echo "Usage: scripts/decode_tri3a_fmllr.sh <graph> <decode-dir> <job-number>"
   exit 1;
fi
...
ali-to-post ark:$dir/$job.pre_ali ark:- | \
  ...
  gmm-est-fmllr $spk2utt_opt --fmllr-min-count=$mincount $model "$sifeats" ark,o:- ark:$dir/$job.fmllr 2>$dir/fmllr${job}.log
