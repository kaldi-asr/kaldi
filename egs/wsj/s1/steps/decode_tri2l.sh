...
if [ $# != 3 ]; then
   echo "Usage: steps/decode_tri2k.sh <graph> <decode-dir> <job-number>"
   exit 1;
fi
...
max_active=7000
alimodel=exp/tri2l/final.alimdl # first-pass model...
model=exp/tri2l/final.mdl
mat=exp/tri2l/final.mat
...
filenames="$scp $model $alimodel $mat $graph data/words.txt"
...
ali-to-post ark:$dir/$job.pre_ali ark:- | \
  ...
  gmm-est-fmllr-gpost --fmllr-min-count=$mincount $spk2utt_opt $model "$sifeats" ark,o:- \
     ark:$dir/$job.trans 2>$dir/fmllr${job}.log
