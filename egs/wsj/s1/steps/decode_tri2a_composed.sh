...
if [ $# != 5 ]; then
   echo "Usage: steps/decode_tri2a.sh <graph> <decode-dir> <job-number> <Gminus> <Gprime>"
   exit 1;
fi
...
gmm-decode-biglm-faster --beam=$beam --acoustic-scale=$acwt --max-active=7000 --word-symbol-table=data/words.txt $model $graph $gminus $gprime "$feats" ark,t:$dir/$job.tra ark,t:$dir/$job.ali  2>>$dir/decode${job}.log 
