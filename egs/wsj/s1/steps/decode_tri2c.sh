...
if [ $# != 3 ]; then
   echo "Usage: scripts/decode_tri2c.sh <graph> <decode-dir> <job-number>"
   exit 1;
fi
...
if [ -f $dir/$job.spk2utt ]; then # The user ran decode.sh with --per-spk
...
compute-cmvn-stats $spk2utt_opt "ark:add-deltas --print-args=false scp:$scp ark:- |" \
    ark:$dir/$job.cmvn

feats="ark:add-deltas --print-args=false scp:$scp ark:- | apply-cmvn $utt2spk_opt ark:$dir/$job.cmvn ark:- ark:- |"
