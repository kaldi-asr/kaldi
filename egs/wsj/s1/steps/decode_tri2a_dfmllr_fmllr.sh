...
# not using SAT).
gmm-decode-faster --beam=$prebeam --max-active=$max_active --acoustic-scale=$acwt --word-symbol-table=data/words.txt $model $graph "$sifeats" ark,t:$dir/$job.pre1.tra ark,t:$dir/$job.pre1.ali  2>>$dir/predecode1.${job}.log 

# Estimate fMLLR transforms (diagonal)
ali-to-post ark:$dir/$job.pre1.ali ark:- | \
  weight-silence-post 0.0 $silphones $model ark:- ark:- | \
  gmm-est-fmllr $spk2utt_opt --fmllr-update-type=diag --fmllr-min-count=$mincount $model "$sifeats" ark,o:- ark:$dir/$job.fmllr1 2>$dir/fmllr1.${job}.log

feats="ark:add-deltas --print-args=false scp:$scp ark:- | transform-feats $utt2spk_opt ark:$dir/$job.fmllr1 ark:- ark:- |"
...
# Estimate fMLLR transforms (full)
ali-to-post ark:$dir/$job.pre2.ali ark:- | \
  weight-silence-post 0.0 $silphones $model ark:- ark:- | \
  gmm-post-to-gpost $model "$feats" ark:- ark:- | \
  gmm-est-fmllr-gpost $spk2utt_opt --fmllr-min-count=$mincount $model "$sifeats" ark,o:- ark:$dir/$job.fmllr2 2>$dir/fmllr2.${job}.log

feats="ark:add-deltas --print-args=false scp:$scp ark:- | transform-feats $utt2spk_opt ark:$dir/$job.fmllr2 ark:- ark:- |"
