...
if [ $# != 8 ]; then
   echo "Usage: steps/train_lda_mllt.sh <num-leaves> <tot-gauss> <data-dir-bn> <data-dir-mfcc> <lang-dir> <ali-dir> <ali-mono> <exp-dir>"
   echo " e.g.: steps/train_lda_mllt.sh 2500 15000 data/train_si84 data/lang data_mfcc exp/tri2b_ali_si84 exp/mono_ali_si84 exp/tandem"
   exit 1;
fi
...
for n in `get_splits.pl $nj`; do
  $cmd $dir/log/lda_acc.$n.log \
    ali-to-post "ark:gunzip -c $malidir/$n.ali.gz|" ark:- \| \
      weight-silence-post 0.0 $silphonelist $malidir/final.mdl ark:- ark:- \| \
      acc-lda --rand-prune=$randprune $malidir/final.mdl "${splicedfeatspart[$n]}" ark,s,cs:- \
