...
for n in 0 1 2 3; do
  featspart[$n]="ark,s,cs:apply-cmvn --norm-vars=false --utt2spk=ark:$data/split4/$n/utt2spk ark:$alidir/$n.cmvn scp:$data/split4/$n/feats.scp ark:- | splice-feats ark:- ark:- | transform-feats $dir/final.mat ark:- ark:- |"
  if [ -f $alidir/0.trans ]; then
...
acc-tree-stats  --ci-phones=$silphonelist $alidir/final.mdl "$feats" "ark:gunzip -c $alidir/*ali.gz|" \
  $dir/treeacc 2> $dir/log/acc_tree.log || exit 1;
...
gmm-init-model  --write-occs=$dir/0.occs  \
    $dir/tree $dir/treeacc $lang/topo $dir/0.gmm 2> $dir/log/init_gmm.log || exit 1;
...
sgmm-init --phn-space-dim=$phndim --spk-space-dim=$spkdim $lang/topo $dir/tree $ubm \
    $dir/0.mdl 2> $dir/log/init_sgmm.log || exit 1;

rm $dir/.error 2>/dev/null
echo "Doing Gaussian selection"
for n in 0 1 2 3; do
  ( sgmm-gselect $dir/0.mdl "${featspart[$n]}" ark,t:- | \
    gzip -c > $dir/$n.gselect.gz ) 2>$dir/log/gselect$n.log \
    || touch $dir/.error &
done
wait;
...
while [ $x -lt $numiters ]; do
   ...
   if echo $realign_iters | grep -w $x >/dev/null; then
      ...
      for n in 0 1 2 3; do
        sgmm-align-compiled ${spkvecs_opt[$n]} $scale_opts "${gselect_opt[$n]}" \
            --utt2spk=ark:$data/split4/$n/utt2spk --beam=8 --retry-beam=40 \
   ...
   if [ $spkdim -gt 0 ] && echo $spkvec_iters | grep -w $x >/dev/null; then
     for n in 0 1 2 3; do
      ( ali-to-post "ark:gunzip -c $dir/$n.ali.gz|" ark:- | \
          ...
          sgmm-est-spkvecs --spk2utt=ark:$data/split4/$n/spk2utt \
           ${spkvecs_opt[$n]} "${gselect_opt[$n]}" \
          --rand-prune=$randprune $dir/$x.mdl \
         "${featspart[$n]}" ark,s,cs:- ark:$dir/tmp$n.vecs ) 2>$dir/log/spkvecs.$x.$n.log && \
   ...
   sgmm-est --update-flags=$flags --split-substates=$numsubstates $add_dim_opts \
     ...
     $dir/$[$x+1].mdl 2> $dir/log/update.$x.log || exit 1;
  ...
  while [ $y -lt $numiters_alimdl ]; do
    ...
    ( ali-to-post "ark:gunzip -c $dir/$n.ali.gz|" ark:- | \
      sgmm-post-to-gpost ${spkvecs_opt[$n]} "${gselect_opt[$n]}" \
        --utt2spk=ark:$data/split4/$n/utt2spk $dir/$x.mdl "${featspart[$n]}" ark,s,cs:- ark:- | \
      sgmm-acc-stats-gpost --update-flags=$flags  $cur_alimdl "${featspart[$n]}" \
         ark,s,cs:- $dir/$y.$n.aliacc ) 2> $dir/log/acc_ali.$y.$n.log || touch $dir/.error &
    done
    wait;
    ...
    sgmm-est --update-flags=$flags --remove-speaker-space=true $cur_alimdl \
       "sgmm-sum-accs - $dir/$y.*.aliacc|" $dir/$[$y+1].alimdl 2>$dir/log/update_ali.$y.log || exit 1;
