...
parser.add_argument(
    "num_nonsil_states", type=int, help="number of states for nonsilence phones"
)
parser.add_argument(
    "num_sil_states", type=int, help="number of states for silence phones"
)
parser.add_argument(
    "num_punctuation_states", type=int, help="number of states for punctuation"
)
parser.add_argument(
    "nonsilence_phones",
...
parser.add_argument(
    "phone_list", help="file containing all phones and their corresponding number."
)
...
for x in range(0, args.num_nonsil_states):
    xp1 = x + 1
    print(
        "<State> {0} <PdfClass> {0} <Transition> {0} 0.75 <Transition> {1} 0.25 </State>".format(
            x, xp1
        )
    )
...
if args.num_sil_states > 1:
    ...
    print(
        "<State> {0} <PdfClass> {0} <Transition> {0} 0.75 <Transition> {1} 0.25 </State>".format(
            second_last, args.num_sil_states
        )
    )
