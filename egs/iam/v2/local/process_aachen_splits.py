...
parser = argparse.ArgumentParser(
    description="""Creates text, utt2spk
                                                and images.scp files."""
)
parser.add_argument(
    "database_path", type=str, help="Path to the downloaded (and extracted) IAM data"
)
parser.add_argument("split_path", type=str, help="location of the train/test/val set")
parser.add_argument("out_dir", type=str, help="location to write output files.")
parser.add_argument(
    "--dataset",
    type=str,
    default="train",
...
dataset_path = os.path.join(args.split_path, args.dataset + ".uttlist")
...
with open(dataset_path) as f:
    for line in f:
        ...
        lines_path = os.path.join(args.database_path, "lines", outerfolder, innerfolder)
