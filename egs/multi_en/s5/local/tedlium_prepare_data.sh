...
###########################################################################################
# This script was copied from egs/tedlium/s5/local/prepare_data.sh
...
;;'
    ...
    cat $data_src/$set/stm/*.stm | sort -k1,1 -k2,2 -k4,4n | \
      sed -e 's:<F0_M>:<o,f0,male>:' \
          ...
          -e 's:([^ ]*)$::' | \
      awk '{ $2 = "A"; print $0; }'
  } | local/tedlium_join_suffix.py $data_src/TEDLIUM.150K.dic > data/tedlium/$set/stm 
