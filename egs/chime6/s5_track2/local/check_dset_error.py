...
def get_args():
    ...
    parser.add_argument(
        "wer_dir_path", type=str, help="path of directory containing wer files"
    )
    parser.add_argument(
        "output_dir_path",
        type=str,
...
def main():
    ...
    for line in open(recording_spkorder_file, "r", encoding="utf8"):
        ...
        assert int(total_words) == int(
            recodingid_error_dict[recordingid][0]
        ), "Total words mismatch"
        assert int(total_errors) == int(
            recodingid_error_dict[recordingid][1]
        ), "Total errors mismatch"
        assert int(total_ins) == int(
            recodingid_error_dict[recordingid][2]
        ), "Total insertions mismatch"
        assert int(total_del) == int(
            recodingid_error_dict[recordingid][3]
        ), "Total deletions mismatch"
        assert int(total_sub) == int(
            recodingid_error_dict[recordingid][4]
        ), "Total substitutions mismatch"
