...
def to_samples(time: str):
    ...
    samples = hours * 3600 * SAMPLE_RATE + minutes * 60 * SAMPLE_RATE + seconds_samples
    return int(samples)
        ...
        for uttid in session_dict:
            try:
                ...
                start_sample_str = str(
                    int(old_div(start_sample * 100, SAMPLE_RATE))
                ).zfill(7)
                end_sample_str = str(int(old_div(end_sample * 100, SAMPLE_RATE))).zfill(
                    7
                )
