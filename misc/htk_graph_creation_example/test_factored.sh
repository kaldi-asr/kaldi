...
../../../src/bin/make-pdf-to-tid-transducer kaldi.mdl > test/p2t.fst
...
cat statesyms.txt | awk 'BEGIN{ print "<eps> 0"; } {printf("%s %d\n", $1, $2+1);} ' > test/pdfplusone.txt
 ...
 fstcompose Ha.fst test/fst.test | ../../../src/bin/add-self-loops --reorder=true kaldi.mdl | \
  fstrandgen | fstcompose test/p2t.fst - | fstprint --isymbols=test/pdfplusone.txt --osymbols=words.txt \
 | awk '{print $3}' | grep -v -w eps |  uniq | awk '{if(NF>0){print;}}' > test/kaldi.state.seq


cmp test/kaldi.state.seq  test/htk.state.seq  || exit 1;
