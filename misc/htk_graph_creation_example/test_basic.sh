...
fstcompose H.fst test/fst.test | fstrandgen | fstcompose test/p2t.fst - | fstprint --isymbols=test/pdfplusone.txt --osymbols=words.txt | awk '{print $3}' | grep -v -w eps |  uniq | awk '{if(NF>0){print;}}' > test/kaldi.state.seq
