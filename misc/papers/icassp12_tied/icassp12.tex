...
$^3$Microsoft Research, Redmond, WA, {\sc USA}\\
...
\begin{abstract}
In the past decade, semi-continuous hidden Markov models (HMMs) have not attracted 
much attention in the speech recognition community. The strongly growing amount
of training data and continuous improvements of the initialization and training
procedures led to the impression that continuous HMMs are the best choice of
acoustic model.
%
However, recent work on tasks involving cross-lingual speech recognition and
languages with little training data faced the same the old problem -- millions
of parameters require a large amount of (unavailable) training data -- thus people are
investigating methods of reducing the number of parameters while maintaining or
...
considerably better with the growing amount of training data available.
%
...
state is reduced.
%
This is the probably strongest advantage of semi-continuous models: While the
codebook Gaussians are always estimated on the whole data set, the only state
specific variables are the weights of the Gaussians, i.e.~increasing the number
of states only slightly increases the number of parameters, and the Gaussian
means and variances are still estimated using all the data; the reduced number
of Gaussian parameters for a large data set also allows to robustly estimate
full covariances.
%
This allows to reliably estimate arbitrary large phonetic context 
(``polyphones''), that showed good performance for the recognition of 
spontaneous  speech \cite{schukattalamazzini1994srf,schukattalamazzini1995as} 
while having a rather small number of parameters.
%
...
and adapted in a solely acoustic way.
...
Gaussian mixture model. We initialize the mixtures with a single component
each, and subsequently allocate more components by splitting components every 
iteration until a certain total number of components is reached (in this case
about 9000). We allocate the Gaussians proportional to a small power (e.g.~0.2) 
of the state's occupation count.
Formally, the emission probability of tree leaf $j$ is computed as
\begin{equation}
...
\section{Summary}
In this article, we evaluated semi-continuous and two-level tree based
semi-continuous acoustic models of the {\sc Kaldi} toolkit in the context of
both continuous and SGMM models.
%
While the first could not produce convincing results, the two-level tree 
based multiple-codebook architecture shows promising performance, especially for 
