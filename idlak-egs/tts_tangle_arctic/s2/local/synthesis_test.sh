...
python local/make_fullctx_mlf_dnn.py $datadir/synth_lab.mlf $datadir/cex.ark $datadir/feat.ark
...
rm -rf $outdir
local/make_forward_fmllr.sh $dnndir $lbldir $outdir ""

# 5. Vocoding
# NB: these are the settings for 16k
mkdir -p $outdir/wav_mlpg/; for cmp in $outdir/cmp/*.cmp; do
    local/mlsa_synthesis_63_mlpg.sh --voice_thresh $voice_thresh --alpha $alpha --fftlen $fftlen --srate $srate --bndap_order $bndap_order --mcep_order $mcep_order --delta_order $delta_order $cmp $outdir/wav_mlpg/`basename $cmp .cmp`.wav $var_cmp
done
